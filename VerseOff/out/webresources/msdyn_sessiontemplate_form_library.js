...
(function (ClientRuntime) {
    ...
    SessionTemplateEntity.msdyn_type = "msdyn_type";
    SessionTemplateEntity.msdyn_entity = "msdyn_entity";
    SessionTemplateEntity.msdyn_anchortab = "msdyn_anchortab";
    SessionTemplateEntity.msdyn_sessiontype = "msdyn_sessiontype";
    SessionTemplateEntity.msdyn_sessionicon = "msdyn_sessionicon";
    SessionTemplateEntity.msdyn_enabletitlesync = "msdyn_enabletitlesync";
    SessionTemplateEntity.genericAnchorTabsViewId = "{595164c2-5b22-421b-9ca5-4a4b758bc4fb}";
    SessionTemplateEntity.entityAnchorTabsViewId = "{aa552954-8242-ef11-bfe2-6045bd0171a0}";
    SessionTemplateEntity.anchorTabSupportedEntity = new Set(["account", "contact", "incident"]);
    ClientRuntime.SessionTemplateEntity = SessionTemplateEntity;
    class SessionTemplateConstants {
    }
    SessionTemplateConstants.WebresourceName = "Localization/CEC_webresource_strings";
    SessionTemplateConstants.alertPopupTitle = "AlertPopupTitle";
    SessionTemplateConstants.alertPopupMessage = "AlertPopupMessage";
    SessionTemplateConstants.alertPopupButton = "AlertPopupButton";
    SessionTemplateConstants.sessionTypeNotificationMessage = "SessionTypeNotificationMessage";
    SessionTemplateConstants.formLevel = "INFO";
    SessionTemplateConstants.formNotificationId = "769421F3-E844-4A09-8496-FF1778628119";
    ClientRuntime.SessionTemplateConstants = SessionTemplateConstants;
    class FCSConstants {
    }
    FCSConstants.CSAppProfileManagement = "CS.AppProfileManagement";
    FCSConstants.EnableEntityAnchorTab = "EnableEntityAnchorTab";
    ClientRuntime.FCSConstants = FCSConstants;
    let PanelType;
    (function (PanelType) {
        PanelType[PanelType["Docked"] = 162450002] = "Docked";
        PanelType[PanelType["Hidden"] = 162450001] = "Hidden";
    })(PanelType = ClientRuntime.PanelType || (ClientRuntime.PanelType = {}));
    let SessionTypeOptions;
    (function (SessionTypeOptions) {
        SessionTypeOptions[SessionTypeOptions["Generic"] = 0] = "Generic";
        SessionTypeOptions[SessionTypeOptions["Entity"] = 1] = "Entity";
    })(SessionTypeOptions = ClientRuntime.SessionTypeOptions || (ClientRuntime.SessionTypeOptions = {}));
})(ClientRuntime || (ClientRuntime = {}));
var ClientRuntime;
(function (ClientRuntime) {
    "use strict";
    class SessionTemplatesEventHandlerLibrary {
        constructor() {
        ...
        onFormLoad(executionContext) {
            ...
            if (this.isFormTypeCreate(formContext)) {
                formContext.ui.setFormNotification(Xrm.Utility.getResourceString(ClientRuntime.SessionTemplateConstants.WebresourceName, ClientRuntime.SessionTemplateConstants.sessionTypeNotificationMessage), ClientRuntime.SessionTemplateConstants.formLevel, ClientRuntime.SessionTemplateConstants.formNotificationId);
            }
            if (this.isInitialLoad) {
                this.onSessionTypeChange(executionContext, this.isInitialLoad);
                this.isInitialLoad = false;
            }
            // Register event handler for entity type change
            if (this.isEntityAnchorTabFCSEnabled()) {
                let entityAttribute = formContext.getAttribute(ClientRuntime.SessionTemplateEntity.msdyn_entity);
                if (entityAttribute) {
                    entityAttribute.addOnChange(this.onEntityTypeChange.bind(this));
                }
                this.setDefaultViewForAnchorTabLookup(executionContext, this.getSessionType(formContext));
            }
        }
        onSessionTypeChange(executionContext, isOnLoad = false) {
            let showEntityField = false;
            let showAnchorTabField = false;
            let showSessionIconField = false;
            let isAnchorTabFieldRequired = true;
            let formContext = executionContext.getFormContext();
            let panelAttribute = formContext.getAttribute("msdyn_panelstate");
            //restrict change of session type while update of form
            this.restrictSessionTypeChanges(formContext, isOnLoad);
            if (formContext.ui.getFormType() == 2 && !isOnLoad) {
                return;
            }
            let sessionTypeValue = this.getSessionType(formContext);
            this.setDefaultPanelModeForGenericSessionTemplate(sessionTypeValue, formContext, panelAttribute);
            if (sessionTypeValue == (ClientRuntime.SessionTypeOptions.Generic).toString()) {
                showAnchorTabField = true;
                showSessionIconField = true;
            }
            if (sessionTypeValue == (ClientRuntime.SessionTypeOptions.Entity).toString()) {
                showEntityField = true;
                showAnchorTabField = this.shouldShowEntityAnchorTabField(executionContext);
                isAnchorTabFieldRequired = false;
                this.setPanelState(panelAttribute, ClientRuntime.PanelType.Hidden);
            }
            this.setDefaultViewForAnchorTabLookup(executionContext, sessionTypeValue);
            this.setComponentVisibility(executionContext, ClientRuntime.SessionTemplateEntity.msdyn_entity, showEntityField, showEntityField, !showEntityField);
            this.setComponentVisibility(executionContext, ClientRuntime.SessionTemplateEntity.msdyn_enabletitlesync, showEntityField, !showEntityField, !showEntityField);
            this.setComponentVisibility(executionContext, ClientRuntime.SessionTemplateEntity.msdyn_anchortab, showAnchorTabField, isAnchorTabFieldRequired, !showAnchorTabField);
            this.setComponentVisibility(executionContext, ClientRuntime.SessionTemplateEntity.msdyn_sessionicon, showSessionIconField, false, !showSessionIconField);
        }
        shouldShowEntityAnchorTabField(executionContext) {
            var _a;
            let formContext = executionContext.getFormContext();
            if (!this.isEntityAnchorTabFCSEnabled()) {
                return false;
            }
            const entityField = (_a = formContext === null || formContext === void 0 ? void 0 : formContext.getAttribute(ClientRuntime.SessionTemplateEntity.msdyn_entity)) === null || _a === void 0 ? void 0 : _a.getValue();
            return ClientRuntime.SessionTemplateEntity.anchorTabSupportedEntity.has(entityField);
        }
        /**
         * Sets the default view for the anchor tab lookup control based on the session type value.
         *
         * @param executionContext - The execution context provided by the CRM form.
         * @param sessionTypeValue - The value of the session type to determine which default view to set.
         *
         * The method sets the default view for the lookup control "msdyn_anchortab" based on the session type:
         * - If the session type is "Generic", it sets the default view to `genericAnchorTabsViewId`.
         * - If the session type is "Entity" and the entity anchor tab FCS is enabled, it sets the default view to `entityAnchorTabsViewId`.
         */
        setDefaultViewForAnchorTabLookup(executionContext, sessionTypeValue) {
            const isEntityAnchorTabFCSEnabled = this.isEntityAnchorTabFCSEnabled();
            if (!isEntityAnchorTabFCSEnabled) {
                return;
            }
            let formContext = executionContext.getFormContext();
            /** eslint-disable-next-line */
            const lookupControl = formContext.getControl("msdyn_anchortab");
            if (sessionTypeValue == ClientRuntime.SessionTypeOptions.Generic.toString()) {
                lookupControl.setDefaultView(ClientRuntime.SessionTemplateEntity.genericAnchorTabsViewId);
            }
            else if (sessionTypeValue == ClientRuntime.SessionTypeOptions.Entity.toString() && isEntityAnchorTabFCSEnabled) {
                lookupControl.setDefaultView(ClientRuntime.SessionTemplateEntity.entityAnchorTabsViewId);
            }
        }
        onEntityChange(executionContext) {
            let formContext = executionContext.getFormContext();
            /* If the Entity is selected as "--Select--" which is the Option set's appended display
             * text with "-1" as value; which is not a valid entity info, resetting to null to preserve
             * the initial load behavior and to invalidate selection.*/
            let entityAttribute = formContext.getAttribute(ClientRuntime.SessionTemplateEntity.msdyn_entity);
            if (entityAttribute != null && entityAttribute.getValue() == "-1") {
                entityAttribute.setValue(null);
            }
        }
        restrictSessionTypeChanges(formContext, isOnLoad) {
            if (formContext.ui.getFormType() == 2) {
                formContext.getControl('msdyn_sessiontype').setDisabled(true);
                if (!isOnLoad) {
                    var alertStrings = { confirmButtonLabel: Xrm.Utility.getResourceString(ClientRuntime.SessionTemplateConstants.WebresourceName, ClientRuntime.SessionTemplateConstants.alertPopupButton), text: Xrm.Utility.getResourceString(ClientRuntime.SessionTemplateConstants.WebresourceName, ClientRuntime.SessionTemplateConstants.alertPopupMessage), title: Xrm.Utility.getResourceString(ClientRuntime.SessionTemplateConstants.WebresourceName, ClientRuntime.SessionTemplateConstants.alertPopupTitle) };
                    var alertOptions = { height: 120, width: 260 };
                    Xrm.Navigation.openAlertDialog(alertStrings, alertOptions).then(function success(result) {
                        let sessionType = formContext.getAttribute('msdyn_sessiontype').getValue() ^ 1;
                        formContext.getAttribute('msdyn_sessiontype').setValue(sessionType);
                    }, function (error) {
                        ClientRuntime.Logger.error(`Error opening alert dialog: ` + error.message);
                    });
                }
            }
        }
        /**
         * Returns true if form type is create
         * @param formContext
         * @returns
         */
        isFormTypeCreate(formContext) {
         ...
         * @returns
         */
        getSessionType(formContext) {
            let sessionType = formContext.getAttribute(ClientRuntime.SessionTemplateEntity.msdyn_sessiontype);
            if (sessionType != null) {
        ...
        /**
         * Set default panel mode to "Docked" for Generic session template type
         * @param sessionTemplateType
         * @param formContext
         * @param panelAttribute
         */
        setDefaultPanelModeForGenericSessionTemplate(sessionTemplateType, formContext, panelAttribute) {
            if (sessionTemplateType == (ClientRuntime.SessionTypeOptions.Generic).toString() && this.isFormTypeCreate(formContext)) {
                this.setPanelState(panelAttribute, ClientRuntime.PanelType.Docked);
            }
        }
        /**
         * set panel state
         ...
         * @param panelOption
         */
        setPanelState(panelAttribute, panelOption) {
            panelAttribute.setValue(panelOption);
        }
        /**
         ...
         * @param requiredState field required state
         * @param resetValue value
         */
        setComponentVisibility(executionContext, entityFieldName, visibilityState, requiredState, resetValue) {
            let formContext = executionContext.getFormContext();
            let entityFieldAttribute = formContext.getAttribute(entityFieldName);
            let entityFieldControl = formContext.getControl(entityFieldName);
            if (!this.isNullOrUndefined(entityFieldControl)) {
                entityFieldControl.setVisible(visibilityState);
            }
            else {
                ClientRuntime.Logger.info(`Control not found ${entityFieldName}`);
            }
            if (!this.isNullOrUndefined(entityFieldAttribute)) {
                this.setAttributeLevel(entityFieldAttribute, requiredState);
            }
            if (!this.isNullOrUndefined(entityFieldAttribute) && resetValue) {
                if (entityFieldAttribute.getAttributeType() === "boolean") {
                    entityFieldAttribute.setValue(false);
                }
                else {
                    entityFieldAttribute.setValue(null);
                }
            }
        }
        /**
         ...
         */
        setAttributeLevel(attribute, visibilityState) {
            let requiredLevel = visibilityState ? "required" /* XrmClientApi.Constants.AttributeRequiredLevel.required */ : "none" /* XrmClientApi.Constants.AttributeRequiredLevel.none */;
        ...
        isManagedEntityRecord(primaryControl) {
            let isManaged = primaryControl.getAttribute("ismanaged");
            if (!this.isNullOrUndefined(isManaged))
                return !(isManaged.getValue());
            else
                return true;
        }
        /**
         * Event handler for entity type change
         * @param executionContext execution context
         */
        onEntityTypeChange(executionContext) {
            var _a;
            let formContext = executionContext.getFormContext();
            let entityField = (_a = formContext.getAttribute(ClientRuntime.SessionTemplateEntity.msdyn_entity)) === null || _a === void 0 ? void 0 : _a.getValue();
            let showAnchorTabField = ClientRuntime.SessionTemplateEntity.anchorTabSupportedEntity.has(entityField);
            this.setComponentVisibility(executionContext, ClientRuntime.SessionTemplateEntity.msdyn_anchortab, showAnchorTabField, !showAnchorTabField, !showAnchorTabField);
        }
        isEntityAnchorTabFCSEnabled() {
            return Xrm && Xrm.Utility.getGlobalContext().getFeatureControlSetting(ClientRuntime.FCSConstants.CSAppProfileManagement, ClientRuntime.FCSConstants.EnableEntityAnchorTab);
        }
    }
    // Properties
    SessionTemplatesEventHandlerLibrary.Instance = new SessionTemplatesEventHandlerLibrary();
    ClientRuntime.SessionTemplatesEventHandlerLibrary = SessionTemplatesEventHandlerLibrary;
})(ClientRuntime || (ClientRuntime = {}));
var ClientRuntime;
(function (ClientRuntime) {
...
//# sourceMappingURL=ClientSessionTemplate.js.map 
