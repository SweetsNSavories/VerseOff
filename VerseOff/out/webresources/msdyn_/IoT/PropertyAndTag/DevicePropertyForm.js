...
var msdyn = msdyn || {};
msdyn.DevicePropertyForm = msdyn.DevicePropertyForm || {};
msdyn.DevicePropertyForm.formContext = msdyn.DevicePropertyForm.formContext || {};
// Updates the property field label to tag when isTag is set to true.
msdyn.DevicePropertyForm.onToggle = function (executionContext) {
    msdyn.DevicePropertyForm.formContext = executionContext.getFormContext();
    var isTagAttr = msdyn.DevicePropertyForm.formContext.getAttribute(IoTConnector.Common.Constants.DeviceProperty.isTagFieldName);
    var propertyControl = msdyn.DevicePropertyForm.formContext.getControl(IoTConnector.Common.Constants.DeviceProperty.propertyFieldName);
    if (isTagAttr && propertyControl) {
        if (isTagAttr.getValue()) {
            propertyControl.setLabel(IoTConnector.Localization.Localization.localize("@:tag"));
        }
        else {
            propertyControl.setLabel(IoTConnector.Localization.Localization.localize("@:property"));
        }
    }
};
// Subgrid new button. We use primaryControl as the form context here because this method can be called without onToggle
msdyn.DevicePropertyForm.addNew = function (primaryItemIds, selectedControl, primaryControl) {
    var parameters = {};
    if (primaryItemIds && primaryItemIds.length > 0 && primaryControl) {
        var formData = primaryControl.data;
        // If the current form is category.
        if (formData && formData.entity && formData.entity.getEntityName() === IoTConnector.Common.Constants.DeviceProperty.IoTDeviceCategoryEntityName) {
            var categoryNameAttr = primaryControl.getAttribute("msdyn_name");
            if (categoryNameAttr) {
                parameters[IoTConnector.Common.Constants.DeviceProperty.AttributeCategoryName] = categoryNameAttr.getValue();
                parameters[IoTConnector.Common.Constants.DeviceProperty.AttributeCategory] = primaryItemIds[0].replace(/[{}]/g, "");
            }
        }
    }
    if (selectedControl && typeof (selectedControl.getName) === "function") {
        var subGridName = selectedControl.getName();
        if (subGridName === "DeviceTagsGrid") {
            parameters[IoTConnector.Common.Constants.DeviceProperty.isTagFieldName] = true;
        }
    }
    var entityFormOptions = {};
    entityFormOptions["entityName"] = IoTConnector.Common.Constants.DeviceProperty.IoTDevicePropertyEntityName;
