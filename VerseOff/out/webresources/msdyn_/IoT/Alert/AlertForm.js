...
/// <reference path="../../../../Scripts/typings/xrm/Crm.ClientApiTypings.1.0.1069-manual/clientapi/XrmClientApi.d.ts"/>
/// <reference path="../Common/Constants.ts" />
/// <reference path="../Utils/FormUtils.ts" />
/// <reference path="../Utils/SuggestionsUtils.ts" />
/// <reference path="../Utils/ApiUtils/Common.ts" />
/// <reference path="../Utils/ApiUtils/WebApiSDK.ts" />
var msdyn = msdyn || {};
msdyn.IoTAlertForm = msdyn.IoTAlertForm || {};
msdyn.IoTAlertForm.formContext = msdyn.IoTAlertForm.formContext || {};
msdyn.IoTAlertForm.iotDeviceEntityName = "msdyn_iotdevices";
msdyn.IoTAlertForm.deviceFieldName = "msdyn_device";
msdyn.IoTAlertForm.onLoad = function (executionContext) {
    msdyn.IoTAlertForm.formContext = executionContext.getFormContext();
    msdyn.IoTAlertForm.deviceField = msdyn.IoTAlertForm.formContext.getAttribute(msdyn.IoTAlertForm.deviceFieldName);
    if (msdyn.IoTAlertForm.formContext.ui.getFormType() !== 1 /* Create */) {
        if (!IoTConnector.ApiUtils.IsMobile()) {
            var generalTab = msdyn.IoTAlertForm.formContext.ui.tabs.get(0);
            ...
            if (powerBISection) {
                powerBISection.setVisible(true);
            }
        }
        // data relating to the alert should not be changed manually
        msdyn.IoTAlertForm.disableAlertRelatedFields();
    }
    else {
        // enable editing of alert data control if in create mode
        IoTConnector.Utils.FormUtils.lockField(msdyn.IoTAlertForm.formContext, IoTConnector.Common.Constants.Msdyn_iotalert.CustomControlAlertData, false);
    }
    msdyn.IoTAlertForm.displayHideSuggestionsSection(executionContext);
};
msdyn.IoTAlertForm.disableAlertRelatedFields = function () {
    var alertConsts = IoTConnector.Common.Constants.Msdyn_iotalert;
    var controlsToLock = [
        alertConsts.AttributeAlertToken,
        alertConsts.AttributeAlertTime,
        alertConsts.AttributeAlertURL,
        alertConsts.AttributeAlertData
    ];
    for (var _i = 0, controlsToLock_1 = controlsToLock; _i < controlsToLock_1.length; _i++) {
        var field = controlsToLock_1[_i];
        IoTConnector.Utils.FormUtils.lockField(msdyn.IoTAlertForm.formContext, field, true);
    }
    // Show Device Insights (Preview) tab when Device field is not empty
    msdyn.IoTAlertForm.ShoworHideDeviceInsights();
    ...
    msdyn.IoTAlertForm.ShoworHideDeviceSummaryVisualizationSection();
};
// Show or Hide Device Insights (Preview) tab based on value in Device field and action name, tsiurl value.
msdyn.IoTAlertForm.ShoworHideDeviceInsights = function () {
    var deviceInsightsTab = msdyn.IoTAlertForm.formContext.ui.tabs.get(IoTConnector.Common.Constants.TabNames.DeviceInsightsTab);
    // If device field has value, then show device insights tab.
    var deviceGuid = IoTConnector.Utils.FormUtils.getGuidFromField(msdyn.IoTAlertForm.deviceField);
    // Show or Hide Device Insights (Preview) tab based on device having iot provider with OOB and TSI deployed 
    // or custom action.
    IoTConnector.Utils.FormUtils.preValidateShowHideDeviceInsightsTab(deviceInsightsTab, deviceGuid, msdyn.IoTAlertForm.deviceField, IoTConnector.Common.DeviceKeyType.DeviceGuid);
};
msdyn.IoTAlertForm.displayHideSuggestionsSection = function (executionContext) {
    var generalTab = msdyn.IoTAlertForm.formContext.ui.tabs.get(0);
    var iotSuggestionsSection = generalTab ? generalTab.sections.get(IoTConnector.Common.Constants.Msdyn_iotalert.SuggestionsSection) : null;
    IoTConnector.Utils.SuggestionsUtils.isSuggestionsEnabled().then(function (isEnabled) {
        iotSuggestionsSection.setVisible(isEnabled);
        if (isEnabled) {
            msdyn.IoTAlertForm.displayHideAlertPriorityScoreField(executionContext);
        }
    });
};
// Show Priority Score Field if Priority value is being set.
msdyn.IoTAlertForm.displayHideAlertPriorityScoreField = function (executionContext) {
    var formContext = executionContext.getFormContext();
    var alertPriorityScoreFieldControl = formContext.getControl(IoTConnector.Common.Constants.Msdyn_iotalert.AttributeAlertPriorityScore);
    // Display Pri Score if Pri is High or Low
    IoTConnector.Utils.FormUtils.displayHideAlertFieldBasedOnSuggestedPriority(formContext, alertPriorityScoreFieldControl, [IoTConnector.Common.Constants.Msdyn_iotalert.HighPriorityValue, IoTConnector.Common.Constants.Msdyn_iotalert.LowPriorityValue]);
};
// Logic to show Device Summary Visualization section if Device ID associated with Device for this Alert is not empty. If it is empty or user in offline mode, this section will be hidden by default.
msdyn.IoTAlertForm.ShoworHideDeviceSummaryVisualizationSection = function () {
    var generalTab = msdyn.IoTAlertForm.formContext.ui.tabs.get(IoTConnector.Common.Constants.TabNames.IoTAlertFormGeneralTab);
    // Check if device field exists, section exists, get value of device field. If yes, then show this section.
    // TODO - Need to add logic to get deviceID of the device
    IoTConnector.Utils.FormUtils.showHideSummaryVisualizationBasedOnField(generalTab, msdyn.IoTAlertForm.deviceField);
};
