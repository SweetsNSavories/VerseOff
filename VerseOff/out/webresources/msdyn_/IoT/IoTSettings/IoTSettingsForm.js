...
var msdyn = msdyn || {};
msdyn.IoTSettingsForm = msdyn.IoTSettingsForm || {};
msdyn.IoTSettingsForm.formContext = msdyn.IoTSettingsForm.formContext || {};
msdyn.IoTSettingsForm.onLoad = function (executionContext) {
    msdyn.IoTSettingsForm.formContext = executionContext.getFormContext();
    var newlcid = IoTConnector.Localization.Localization.getLCID();
    var deviceDataPullsTab = msdyn.IoTSettingsForm.formContext.ui.tabs.get(IoTConnector.Common.Constants.IoTSettingsForm.DeviceDataPullsTab);
    msdyn.IoTSettingsForm.updateDeviceDataPullsTabVisibility(deviceDataPullsTab);
    var deploymentUrl = msdyn.IoTSettingsForm.formContext.getAttribute(IoTConnector.Common.Constants.Msdyn_iotsettings.AttributeDeploymentAppURL);
    if (deploymentUrl) {
        var orgUniqueId = IoTConnector.Utils.ClientUtils.getOrgUniqueId();
        var deploymentUrlValue = IoTConnector.Common.Constants.IoTSettingsForm.DeploymentFieldValue;
        if (orgUniqueId) {
            deploymentUrlValue += "?organizationid=" + orgUniqueId.replace(/[{}]/g, "") + "&&lcid=" + newlcid;
        }
        deploymentUrl.setValue(deploymentUrlValue);
    }
    msdyn.IoTSettingsForm.displayOrHideModelStatusSection(false);
};
// Display the model status section when iot suggestion enabled button is on, hide otherwise
msdyn.IoTSettingsForm.displayOrHideModelStatusSection = function (isIoTSuggestionsOff) {
    var iotsuggestionTab = msdyn.IoTSettingsForm.formContext.ui.tabs.get(IoTConnector.Common.Constants.Msdyn_iotsettings.SuggestionTab);
    var modelStatusSection = iotsuggestionTab ? iotsuggestionTab.sections.get(IoTConnector.Common.Constants.Msdyn_iotsettings.ModelStatusSection) : null;
    if (modelStatusSection == null) {
        return;
    }
    if (isIoTSuggestionsOff) {
        modelStatusSection.setVisible(false);
    }
    else {
        IoTConnector.Utils.SuggestionsUtils.isSuggestionsEnabled().then(function (isEnabled) {
            modelStatusSection.setVisible(isEnabled);
        });
    }
};
msdyn.IoTSettingsForm.updateDeviceDataPullsTabVisibility = function (deviceDataPullsTab) {
    IoTConnector.Utils.ServiceEndpointUtils.isValidEndpoint().then(function (isValid) {
        deviceDataPullsTab && deviceDataPullsTab.setVisible(isValid);
    });
};
// Called when IoT Suggestions toggle is changed
msdyn.IoTSettingsForm.onChangeEnableIoTSuggestions = function (eventContext) {
    var formContext = msdyn.IoTSettingsForm.formContext;
    var enableiotSuggestionsField = formContext.getAttribute(IoTConnector.Common.Constants.IoTSettingsForm.AttributeEnableIoTSuggestions);
    if (enableiotSuggestionsField) {
        // Open the dialog when it is enabled
        if (enableiotSuggestionsField.getValue()) {
            msdyn.IoTSettingsForm.showIoTSuggestionsDialog(formContext, enableiotSuggestionsField);
        }
        else {
            msdyn.IoTSettingsForm.showIoTSuggestionsDisableConfirmation(formContext, enableiotSuggestionsField);
        }
    }
};
// Called when form is saving
msdyn.IoTSettingsForm.onSave = function (eventContext) {
    // Prevent auto save feature because IoT Suggestions toggle should not be auto saved
    var eventArgs = eventContext.getEventArgs();
    IoTConnector.Utils.FormUtils.preventAutoSave(eventArgs);
};
// Show IoT Suggestions dialog
msdyn.IoTSettingsForm.showIoTSuggestionsDialog = function (formContext, enableiotSuggestionsField) {
    var parameters = {};
    ...
    Xrm.Navigation.openDialog(IoTConnector.Common.Constants.IoTSuggestionsDialog.DialogName, dialogOptions, parameters).then(function (param) {
        IoTConnector.Utils.SuggestionsUtils.isSuggestionsEnabled().then(function (isEnabled) {
            enableiotSuggestionsField.setValue(isEnabled);
            formContext.data.save();
        });
    });
};
// Show confirmation information before IoT Suggestions feature is disabled
msdyn.IoTSettingsForm.showIoTSuggestionsDisableConfirmation = function (formContext, enableiotSuggestionsField) {
    var localization = IoTConnector.Localization.Localization;
    var constants = IoTConnector.Common.Constants;
    var confirmStrings = {
        cancelButtonLabel: localization.localize(constants.IoTSuggestionsDialog.NoConfirmationDialogLabelKey),
        confirmButtonLabel: localization.localize(constants.IoTSuggestionsDialog.YesConfirmationDialogLabelKey),
        text: localization.localize(constants.IoTSuggestionsDialog.DisableIoTSuggestionsTextLabelKey),
        title: localization.localize(constants.IoTSuggestionsDialog.DisableIoTSuggestionsTitleLabelKey),
    };
    Xrm.Navigation.openConfirmDialog(confirmStrings).then(function (success) {
        if (success.confirmed) {
            enableiotSuggestionsField.setValue(false);
            formContext.data.save();
            msdyn.IoTSettingsForm.displayOrHideModelStatusSection(true);
        }
        else {
            enableiotSuggestionsField.setValue(true);
        }
    }, function () {
        Xrm.Navigation.openAlertDialog({ text: localization.localize(constants.IoTSuggestionsDialog.ErrorLabelKey) });
    });
};
