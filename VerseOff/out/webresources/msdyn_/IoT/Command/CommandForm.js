...
var msdyn = msdyn || {};
msdyn.IoTCommandForm = msdyn.IoTCommandForm || {};
msdyn.IoTCommandForm.commandFieldName = "msdyn_command";
msdyn.IoTCommandForm.messageFieldName = "msdyn_message";
msdyn.IoTCommandForm.deviceFieldName = "msdyn_device";
msdyn.IoTCommandForm.commandEntityName = "msdyn_iotdevicecommanddefinition";
msdyn.IoTCommandForm.formContext = msdyn.IoTCommandForm.formContext || {};
msdyn.IoTCommandForm.onLoad = function (executionContext) {
    msdyn.IoTCommandForm.formContext = executionContext.getFormContext();
    // Set required fields
    msdyn.IoTCommandForm.formContext.getAttribute(msdyn.IoTCommandForm.messageFieldName).setRequiredLevel(Xrm.Constants.AttributeRequiredLevels.required);
    // Add a save handler for create form.
    if (msdyn.IoTCommandForm.formContext.ui.getFormType() == 1) {
        msdyn.IoTCommandForm.formContext.data.entity.addOnSave(confirmBeforeSave);
        // Set command lookup filter if the form is in create mode.
        var commandLookupControl = msdyn.IoTCommandForm.formContext.getControl(msdyn.IoTCommandForm.commandFieldName);
        if (commandLookupControl != null) {
            msdyn.IoTCommandForm.defaultCommandLookupViewId = commandLookupControl.getDefaultView();
        }
        msdyn.IoTCommandForm.setCommandLookup(executionContext);
    }
    else {
        makeFormReadOnly();
    }
    function confirmBeforeSave(context) {
        var eventArgs = context.getEventArgs();
        if (!eventArgs.isDefaultPrevented()) {
            // If the form is saved by clicking the save icon at the bottom or keyboard shortcuts (not through save & close).
            if (eventArgs.getSaveMode() == 1) {
                // Prevent the save.
                eventArgs.preventDefault();
                // Confirm and save.
                Xrm.Utility.confirmDialog(IoTConnector.Localization.Localization.localize("@:confirmCommandSave"), function () {
                    //Remove self.
                    msdyn.IoTCommandForm.formContext.data.entity.removeOnSave(confirmBeforeSave);
                    msdyn.IoTCommandForm.formContext.data.entity.save();
                }, function () { });
            }
        }
    }
    ...
    function makeFormReadOnly() {
        var controls = msdyn.IoTCommandForm.formContext.ui.controls.get();
...
// executionContext: includes the execution context information from the form
msdyn.IoTCommandForm.setCommandLookup = function (executionContext) {
    var formContext = executionContext ? executionContext.getFormContext() : null;
    if (!formContext) {
        return;
    }
    ...
    var deviceField = formContext.getAttribute(commandViewAttributes.CommandAttributeMsdynDevice);
    var device = deviceField ? deviceField.getValue() : null;
    if (device) {
        ...
        var commandLookupField = formContext.getAttribute(commandViewAttributes.CommandAttributeMsdynCommand);
        var commandUtils = new IoTConnector.Utils.CommandUtils(commandLookupControl, commandLookupField, msdyn.IoTCommandForm.defaultCommandLookupViewId, commandModel);
