...
/// <reference path="../Common/Constants.ts" />
/// <reference path="ODataUtils.ts" />
/// <reference path="CommandDialogUtils.ts" />
var IoTConnector;
(function (IoTConnector) {
    var Utils;
    (function (Utils) {
        // This class includes the common methods from both CommandDialog and CommandForm
        // Also, centralizes the required files
        var CommandUtils = /** @class */ (function () {
            function CommandUtils(commandLookupControl, commandLookupField, defaultCommandLookupViewId, commandModel) {
                this.commandLookupControl = commandLookupControl;
                this.commandLookupField = commandLookupField;
                this.defaultCommandLookupViewId = defaultCommandLookupViewId;
                this.commandModel = commandModel;
            }
            // Set command lookup if there is device selected
            CommandUtils.prototype.setCommandLookup = function () {
                if (!this.commandModel) {
                    return;
                }
                if (this.commandModel.msdyn_device && !IoTConnector.ApiUtils.CrudApiSDK.isAvailableOffline(IoTConnector.Common.Constants.EntityLogicalNames.CommandEntityLogicalName)) {
                    var Localization_1 = IoTConnector.Localization.Localization;
                    var Constants = IoTConnector.Common.Constants;
                    // Create new lookup view.
                    var commandCustomViewId = "{47B90405-334B-438f-8EFB-FBF8A9617F97}"; // A random guid
                    var localizedViewDisplayName = Localization_1.localize("@:commandViewDisplayName");
                    var viewDisplayName = Localization_1.format(localizedViewDisplayName, this.commandModel.msdyn_devicename);
                    var fetchXml = CommandUtils.getCommandsFromDeviceFetchXml(this.commandModel.msdyn_device);
                    var layoutXml = Utils.CommandDialogUtils.getLayoutXml(Constants.EntityLogicalNames.CommandDefinitionEntityLogicalName);
                    this.commandLookupControl.addCustomView(commandCustomViewId, Constants.EntityLogicalNames.CommandDefinitionEntityLogicalName, viewDisplayName, fetchXml, layoutXml, true);
                }
                else {
                    // Reset lookup if there is no device selected or we are in offline app.
                    this.commandLookupControl.setDefaultView(this.defaultCommandLookupViewId);
                }
            };
            // Default the command lookup field if the device belongs to a category which has command definition associated with
            CommandUtils.prototype.setCommandLookupField = function () {
                if (!this.commandModel) {
                    return;
                }
                var Constants = IoTConnector.Common.Constants;
                var ODataUtils = IoTConnector.Utils.ODataUtils;
                var self = this;
                if (this.commandModel.msdyn_device) {
                    ODataUtils.executeRequest("GET", ODataUtils.formatFetchXml(Constants.PluralEntityLogicalNames.CommandDefinitionEntityLogicalName, CommandUtils.getCommandsFromDeviceFetchXml(this.commandModel.msdyn_device)), "", function (data) {
                        if (data) {
                            var commands = data.value;
                            if (!Utils.CommandDialogUtils.isNullOrEmpty(commands)) {
                                self.commandLookupField.setValue([{
                                        id: commands[0][Constants.EntityLogicalNames.CommandDefinitionEntityLogicalName + "id"],
                                        name: commands[0]["msdyn_name"],
                                        entityType: Constants.EntityLogicalNames.CommandDefinitionEntityLogicalName
                                    }]);
                            }
                        }
                    }, function (err) {
            ...
            ;
            // Shows notification if the device has a device category with commands, clears notification if not
            // formContext: context needed to set and clear the notification
            CommandUtils.prototype.showNotificationNoCommandsAssociatedWithDevice = function (formContext) {
                if (!(this.commandModel && formContext && this.commandModel.msdyn_device)) {
                    return;
                }
                var Constants = IoTConnector.Common.Constants;
                var ODataUtils = IoTConnector.Utils.ODataUtils;
                ODataUtils.executeRequest("GET", ODataUtils.formatFetchXml(Constants.PluralEntityLogicalNames.CommandDefinitionEntityLogicalName, CommandUtils.getCommandsFromDeviceFetchXml(this.commandModel.msdyn_device)), "", function (data) {
                    if (data) {
                        var commands = data.value;
                        var message = "commandLookupDeviceCategoryNotSet";
                        if (!Utils.CommandDialogUtils.isNullOrEmpty(commands)) {
                            formContext.ui.clearFormNotification(message);
                        }
                        else {
                            formContext.ui.setFormNotification(IoTConnector.Localization.Localization.localize("@:" + message), "INFO", message);
                        }
                    }
                }, function (err) {
                    console.log(err.error.message);
            ...
            // Set device lookup
            CommandUtils.prototype.setDeviceLookup = function () {
                var Constants = IoTConnector.Common.Constants;
                var deviceCustomViewId = "{55C9DE17-7155-4A14-BCB2-34997D180BC4}"; // A random guid
                var viewDisplayName = IoTConnector.Localization.Localization.localize("@:deviceViewDisplayName");
                var fetchXml = CommandUtils.getDevicesFromParentEntityFetchXml(this.parentEntityId);
                var layoutXml = Utils.CommandDialogUtils.getLayoutXml(Constants.EntityLogicalNames.DeviceEntityLogicalName);
                this.deviceLookupControl.addCustomView(deviceCustomViewId, Constants.EntityLogicalNames.DeviceEntityLogicalName, viewDisplayName, fetchXml, layoutXml, true);
            };
            // Get commands from the device fetchXml
            // deviceId: the device id which is used to get related command definition
            CommandUtils.getCommandsFromDeviceFetchXml = function (deviceId) {
                if (!deviceId) {
                    return;
                }
                var fetchXml = "<fetch> " +
                    "<entity name='msdyn_iotdevicecommanddefinition'> " + // Command definition.
                    "<attribute name='msdyn_iotdevicecommanddefinitionid'/> " +
                    "<attribute name='msdyn_name'/> " +
                    "<attribute name='createdon'/> " +
                    "<link-entity name='msdyn_iotdevicecategorycommands' " + // Command - Category.
                    "from='msdyn_iotdevicecommanddefinitionid' " +
                    "to='msdyn_iotdevicecommanddefinitionid' " +
                    "link-type='inner' " +
                    "alias='msdyn_iotdevicecategorycommands'> " +
                    "<attribute name='msdyn_iotdevicecategoryid'/> " +
                    "<link-entity name='msdyn_iotdevice' " + // Device
                    "from='msdyn_category' " +
                    "to='msdyn_iotdevicecategoryid' " +
                    "link-type='inner' " +
                    ...
                    "</link-entity> " +
                    "<filter type='and'> " +
                    "<condition entityname='msdyn_iotdevice' " + // Filter on device.
                    "attribute='msdyn_iotdeviceid' " +
                    "operator='eq' " +
                    "value='" + deviceId + "'/> " +
                    "</filter> " +
                    "</entity> " +
            ...
            ;
            // Get devices from the parent entity fetchXml
            // parentEntityId: the parent entity id which is used to get related devices
            CommandUtils.getDevicesFromParentEntityFetchXml = function (parentEntityId) {
                var fetchXml = "<fetch> " +
                    ...
                    "</link-entity> " +
                    "<filter type='and'> " +
                    "<condition entityname='connection' " + // Filter on parent entity.
                    "attribute='record1id' " +
                    "operator='eq' " +
                    ...
                    "operator='eq' " +
                    "value='" + IoTConnector.Common.Constants.Connection.IoTRoleId + "'/> " +
                    "</filter> " +
                    "</entity> " +
                    "</fetch>";
                return fetchXml;
...
/// <reference path="../../Utils/CommandUtils.ts" />
/// <reference path="../Models/Command.ts" />
/// <reference path="../../Models/EntityForLookup.ts" />
var msdyn = msdyn || {};
msdyn.IoTCommandDialog = msdyn.IoTCommandDialog || {};
// Command section metadata in dialog
msdyn.IoTCommandDialog.commandTitle = "lbl_command_title";
msdyn.IoTCommandDialog.defaultCommandLookupViewId = "{bb72e8d0-b1f7-4388-9c92-bd1e76d09e05}";
msdyn.IoTCommandDialog.sendCommandToLabelKey = "@:sendCommandTo";
msdyn.IoTCommandDialog.sendCommandLabelKey = "@:sendCommand";
msdyn.IoTCommandDialog.commandNameWithCommandFilledLabelKey = "@:iotCommandNameWithCommandFilled";
msdyn.IoTCommandDialog.commandNameOnlyMessageFilledLabelKey = "@:iotCommandNameOnlyMessageFilled";
msdyn.IoTCommandDialog.deviceViewDisplayNameLabelKey = "@:deviceViewDisplayName";
msdyn.IoTCommandDialog.commandAndMessageBothEmptyErrorLableKey = "@:commandLookupAndMessageCannotBeBothEmpty";
msdyn.IoTCommandDialog.commandSentOffline = "@:commandSentOffline";
msdyn.IoTCommandDialog.commandNotSentOffline = "@:commandNotSentOffline";
msdyn.IoTCommandDialog.onSaveMFD = function (eventContext) {
    var formContext = eventContext ? eventContext.getFormContext() : null;
    var nameAttribute = formContext.getAttribute(IoTConnector.Common.Constants.CommandViewAttributes.CommandAttributeMsdynName);
    var commandAttribute = formContext.getAttribute(IoTConnector.Common.Constants.CommandViewAttributes.CommandAttributeMsdynCommand);
    var messageAttribute = formContext.getAttribute(IoTConnector.Common.Constants.CommandViewAttributes.CommandAttributeMsdynMessage);
    if (commandAttribute && messageAttribute && !commandAttribute.getValue() && !messageAttribute.getValue()) {
        formContext.ui.setFormNotification(IoTConnector.Localization.Localization.localize(msdyn.IoTCommandDialog.commandAndMessageBothEmptyErrorLableKey), "ERROR", msdyn.IoTCommandDialog.commandAndMessageBothEmptyErrorLableKey.substring(2));
        eventContext.getEventArgs().preventDefault();
        return;
    }
    var CommandDialogUtils = IoTConnector.Utils.CommandDialogUtils;
    var Constants = IoTConnector.Common.Constants;
    var formAttributes = CommandDialogUtils.getFormAttributes(formContext);
    var isMfdParamValue = CommandDialogUtils.getFieldValue(Constants.CommandMFDParameters.CommandAttributeIsMFD, formAttributes);
    if (isMfdParamValue) {
        var deviceAttribute = formContext.getAttribute(IoTConnector.Common.Constants.CommandViewAttributes.CommandAttributeMsdynDevice);
        var device = deviceAttribute != null ? deviceAttribute.getValue() : null;
        var deviceName = device != null && device.length != 0 ? device[0].name : null;
        var command = commandAttribute != null ? commandAttribute.getValue() : null;
        var commandName = command != null && command.length != 0 ? command[0].name : null;
        nameAttribute.setValue(msdyn.IoTCommandDialog.getCommandName(deviceName, commandName));
    }
};
msdyn.IoTCommandDialog.onLoadMFD = function (eventContext) {
    var formContext = eventContext ? eventContext.getFormContext() : null;
    if (!formContext) {
        return;
    }
    ...
    if (isMfdParamValue) {
        ...
        formContext.ui.tabs.get("mfdTab").setVisible(true);
        formContext.getAttribute("msdyn_device").setRequiredLevel(Xrm.Constants.AttributeRequiredLevels.required);
        var nameAttribute = formContext.getAttribute(IoTConnector.Common.Constants.CommandViewAttributes.CommandAttributeMsdynName);
        nameAttribute.setValue("CommandName");
    }
};
msdyn.IoTCommandDialog.onLoad = function (eventContext) {
    var formContext = eventContext ? eventContext.getFormContext() : null;
    if (!formContext) {
        return;
    }
    msdyn.IoTCommandDialog.setDeviceLookup(formContext);
    msdyn.IoTCommandDialog.setDeviceDefaultValue(formContext);
    msdyn.IoTCommandDialog.setCommandLookup(formContext);
    msdyn.IoTCommandDialog.setTitle(formContext);
    msdyn.IoTCommandDialog.addCommandOnChange(formContext);
};
// Set the filter on device's lookup according to the parent entity associated with the selected device.
// formContext: includes the form context information
msdyn.IoTCommandDialog.setDeviceLookup = function (formContext) {
    if (!formContext) {
        return;
    }
    var Constants = IoTConnector.Common.Constants;
    var deviceLookupControl = formContext.getControl(Constants.CommandViewAttributes.CommandAttributeMsdynDevice);
    if (!deviceLookupControl) {
        return;
    }
    var CommandDialogUtils = IoTConnector.Utils.CommandDialogUtils;
    var formAttributes = CommandDialogUtils.getFormAttributes(formContext);
    var parentEntityId = CommandDialogUtils.getFieldValue(Constants.CommandAttributeParameters.CommandAttributeParentEntityId, formAttributes);
    if (!parentEntityId) {
        return;
    }
    // Create new lookup view.
    var commandUtils = new IoTConnector.Utils.CommandUtils();
    commandUtils.parentEntityId = parentEntityId;
    commandUtils.deviceLookupControl = deviceLookupControl;
    commandUtils.setDeviceLookup();
};
// Set Device related default values and visibility
// formContext: includes form context information
msdyn.IoTCommandDialog.setDeviceDefaultValue = function (formContext) {
    if (formContext && formContext.data && formContext.data.attributes) {
        var CommandDialogUtils = IoTConnector.Utils.CommandDialogUtils;
        var Constants = IoTConnector.Common.Constants;
        // Parameters passed to dialog
        var formAttributes = CommandDialogUtils.getFormAttributes(formContext);
        var commandAttributeParameters = Constants.CommandAttributeParameters;
        var commandViewAttributes = Constants.CommandViewAttributes;
        var deviceLookupParamValue = CommandDialogUtils.getFieldValue(commandAttributeParameters.CommandAttributeIoTDeviceId, formAttributes);
        var deviceNameParamValue = CommandDialogUtils.getFieldValue(commandAttributeParameters.CommandAttributeIoTDeviceName, formAttributes);
        // Device fields from dialog
        var deviceLookup = CommandDialogUtils.getField(commandViewAttributes.CommandAttributeMsdynDevice, formAttributes);
        // Only set Device value if parameter has value, else leave it empty
        if (deviceLookupParamValue && deviceNameParamValue) {
            var lookupValue = new Array();
            lookupValue[0] = new Object();
            lookupValue[0].id = deviceLookupParamValue;
            lookupValue[0].name = deviceNameParamValue;
            lookupValue[0].entityType = Constants.EntityLogicalNames.DeviceEntityLogicalName;
            CommandDialogUtils.setFieldValue(deviceLookup, lookupValue);
        }
    }
};
// Initialize the command lookup in the dialog
// formContext: includes form context information
msdyn.IoTCommandDialog.setCommandLookup = function (formContext) {
    if (!formContext) {
        return;
    }
    var CommandDialogUtils = IoTConnector.Utils.CommandDialogUtils;
    var Constants = IoTConnector.Common.Constants;
    // Retrieve command definitions by device information
    var commandAttributeParameters = Constants.CommandAttributeParameters;
    var commandViewAttributes = Constants.CommandViewAttributes;
    var commandLookupControl = formContext.getControl(commandViewAttributes.CommandAttributeMsdynCommand);
    if (!commandLookupControl) {
        return;
    }
    var formAttributes = CommandDialogUtils.getFormAttributes(formContext);
    // Populate the command model with the command parameters passed into the dialog
    var commandModel = new IoTConnector.Command.Models.Command();
    var device = CommandDialogUtils.getFieldValue(commandViewAttributes.CommandAttributeMsdynDevice, formAttributes);
    if (!CommandDialogUtils.isNullOrEmpty(device)) {
        commandModel.msdyn_device = device[0]['id'];
        commandModel.msdyn_devicename = device[0]['name'];
    }
    commandModel.msdyn_parentalert = CommandDialogUtils.getFieldValue(commandAttributeParameters.CommandAttributeAlertId, formAttributes);
    commandModel.msdyn_parentalertname = CommandDialogUtils.getFieldValue(commandAttributeParameters.CommandAttributeAlertName, formAttributes);
    var commandLookupField = CommandDialogUtils.getField(commandViewAttributes.CommandAttributeMsdynCommand, formAttributes);
    var commandMessageField = CommandDialogUtils.getField(commandViewAttributes.CommandAttributeMsdynMessage, formAttributes);
    var commandReferenceField = CommandDialogUtils.getField(commandViewAttributes.CommandAttributeMsdynCommandReference, formAttributes);
    // Clean command and message fields
    commandLookupField.setValue("");
    commandMessageField.setValue("");
    commandReferenceField.setValue("{}");
    // Pass the form information to CommandUtils
    var commandUtils = new IoTConnector.Utils.CommandUtils(commandLookupControl, commandLookupField, msdyn.IoTCommandDialog.defaultCommandLookupViewId, commandModel);
    commandUtils.setCommandLookup();
    // Check to see if there is a device category, show notification if not
    commandUtils.showNotificationNoCommandsAssociatedWithDevice(formContext);
};
// Send command when 'Send' button is clicked
// eventContext: includes form information for the event
msdyn.IoTCommandDialog.sendCommand = function (eventContext) {
    if (!eventContext) {
        return;
    }
    var commandEntity = new IoTConnector.Command.Models.Command();
    msdyn.IoTCommandDialog.setCommandEntity(commandEntity, eventContext);
    msdyn.IoTCommandDialog.send(commandEntity, eventContext);
};
// Store command parameters to the JSON object commandEntity for sending request
// commandEntity: the JSON object that holds the command parameters
// CommandUtils: reference to a class containing common methods for dialog and form
// eventContext: includes form information for the event
msdyn.IoTCommandDialog.setCommandEntity = function (commandEntity, eventContext) {
    var CommandDialogUtils = IoTConnector.Utils.CommandDialogUtils;
    var Constants = IoTConnector.Common.Constants;
    var formAttributes = CommandDialogUtils.getFormAttributes(CommandDialogUtils.getFormContext(eventContext));
    var CommandViewAttributes = Constants.CommandViewAttributes;
    var device = CommandDialogUtils.getFieldValue(CommandViewAttributes.CommandAttributeMsdynDevice, formAttributes);
    if (CommandDialogUtils.isNullOrEmpty(device)) {
        return;
    ...
    var CommandAttributeParameters = Constants.CommandAttributeParameters;
    var PluralEntityLogicalNames = Constants.PluralEntityLogicalNames;
    // Add command definition information as command parameters to the JSON object commandEntity
    var commandDefLookup = CommandDialogUtils.getFieldValue(CommandViewAttributes.CommandAttributeMsdynCommand, formAttributes);
    if (!CommandDialogUtils.isNullOrEmpty(commandDefLookup)) {
        var commandDefName = commandDefLookup[0]['name'];
        var commandDefId = commandDefLookup[0]['id'];
        commandEntity.msdyn_name = msdyn.IoTCommandDialog.getCommandName(device[0]['name'], commandDefName);
        commandEntity[CommandViewAttributes.CommandAttributeMsdynCommandOdataBind] = CommandDialogUtils.getOdataBindValue(PluralEntityLogicalNames.CommandDefinitionEntityLogicalName, commandDefId);
    }
    else {
        commandEntity.msdyn_name = msdyn.IoTCommandDialog.getCommandName(device[0]['name']);
    }
    commandEntity.msdyn_message = CommandDialogUtils.getFieldValue(CommandViewAttributes.CommandAttributeMsdynMessage, formAttributes);
    // Add alert information as command parameters to the JSON object commandEntity
    var alertIdField = CommandDialogUtils.getField(CommandAttributeParameters.CommandAttributeAlertId, formAttributes);
    if (alertIdField && alertIdField.getValue()) {
        commandEntity[CommandViewAttributes.CommandAttributeMsdynParentAlertOdataBind] = CommandDialogUtils.getOdataBindValue(PluralEntityLogicalNames.AlertEntityLogicalName, alertIdField.getValue());
    }
    // Add device information as command parameters to the JSON object commandEntity
    var deviceField = CommandDialogUtils.getField(CommandViewAttributes.CommandAttributeMsdynDevice, formAttributes);
    if (deviceField && !CommandDialogUtils.isNullOrEmpty(deviceField.getValue())) {
        commandEntity[CommandViewAttributes.CommandAttributeMsdynDeviceOdataBind] = CommandDialogUtils.getOdataBindValue(PluralEntityLogicalNames.DeviceEntityLogicalName, deviceField.getValue()[0]['id']);
    }
};
msdyn.IoTCommandDialog.setCommandEntityOffline = function (commandEntity, eventContext) {
    var CommandDialogUtils = IoTConnector.Utils.CommandDialogUtils;
    var Constants = IoTConnector.Common.Constants;
    var formAttributes = CommandDialogUtils.getFormAttributes(CommandDialogUtils.getFormContext(eventContext));
    var CommandViewAttributes = Constants.CommandViewAttributes;
    var device = CommandDialogUtils.getFieldValue(CommandViewAttributes.CommandAttributeMsdynDevice, formAttributes);
    function getOfflineEntityReference(logicalName, id) {
        id = IoTConnector.Utils.CommandDialogUtils.formatGuid(id);
    ...
    var CommandAttributeParameters = Constants.CommandAttributeParameters;
    var EntityLogicalNames = Constants.EntityLogicalNames;
    // Add command definition information as command parameters to the JSON object commandEntity
    var commandDefLookup = CommandDialogUtils.getFieldValue(CommandViewAttributes.CommandAttributeMsdynCommand, formAttributes);
    if (!CommandDialogUtils.isNullOrEmpty(commandDefLookup)) {
        var commandDefName = commandDefLookup[0]['name'];
        var commandDefId = commandDefLookup[0]['id'];
        commandEntity.msdyn_name = msdyn.IoTCommandDialog.getCommandName(device[0]['name'], commandDefName);
        commandEntity[CommandViewAttributes.CommandAttributeMsdynCommand] = getOfflineEntityReference(EntityLogicalNames.CommandDefinitionEntityLogicalName, commandDefId);
    }
    else {
        commandEntity.msdyn_name = msdyn.IoTCommandDialog.getCommandName(device[0]['name']);
    }
    ...
    var alertIdField = CommandDialogUtils.getField(CommandAttributeParameters.CommandAttributeAlertId, formAttributes);
    if (alertIdField && alertIdField.getValue()) {
        commandEntity[CommandViewAttributes.CommandAttributeMsdynParentAlert] = getOfflineEntityReference(EntityLogicalNames.AlertEntityLogicalName, alertIdField.getValue());
    }
    // Add device information as command parameters to the JSON object commandEntity
    var deviceField = CommandDialogUtils.getField(CommandViewAttributes.CommandAttributeMsdynDevice, formAttributes);
    if (deviceField && !CommandDialogUtils.isNullOrEmpty(deviceField.getValue())) {
        commandEntity[CommandViewAttributes.CommandAttributeMsdynDevice] = getOfflineEntityReference(EntityLogicalNames.DeviceEntityLogicalName, deviceField.getValue()[0]['id']);
    }
};
// Get localized command name, i.e. <CommandName> sent to <devicename> if there is command, otherwise, Command sent to <devicename>
// CommandUtils: reference to a class containing common methods for dialog and form
// deviceName: the name of the device that command sent to
// commandDefName: command definition name
// Return the localized command name
msdyn.IoTCommandDialog.getCommandName = function (deviceName, commandDefName) {
    var Localization = IoTConnector.Localization.Localization;
    if (commandDefName) {
        return Localization.format(Localization.localize(msdyn.IoTCommandDialog.commandNameWithCommandFilledLabelKey), commandDefName, deviceName);
    }
    else {
        return Localization.format(Localization.localize(msdyn.IoTCommandDialog.commandNameOnlyMessageFilledLabelKey), deviceName);
    }
};
// Send request over webapi for creating command record
// commandEntity: the JSON object that holds the command parameters
// CommandUtils: reference to a class containing common methods for dialog and form
// eventContent: includes event context information
msdyn.IoTCommandDialog.send = function (commandEntity, eventContext) {
    if (!eventContext) {
        return;
    }
    var formContext = eventContext.getFormContext();
    var CommandDialogUtils = IoTConnector.Utils.CommandDialogUtils;
    CommandDialogUtils.removeNullvalue(commandEntity);
    if (!msdyn.IoTCommandDialog.validate(commandEntity, formContext)) {
        return;
    }
    // will review with other definitions together in this task:1421694
    var XrmWeb = Xrm;
    if (IoTConnector.ApiUtils.isMobileClientOffline()) {
        // try to save entity in case if we have internet connection in Offline mode
        IoTConnector.Utils.ODataUtils.executeRequest("POST", IoTConnector.Common.Constants.PluralEntityLogicalNames.CommandEntityLogicalName, commandEntity, function (result) {
            // saved to online database
            Xrm.Utility.alertDialog(IoTConnector.Localization.Localization.localize(msdyn.IoTCommandDialog.commandSentOffline));
            CommandDialogUtils.closeDialog(eventContext);
        }, function (error) {
            // no internet connection, saving to offline database
            var offlineCommandEntity = {};
            msdyn.IoTCommandDialog.setCommandEntityOffline(offlineCommandEntity, eventContext);
            XrmWeb.WebApi.offline.createRecord(IoTConnector.Common.Constants.EntityLogicalNames.CommandEntityLogicalName, offlineCommandEntity).then(function success(result) {
                //msdyn.IoTCommandDialog.sendToastNotification(XrmWeb, result.id);
                Xrm.Utility.alertDialog(IoTConnector.Localization.Localization.localize(msdyn.IoTCommandDialog.commandNotSentOffline));
                CommandDialogUtils.closeDialog(eventContext);
            }, function (error) {
                formContext.ui.setFormNotification(IoTConnector.Localization.Localization.localize("@:commandCreationFailed"), "ERROR", null);
                console.log(error.message);
            });
        });
    }
    else {
        XrmWeb.WebApi.createRecord(IoTConnector.Common.Constants.EntityLogicalNames.CommandEntityLogicalName, commandEntity).then(function success(result) {
            msdyn.IoTCommandDialog.sendToastNotification(XrmWeb, result.id);
            CommandDialogUtils.closeDialog(eventContext);
        }, function (error) {
            formContext.ui.setFormNotification(IoTConnector.Localization.Localization.localize("@:commandCreationFailed"), "ERROR", null);
...
// Register onchange event for commandlookup and commandmessage fields to validate command information
msdyn.IoTCommandDialog.addCommandOnChange = function (formContext) {
    if (!formContext) {
        return;
    }
    var CommandDialogUtils = IoTConnector.Utils.CommandDialogUtils;
    var Constants = IoTConnector.Common.Constants;
    var formAttributes = CommandDialogUtils.getFormAttributes(formContext);
    var commandLookupField = CommandDialogUtils.getField(Constants.CommandViewAttributes.CommandAttributeMsdynCommand, formAttributes);
    var commandMessageField = CommandDialogUtils.getField(Constants.CommandViewAttributes.CommandAttributeMsdynMessage, formAttributes);
    if (!commandLookupField || !commandMessageField) {
        return;
    }
    // Register onchange event for commandlookup and commandmessage fields
    msdyn.IoTCommandDialog.addOnChange(commandLookupField, msdyn.IoTCommandDialog.onCommandLookupChange);
    msdyn.IoTCommandDialog.addOnChange(commandMessageField, msdyn.IoTCommandDialog.onCommandMessageChange);
};
// Helper function to register function to change event
msdyn.IoTCommandDialog.addOnChange = function (field, functionName) {
    if (field && field.addOnChange && functionName) {
        // Avoid register multiple same event functions
        field.removeOnChange(functionName);
        field.addOnChange(functionName);
    }
};
// Event function when commandlookup field get changed
msdyn.IoTCommandDialog.onCommandLookupChange = function (eventContext) {
    if (!eventContext) {
        return;
    }
    msdyn.IoTCommandDialog.onCommandChange(eventContext);
    msdyn.IoTCommandDialog.updateMessage(eventContext);
};
// Event function when commandmessage field get changed
msdyn.IoTCommandDialog.onCommandMessageChange = function (eventContext) {
    if (!eventContext) {
        return;
    }
    msdyn.IoTCommandDialog.onCommandChange(eventContext);
};
msdyn.IoTCommandDialog.onCommandChange = function (eventContext) {
    if (!eventContext) {
        return;
    }
    var formContext = eventContext.getFormContext();
    var commandEntity = new IoTConnector.Command.Models.Command();
    msdyn.IoTCommandDialog.setCommandEntity(commandEntity, eventContext);
    // Validate commandlookup and commandmessage fields
    msdyn.IoTCommandDialog.validateCommandLookupAndValue(commandEntity, formContext);
};
// Validate commandlookup and commandmessage fields cannot be both empty
msdyn.IoTCommandDialog.validateCommandLookupAndValue = function (commandEntity, formContext) {
    var isvalid = false;
    var messageKey = "commandLookupAndMessageCannotBeBothEmpty";
    if (commandEntity && commandEntity[IoTConnector.Common.Constants.CommandViewAttributes.CommandAttributeMsdynDeviceOdataBind]
        && !commandEntity[IoTConnector.Common.Constants.CommandViewAttributes.CommandAttributeMsdynCommandOdataBind]
        && !commandEntity.msdyn_message) {
        formContext.ui.setFormNotification(IoTConnector.Localization.Localization.localize("@:" + messageKey), "ERROR", messageKey);
    }
    else {
        formContext.ui.clearFormNotification(messageKey);
        isvalid = true;
    }
    return isvalid;
};
// Check if the device field is filled out, and commandlookup and commandmessage fields are not both empty
msdyn.IoTCommandDialog.validate = function (commandEntity, formContext) {
    return msdyn.IoTCommandDialog.validateDeviceFieldValue(formContext) &&
        msdyn.IoTCommandDialog.validateCommandLookupAndValue(commandEntity, formContext);
};
msdyn.IoTCommandDialog.validateDeviceFieldValue = function (formContext) {
    var deviceField = IoTConnector.Utils.CommandDialogUtils.getField(IoTConnector.Common.Constants.CommandViewAttributes.CommandAttributeMsdynDevice, IoTConnector.Utils.CommandDialogUtils.getFormAttributes(formContext));
    if (!deviceField) {
        return false;
    }
    if (!deviceField.getValue()) {
        deviceField.setValue("");
        return false;
    }
    return true;
};
// Display toast notification after command is sent
msdyn.IoTCommandDialog.sendToastNotification = function (xrmWeb, commandID) {
    IoTConnector.Utils.CommandDialogUtils.sendToastNotification(xrmWeb, commandID);
};
// Show title in the dialog as Send Command To '@Device Name' if device name exists, otherwise, show Send Command
// formContext: includes form context information
msdyn.IoTCommandDialog.setTitle = function (formContext) {
    if (!formContext) {
        return;
    }
    var CommandDialogUtils = IoTConnector.Utils.CommandDialogUtils;
    var Localization = IoTConnector.Localization.Localization;
    // Get command title control
    var commandTitleControl = formContext.getControl(msdyn.IoTCommandDialog.commandTitle);
    if (commandTitleControl) {
        var formAttributes = CommandDialogUtils.getFormAttributes(formContext);
        // Get the value of device name from the dialog to decide which localized title to show up
        var deviceField = CommandDialogUtils.getField(IoTConnector.Common.Constants.CommandViewAttributes.CommandAttributeMsdynDevice, formAttributes);
        if (deviceField && !CommandDialogUtils.isNullOrEmpty(deviceField.getValue())) {
            var localizedCommandTitle = Localization.localize(msdyn.IoTCommandDialog.sendCommandToLabelKey);
            // Show title: Send Command To '@Device Name'
            commandTitleControl.setLabel(Localization.format(localizedCommandTitle, deviceField.getValue()[0]['name']));
        }
        else {
            // Show title: Send Command
            commandTitleControl.setLabel(Localization.localize(msdyn.IoTCommandDialog.sendCommandLabelKey));
        }
    }
};
// Handle the event when 'Advanced' button is clicked to direct the user to the standard command form
// eventContext: includes event context information
msdyn.IoTCommandDialog.onClickAdvanced = function (eventContext) {
    var formContext = eventContext ? eventContext.getFormContext() : null;
    if (!formContext) {
        return;
    }
    ...
    var CommandViewAttributes = Constants.CommandViewAttributes;
    var commandModel = new IoTConnector.Command.Models.Command();
    commandModel.msdyn_device = CommandDialogUtils.getFieldValue(CommandViewAttributes.CommandAttributeMsdynDevice, formAttributes);
    commandModel.msdyn_command = CommandDialogUtils.getFieldValue(CommandViewAttributes.CommandAttributeMsdynCommand, formAttributes);
    commandModel.msdyn_message = CommandDialogUtils.getFieldValue(CommandViewAttributes.CommandAttributeMsdynMessage, formAttributes);
    commandModel.msdyn_parentalert = CommandDialogUtils.getFieldValue(commandAttributeParameters.CommandAttributeAlertId, formAttributes);
    commandModel.msdyn_parentalertname = CommandDialogUtils.getFieldValue(commandAttributeParameters.CommandAttributeAlertName, formAttributes);
    var entityFormOptions = {};
    entityFormOptions["entityName"] = Constants.EntityLogicalNames.CommandEntityLogicalName;
    entityFormOptions["openInNewWindow"] = true;
    Xrm.Navigation.openForm(entityFormOptions, commandModel);
    CommandDialogUtils.closeDialog(eventContext);
};
// Update title and command lookup when device lookup changes
// eventContext: includes event context information
msdyn.IoTCommandDialog.onChangeDevice = function (eventContext) {
    var formContext = eventContext ? eventContext.getFormContext() : null;
    if (!formContext) {
        return;
    }
...
// This is work around for metadata control in MDD
// The lookup attribute is replaced with single text control,
// which update will trigger the metadata control to execute to update the message field
// eventContext: includes the event context information
msdyn.IoTCommandDialog.updateMessage = function (eventContext) {
    var formContext = eventContext ? eventContext.getFormContext() : null;
    if (!formContext) {
        return;
    }
    var CommandDialogUtils = IoTConnector.Utils.CommandDialogUtils;
    var formAttributes = CommandDialogUtils.getFormAttributes(formContext);
    var commandViewAttributes = IoTConnector.Common.Constants.CommandViewAttributes;
    var commandMessageField = CommandDialogUtils.getField(commandViewAttributes.CommandAttributeMsdynMessage, formAttributes);
    var commandReferenceField = CommandDialogUtils.getField(commandViewAttributes.CommandAttributeMsdynCommandReference, formAttributes);
    // Clean message field
    commandMessageField.setValue("");
    // Set command lookup payload to the command reference field, and the value change of this field will trigger metadata control
    var commandLookupValue = CommandDialogUtils.getFieldValue(commandViewAttributes.CommandAttributeMsdynCommand, formAttributes);
    if (!CommandDialogUtils.isNullOrEmpty(commandLookupValue)) {
        var commandRefValue = new Array();
        commandRefValue[0] = new Object();
        commandRefValue[0].Id = commandLookupValue[0]['id'];
        commandRefValue[0].LogicalName = commandLookupValue[0]['entityType'];
        commandReferenceField.setValue(JSON.stringify(commandRefValue));
    }
    else {
        commandReferenceField.setValue("{}");
    }
