...
msdyn.IoTDevice.sendCommand = function (primaryControl, recordId) {
    ...
    if (recordId) {
        ...
        if (recordId.length === 1) {
            ...
            var deviceInfo = new IoTAssetCommon.Models.AlertAndDeviceInfo();
            deviceInfo.iotdeviceid = deviceReference.Id;
            deviceInfo.iotdevicename = deviceReference.Name;
            deviceInfo.assethide = true;
            if (IoTConnector.ApiUtils.IsMobile()) {
                if (!IoTConnector.ApiUtils.CrudApiSDK.isNetworkAvailable()) {
                    Xrm.Navigation.openAlertDialog({ text: IoTAssetCommon.StringProvider.getResourceString("sendCommandRequiresInternetConnection") });
                    return;
                }
                IoTConnector.Utils.CommandDialogUtils.openNewCommandDialog(deviceInfo);
            }
        ...
        primaryControl.data.save().then(function () {
            ...
            if (iotDeviceNameAttr) {
                var deviceInfo = new IoTAssetCommon.Models.AlertAndDeviceInfo();
                deviceInfo.iotdeviceid = iotDeviceId;
                deviceInfo.iotdevicename = iotDeviceNameAttr.getValue();
                deviceInfo.assethide = true;
                if (IoTConnector.ApiUtils.IsMobile()) {
                    IoTConnector.Utils.CommandDialogUtils.openNewCommandDialog(deviceInfo);
                }
