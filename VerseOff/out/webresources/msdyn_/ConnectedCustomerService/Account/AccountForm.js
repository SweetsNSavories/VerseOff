...
var msdyn = msdyn || {};
msdyn.CCS = msdyn.CCS || {};
msdyn.CCS.accountForm = msdyn.CCS.accountForm || {};
msdyn.CCS.accountForm.devicesTab = "Devices_Tab";
msdyn.CCS.accountForm.formContext = null;
msdyn.CCS.accountForm.onLoad = function (executionContext) {
    msdyn.CCS.accountForm.formContext = executionContext.getFormContext();
    // Show Devices tab only if IoT is configured
    msdyn.CCS.accountForm.ShoworHideDevicesTab();
};
// Show or Hide Devices tab
msdyn.CCS.accountForm.ShoworHideDevicesTab = function () {
    var devicesTab = msdyn.CCS.accountForm.formContext.ui.tabs.get(msdyn.CCS.accountForm.devicesTab);
    CCS.Utils.CCSIndicatorUtils.iotIsActive().then(function (isActive) {
        if (devicesTab !== null) {
            devicesTab.setVisible(isActive);
        }
    }, function (error) {
        CCS.Common.Telemetry.logError('AccountForm', 'ShoworHideDevicesTab', {
            Control: CCS.Common.Constants.TelemetryConstants.ClientName,
            Error: error
        });
    });
