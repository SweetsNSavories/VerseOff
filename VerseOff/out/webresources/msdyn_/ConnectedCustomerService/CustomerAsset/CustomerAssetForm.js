...
var msdyn = msdyn || {};
msdyn.CCS = msdyn.CCS || {};
msdyn.CCS.CustomerAssetForm = msdyn.CCS.CustomerAssetForm || {};
msdyn.CCS.CustomerAssetForm.formContext = null;
msdyn.CCS.CustomerAssetForm.onLoad = function (executionContext) {
    msdyn.CCS.CustomerAssetForm.formContext = executionContext.getFormContext();
    msdyn.CCS.CustomerAssetForm.HideCustomerAssetCategoryField();
    var deviceInsightsTab = msdyn.CCS.CustomerAssetForm.formContext.ui.tabs.get(CCSRef.Common.Constants.TabNames.DeviceInsightsTab);
    if (msdyn.CCS.CustomerAssetForm.formContext.ui.getFormType() !== 1 /* Create */) {
        // Logic to show or hide Device Insights (Preview) tab on form load
        CCS.Utils.CCSIndicatorUtils.iotIsActive().then(function (isActive) {
            if (isActive) {
                msdyn.CCS.CustomerAssetForm.ShoworHideDeviceInsights(deviceInsightsTab);
            }
        }, function (error) {
            CCS.Common.Telemetry.logError('CustomerAssetForm', 'onLoad', {
                Control: CCS.Common.Constants.TelemetryConstants.ClientName,
                Error: error
            });
        });
...
// Show or hide category attribute on customer asset main form
msdyn.CCS.CustomerAssetForm.HideCustomerAssetCategoryField = function () {
    CCS.Utils.CCSIndicatorUtils.isCSHApp().then(function (isCSH) {
        var iotCustomerassetcategory = msdyn.CCS.CustomerAssetForm.formContext.ui.controls.get("msdyn_customerassetcategory");
        if (iotCustomerassetcategory !== null) {
            iotCustomerassetcategory.setVisible(!isCSH);
        }
    }, function (error) {
        CCS.Common.Telemetry.logError('CustomerAssetForm', 'HideCustomerAssetCategoryField', {
            Control: CCS.Common.Constants.TelemetryConstants.ClientName,
            Error: error
        });
    });
};
msdyn.CCS.CustomerAssetForm.ShoworHideDeviceInsights = function (tab) {
    if (tab) {
        var assetId = msdyn.CCS.CustomerAssetForm.formContext.data.entity.getId();
        var ioTDevicesFromAssetFetchXml = CCS.Utils.CCSIndicatorUtils.getIoTDevicesFromAssetFetchXml(assetId);
        CCSRef.Utils.ODataUtils.executeRequest("GET", CCSRef.Utils.ODataUtils.formatFetchXml(CCSRef.Common.Constants.PluralEntityLogicalNames.DeviceEntityLogicalName, ioTDevicesFromAssetFetchXml), "", function (data) {
            if (data) {
                ...
                if (devices && devices.length > 0) {
                    ...
                    CCS.Utils.CCSIndicatorUtils.shouldDeviceInsightsBeVisible("msdyn_iotsettingses", fetchXmlQuery).then(function (showDeviceInsights) {
                        ...
                        CCS.Common.Telemetry.logError('CustomerAssetForm', 'ShoworHideDeviceInsights', {
                            Control: CCS.Common.Constants.TelemetryConstants.ClientName,
                            Error: error
                        });
                    });
            ...
            CCS.Common.Telemetry.logInfo('CustomerAssetForm', 'ShoworHideDeviceInsights', {
                Control: CCS.Common.Constants.TelemetryConstants.ClientName,
