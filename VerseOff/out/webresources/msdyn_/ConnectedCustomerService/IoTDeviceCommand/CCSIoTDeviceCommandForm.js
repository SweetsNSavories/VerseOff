...
/// <reference path="../../Package/IoT/CrmPackage/WebResources/msdyn_/IoT/Utils/CommandDialogUtils.ts" />
/// <reference path="../../Package/IoT/CrmPackage/WebResources/msdyn_/IoT/Common/Constants.ts" />
/// <reference path="../../Package/IoTAssetCommon/CrmPackage/WebResources/msdyn_/IoTAssetCommon/Constants/Constants.ts" />
/// <reference path="../Utils/Common/Telemetry.ts" />
var msdyn = msdyn || {};
msdyn.CCS = msdyn.CCS || {};
msdyn.CCS.IoTDeviceCommandForm = msdyn.CCS.IoTDeviceCommandForm || {};
msdyn.CCS.IoTDeviceCommandForm.formContext = null;
msdyn.CCS.IoTDeviceCommandForm.onLoad = function (executionContext) {
    msdyn.CCS.IoTDeviceCommandForm.formContext = executionContext.getFormContext();
    msdyn.CCS.IoTDeviceCommandForm.ShowOrHideIoTDeviceOnIoTDeviceCommandMainForm();
};
// Show or hide Device lookup attribute on IOTAlert main form
msdyn.CCS.IoTDeviceCommandForm.ShowOrHideIoTDeviceOnIoTDeviceCommandMainForm = function () {
    CCS.Utils.CCSIndicatorUtils.isCSHApp().then(function (isCSH) {
        var iotDeviceLookup = msdyn.CCS.IoTDeviceCommandForm.formContext.ui.controls.get("msdyn_device");
        var optionToSendAllDevices = msdyn.CCS.IoTDeviceCommandForm.formContext.ui.controls.get("msdyn_sendtoallconnecteddevices");
        if (iotDeviceLookup !== null) {
            iotDeviceLookup.setVisible(!isCSH);
        }
        if (optionToSendAllDevices !== null) {
            optionToSendAllDevices.setVisible(!isCSH);
        }
    }, function (error) {
        CCS.Common.Telemetry.logError('CCSIoTDeviceCommandForm', 'ShowOrHideIoTDeviceOnIoTDeviceCommandMainForm', {
            Control: CCS.Common.Constants.TelemetryConstants.ClientName,
            Error: error
        });
    });
};
msdyn.CCS.IoTDeviceCommandForm.onSave = function (eventContext) {
    var formContext = eventContext ? eventContext.getFormContext() : null;
    var CommandDialogUtils = CCSRef.Utils.CommandDialogUtils;
    var formAttributes = CommandDialogUtils.getFormAttributes(formContext);
    var caseIdParam = CommandDialogUtils.getFieldValue("param_msdyn_caseId", formAttributes);
    if (caseIdParam) {
        var assetAttribute = formContext.getAttribute(CCSRefAsset.Common.Constants.CommandViewAttributes.CommandAttributeMsdynCustomerAsset);
        var asset = assetAttribute != null ? assetAttribute.getValue() : null;
        if (asset) {
            msdyn.CaseDeviceAssociationUtils.CCS.associateRequest(caseIdParam.replace(/[{}]/g, ""), asset[0].id.replace(/[{}]/g, ""));
        }
    }
};
