...
var msdyn = msdyn || {};
msdyn.CCS = msdyn.CCS || {};
msdyn.CCS.IoTAlertForm = msdyn.CCS.IoTAlertForm || {};
msdyn.CCS.IoTAlertForm.formContext = null;
msdyn.CCS.IoTAlertForm.onLoad = function (executionContext) {
    msdyn.CCS.IoTAlertForm.formContext = executionContext.getFormContext();
    msdyn.CCS.IoTAlertForm.ShowOrHideIoTDeviceOnIoTAlertMainForm();
};
// Show or hide Device lookup attribute on IOTAlert main form
msdyn.CCS.IoTAlertForm.ShowOrHideIoTDeviceOnIoTAlertMainForm = function () {
    CCS.Utils.CCSIndicatorUtils.isCSHApp().then(function (isCSH) {
        var iotDeviceLookup = msdyn.CCS.IoTAlertForm.formContext.ui.controls.get("msdyn_device");
        if (iotDeviceLookup !== null) {
            iotDeviceLookup.setVisible(!isCSH);
        }
    }, function (error) {
        CCS.Common.Telemetry.logError('CCSIoTAlertForm', 'ShowOrHideIoTDeviceOnIoTAlertMainForm', {
            Control: CCS.Common.Constants.TelemetryConstants.ClientName,
            Error: error
        });
    });
