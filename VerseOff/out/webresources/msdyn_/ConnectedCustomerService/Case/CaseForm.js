...
(function (CCS) {
    ...
    (function (ODataContract) {
        var GetRelatedIoTDevicesByEntityRequest = /** @class */ (function () {
            function GetRelatedIoTDevicesByEntityRequest(entityLogicalName, templateId) {
                this.EntityLogicalName = entityLogicalName;
                this.EntityId = templateId;
            }
            GetRelatedIoTDevicesByEntityRequest.prototype.getMetadata = function () {
                var metadata = {
                    boundParameter: null,
                    parameterTypes: {
                        EntityId: {
                    ...
                    operationName: "msdyn_GetRelatedIoTDevicesByEntity",
                    operationType: 0,
                };
                return metadata;
            };
            return GetRelatedIoTDevicesByEntityRequest;
        }());
        ODataContract.GetRelatedIoTDevicesByEntityRequest = GetRelatedIoTDevicesByEntityRequest;
    })(ODataContract = CCS.ODataContract || (CCS.ODataContract = {}));
})(CCS || (CCS = {}));
/*! WARNING! Do not update this file manually! Manual update of this file is not supported and will likely lead to issues. In addition, future solution upgrades wont apply to manually edited files. */
...
/// <reference path="../Utils/DataContracts/Action/GetRelatedIoTDevicesByEntityRequest.ts" />
/// <reference path="../Utils/Common/Telemetry.ts" />
var msdyn = msdyn || {};
msdyn.CCS = msdyn.CCS || {};
msdyn.CCS.caseForm = msdyn.CCS.caseForm || {};
msdyn.CCS.caseForm.alertFieldName = "msdyn_iotalert";
msdyn.CCS.caseForm.caseEntityName = "incident";
msdyn.CCS.caseForm.deviceTab = "DeviceTab";
msdyn.CCS.caseForm.formContext = null;
msdyn.CCS.caseForm.getRelatedIoTDevicesByEntityCustomAction = "msdyn_GetRelatedIoTDevicesByEntity";
msdyn.CCS.caseForm.onLoad = function (executionContext) {
    msdyn.CCS.caseForm.formContext = executionContext.getFormContext();
    ...
    var alertField = msdyn.CCS.caseForm.formContext.getAttribute(msdyn.CCS.caseForm.alertFieldName);
    if (alertField) {
        msdyn.CCS.caseForm.displayHideIoTAlertField(alertField);
    }
    // Show Devices tab only if IoT is configured
    msdyn.CCS.caseForm.ShowOrHideDevicesTab();
    // Show Device Insights (Preview) tab when IoT Alert field is not empty
    var deviceInsightsTab = msdyn.CCS.caseForm.formContext.ui.tabs.get(CCSRef.Common.Constants.TabNames.DeviceInsightsTab);
    if (msdyn.CCS.caseForm.formContext.ui.getFormType() !== 1 /* Create */) {
        CCS.Utils.CCSIndicatorUtils.iotIsActive().then(function (isActive) {
            if (isActive) {
                msdyn.CCS.caseForm.ShoworHideDeviceInsights(deviceInsightsTab);
            }
        }, function (error) {
            CCS.Common.Telemetry.logError('CaseForm', 'onLoad', {
                Control: CCS.Common.Constants.TelemetryConstants.ClientName,
                Error: error
            });
        });
...
//Show or Hide Device tab
msdyn.CCS.caseForm.ShowOrHideDevicesTab = function () {
    var devicesTab = msdyn.CCS.caseForm.formContext.ui.tabs.get(msdyn.CCS.caseForm.deviceTab);
    CCS.Utils.CCSIndicatorUtils.iotIsActive().then(function (isActive) {
        if (devicesTab !== null) {
            devicesTab.setVisible(isActive);
        }
    }, function (error) {
        CCS.Common.Telemetry.logError('CaseForm', 'ShowOrHideDevicesTab', {
            Control: CCS.Common.Constants.TelemetryConstants.ClientName,
            Error: error
        });
    });
};
// Show or Hide Device Insights (Preview) tab based on value in IoT Alert field
msdyn.CCS.caseForm.ShoworHideDeviceInsights = function (tab) {
    var caseId = msdyn.CCS.caseForm.formContext.data.entity.getId();
    if (tab && caseId) {
        var request = new CCS.ODataContract.GetRelatedIoTDevicesByEntityRequest(msdyn.CCS.caseForm.caseEntityName, caseId);
        Xrm.WebApi.online.execute(request).then(function (response) {
            response.json().then(function (jsonResponse) {
                try {
                    var devices = JSON.parse(jsonResponse["RelatedIoTDevicesByEntityOutput"]);
                    if (devices && devices.length > 0) {
                        var fetchXmlQuery = CCS.Utils.CCSIndicatorUtils.getDeviceInsightsConfigFromIoTSettingsFetchXml();
                        //Determine showDeviceInsightsTab based on Device Insights Config
                        CCS.Utils.CCSIndicatorUtils.shouldDeviceInsightsBeVisible("msdyn_iotsettingses", fetchXmlQuery).then(function (showDeviceInsights) {
                            tab.setVisible(showDeviceInsights);
                        }, function (error) {
                            tab.setVisible(false);
                            CCS.Common.Telemetry.logError('CaseForm', 'ShoworHideDeviceInsights', {
                                Control: CCS.Common.Constants.TelemetryConstants.ClientName,
                                Error: error
                            });
                        });
...
msdyn.CCS.caseForm.displayHideIoTAlertField = function (alertField) {
    if (msdyn.CCS.caseForm.formContext.data && msdyn.CCS.caseForm.formContext.data.attributes) {
        var hideAlertFieldQueryParam = msdyn.CCS.caseForm.formContext.data.attributes.getByName("CCS_hide_iot_alert");
        ...
        CCS.Utils.CCSIndicatorUtils.iotIsActive().then(function (isActive) {
            ...
            CCS.Common.Telemetry.logError('CaseForm', 'displayHideIoTAlertField', {
                Control: CCS.Common.Constants.TelemetryConstants.ClientName,
                Error: error
            });
        });
