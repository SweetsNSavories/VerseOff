...
(function (AssetCommonUtils) {
    var Constants;
    (function (Constants) {
        Constants.CACHE_INVALIDATION_DURATION_IN_MS = 1800000; // 30 minutes
    })(Constants = AssetCommonUtils.Constants || (AssetCommonUtils.Constants = {}));
})(AssetCommonUtils || (AssetCommonUtils = {}));
...
(function (AssetCommonUtils) {
    var Utility;
    (function (Utility) {
        //Used to store key value pairs in a cache. Default expiry is 30 minutes
        function setCachedValueByKey(key, value) {
            // Set the cache with the last stored value and its expiry date
            var cacheDurationInMs = AssetCommonUtils.Constants.CACHE_INVALIDATION_DURATION_IN_MS;
            var sessionObject = {
                expiryDate: new Date(new Date().getTime() + cacheDurationInMs),
                value: value,
            };
            var objectToCache = JSON.stringify(sessionObject);
            sessionStorage.setItem(key, objectToCache);
        }
        Utility.setCachedValueByKey = setCachedValueByKey;
        //Used to retrieve key value pairs from a cache. Default response is null
        function getCachedValueByKey(key) {
            ...
            var cachedObject = JSON.parse(sessionStorage.getItem(key));
            var value;
            if (IsNullOrUndefined(cachedObject) ||
                IsNullOrUndefined(cachedObject.expiryDate) ||
                IsNullOrUndefined(cachedObject.value) ||
                new Date() >= new Date(cachedObject.expiryDate)) {
                value = null;
            }
            else {
        ...
        Utility.getCachedValueByKey = getCachedValueByKey;
        function IsNullOrUndefined(value) {
            return typeof value === "undefined" || value === null;
        }
    })(Utility = AssetCommonUtils.Utility || (AssetCommonUtils.Utility = {}));
})(AssetCommonUtils || (AssetCommonUtils = {}));
var AssetCommonUtils;
(function (AssetCommonUtils) {
    var Visibility;
...
/// <reference path="../../Utilities/Utility.ts" />
/// <reference path="../../Utilities/Visibility.ts" />
/// <reference path="../../Utilities/WebApiSDK.ts" />
var AccountFormController = /** @class */ (function () {
    function AccountFormController(form) {
        this.AssetsAndLocationsTab = "AssetsAndLocationsTab";
        this.form = form;
        //Store tab default visibility
        var assetsAndLocationsTab = AssetCommonUtils.Visibility.GetTab(this.form, this.AssetsAndLocationsTab);
        if (assetsAndLocationsTab) {
            AssetCommonUtils.Utility.setCachedValueByKey(this.AssetsAndLocationsTab, assetsAndLocationsTab.getVisible());
        }
    }
    AccountFormController.prototype.init = function () {
        this.HideAssetAndLocationsTabs();
        return this;
    };
    AccountFormController.prototype.refresh = function () {
        this.HideAssetAndLocationsTabs();
        return this;
    };
    AccountFormController.prototype.HideAssetAndLocationsTabs = function () {
        var assetsAndLocationsTab = AssetCommonUtils.Visibility.GetTab(this.form, this.AssetsAndLocationsTab);
        var isCreate = this.isCreateForm();
        var defaultTabVisibility = AssetCommonUtils.Utility.getCachedValueByKey(this.AssetsAndLocationsTab);
        if (defaultTabVisibility === null) {
            defaultTabVisibility = true;
        }
        assetsAndLocationsTab &&
            defaultTabVisibility &&
            assetsAndLocationsTab.setVisible(!isCreate);
    };
    AccountFormController.prototype.isCreateForm = function () {
        return AssetCommonUtils.Form.isFormType(this.form, AssetCommonUtils.FormType.Create);
    };
    return AccountFormController;
}());
/// <reference path="AssetCommon.AccountFormController.ts"/>
var AssetCommon;
(function (AssetCommon) {
    var Account = /** @class */ (function () {
        function Account() {
        }
        Account.OnLoad = function (eventContext) {
            if (!Account.FormController) {
                // New Form
                Account.FormController = new AccountFormController(eventContext.getFormContext()).init();
            }
            else {
                // A create form transitioning to an update should refresh all the controls.
                Account.FormController.refresh();
            }
        };
        return Account;
    }());
    AssetCommon.Account = Account;
})(AssetCommon || (AssetCommon = {}));
