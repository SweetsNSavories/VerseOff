...
(function (CrmService) {
    ...
    var ServiceActivity = (function () {
        function ServiceActivity() {
            var _this = this;
            this.commandBarActions = null;
            this.convertToCase = function (iObjType) {
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "ServiceActivity", "convertToCase::initialSave", Xrm.Page.data.entity.getId());
                if (Xrm.Page.data.entity.getIsDirty()) {
                    var isDirtyWarningResourceString = "";
                    if (ClientUtility.ClientUtil.isUCI() || Mscrm.InternalUtilities.Utilities.isTurboForm()) {
                        isDirtyWarningResourceString = CrmService.ResourceStringProvider.getResourceString("LOCID_CONV_ACT_SAVE_WARNING");
                    }
                    else {
                        isDirtyWarningResourceString = Xrm.Internal.getResourceString('LOCID_CONV_ACT_SAVE_WARNING');
                    }
                    Xrm.Navigation.openAlertDialog({
                        text: isDirtyWarningResourceString
                    });
                    if (_this.AppCommonNullCheck())
                        AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "convertToCase::initialSave", Xrm.Page.data.entity.getId());
                    return false;
                }
                var that = _this;
                var convertToCaseInternalCallback = function () {
                    var directionCodeCtrl = Xrm.Page.data.entity.attributes.get("directioncode"), directionCodeVal = true;
                    if (!ClientUtility.DataUtil.isNullOrUndefined(directionCodeCtrl)) {
                        var directionCodeValue = directionCodeCtrl.getValue();
                        directionCodeVal = directionCodeValue == "0" ? false : true;
                    }
                    var sUrlElement = "", subjectField = "", _subject = Xrm.Page.data.entity.attributes.get("subject");
                    if (!ClientUtility.DataUtil.isNullOrUndefined(_subject) && !ClientUtility.DataUtil.isNullOrUndefined(_subject.getValue()))
                        subjectField = _subject.getValue();
                    sUrlElement += "subject=" + encodeURIComponent(subjectField);
                    var customerItems = null, secondaryCust = null;
                    ...
                    if (directionCodeVal == true) {
                        ...
                        if (!ClientUtility.DataUtil.isNullOrUndefined(toBehavior))
                        ...
                        if (!ClientUtility.DataUtil.isNullOrUndefined(ccBehavior)) {
                        ...
                        if (!ClientUtility.DataUtil.isNullOrUndefined(bccBehavior)) {
                        ...
                        if (!ClientUtility.DataUtil.isNullOrUndefined(fromBehavior))
                            customerItems = fromBehavior.getValue();
                    }
                    if (iObjType == CrmService.EntityTypeCodes.Appointment || iObjType == CrmService.EntityTypeCodes.RecurringAppointmentMaster) {
                        var requiredattendees = Xrm.Page.data.entity.attributes.get("requiredattendees");
                        if (!ClientUtility.DataUtil.isNullOrUndefined(requiredattendees))
                            customerItems = requiredattendees.getValue();
                        if (ClientUtility.DataUtil.isNullOrUndefined(customerItems))
                            customerItems = [];
                        var optionalattendees = Xrm.Page.data.entity.attributes.get("optionalattendees");
                        if (!ClientUtility.DataUtil.isNullOrUndefined(optionalattendees)) {
                    ...
                    if (iObjType == CrmService.EntityTypeCodes.Task) {
                        var regardingObjectId = Xrm.Page.data.entity.attributes.get("regardingobjectid");
                        if (!ClientUtility.DataUtil.isNullOrUndefined(regardingObjectId))
                            customerItems = regardingObjectId.getValue();
                        if (ClientUtility.DataUtil.isNullOrUndefined(customerItems))
                            customerItems = [];
                    }
                    if (!ClientUtility.DataUtil.isNullOrUndefined(customerItems)) {
                        that.GetValidCustomer(customerItems, sUrlElement, iObjType, 0, CrmService.EntityNames.Incident);
                    }
                    else {
                        that.GetValidCustomerSuccess(sUrlElement, iObjType, customerItems, null, null);
                    }
                };
                if (ClientUtility.ClientUtil.isUCI() || (Mscrm.InternalUtilities.Utilities).isRefreshForm()) {
                    if (Xrm.Page.data.entity.getIsDirty()) {
                        Xrm.Page.data.save().then(function () {
                            if (_this.AppCommonNullCheck())
                                AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "convertToCase::initialSave", Xrm.Page.data.entity.getId(), "", true);
                            convertToCaseInternalCallback();
                        }, ClientUtility.ActionFailedHandler.actionFailedCallback);
                    }
                    else {
                        if (_this.AppCommonNullCheck()) {
                ...
                else if (Xrm.Page.data.isValid()) {
                    convertToCaseInternalCallback();
                }
            };
            this.GetValidCustomer = function (customerItems, sUrlElement, iObjType, customerNo, customerType) {
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomer::initiateValidation", Xrm.Page.data.entity.getId());
                if (customerNo == customerItems.length) {
                    var isValidCustomer_wm = _this.GetValidCustomerSuccess(sUrlElement, iObjType, customerItems, customerType, null);
                    if (_this.AppCommonNullCheck())
                        AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomer::initiateValidation", Xrm.Page.data.entity.getId());
                    return isValidCustomer_wm;
                }
                try {
                    var isValidCustomer = true;
                    if (!ClientUtility.DataUtil.isNullOrUndefined(customerType) && customerType == "lead" && customerItems[customerNo].entityType != CrmService.EntityNames.Lead)
                        isValidCustomer = false;
                    if (!ClientUtility.DataUtil.isNullOrUndefined(customerType) && customerType != "lead" && (customerItems[customerNo].entityType != CrmService.EntityNames.Account && customerItems[customerNo].entityType != CrmService.EntityNames.Contact))
                        isValidCustomer = false;
                    if (!isValidCustomer) {
                        var isValidCustomer_wm_1 = _this.GetValidCustomer(customerItems, sUrlElement, iObjType, customerNo + 1, customerType);
                        if (_this.AppCommonNullCheck())
                            AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomer::initiateValidation", Xrm.Page.data.entity.getId());
                        return isValidCustomer_wm_1;
                    }
                    if (ClientUtility.ClientUtil.isMobileOffline()) {
                        ...
                        var columns = [];
                        columns[0] = "statecode";
                        Xrm.WebApi.retrieveRecord(customerItems[customerNo].entityType, customerItems[customerNo].id, ClientUtility.ODataUtil.getSelectOption(columns)).then(function (response) {
                            if (_this.AppCommonNullCheck())
                                AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomer::initiateValidation", Xrm.Page.data.entity.getId(), "", true);
                            if (_this.AppCommonNullCheck())
                                AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomer::extensiveValidation", Xrm.Page.data.entity.getId());
                            if (typeof response != "undefined" && response.statecode === 0) {
                                var accountContactId = response.accountid || response.contactid;
                                if (!ClientUtility.DataUtil.isNullOrUndefined(accountContactId)) {
                                    for (var id = accountContactId.toString(), k = 0; k < customerItems.length; k++)
                                        if (!ClientUtility.DataUtil.isNullOrUndefined(customerItems[k].id) && customerItems[k].id.toLowerCase().indexOf(id.toLowerCase()) != -1) {
                                            sUrlElement += _this.AppendValidCustomerAttributes(customerItems, k, customerType);
                                            var isValidCustomer_wm_2 = _this.GetValidCustomerSuccess(sUrlElement, iObjType, customerItems, customerType, _this);
                                            if (_this.AppCommonNullCheck())
                                                AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomer::extensiveValidation", Xrm.Page.data.entity.getId());
                                            return isValidCustomer_wm_2;
                                        }
                                    var isValidCustomer_wm_3 = _this.GetValidCustomer(customerItems, sUrlElement, iObjType, customerNo + 1, customerType);
                                    if (_this.AppCommonNullCheck())
                                        AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomer::extensiveValidation", Xrm.Page.data.entity.getId());
                                    return isValidCustomer_wm_3;
                                }
                                else {
                                    var isValidCustomer_wm_4 = _this.GetValidCustomer(customerItems, sUrlElement, iObjType, customerNo + 1, customerType);
                                    if (_this.AppCommonNullCheck())
                                        AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomer::extensiveValidation", Xrm.Page.data.entity.getId());
                                    return isValidCustomer_wm_4;
                                }
                            }
                            else {
                                var isValidCustomer_wm_5 = _this.GetValidCustomer(customerItems, sUrlElement, iObjType, customerNo + 1, customerType);
                                if (_this.AppCommonNullCheck())
                                    AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomer::extensiveValidation", Xrm.Page.data.entity.getId());
                                return isValidCustomer_wm_5;
                            }
                        }, function (response) {
                            var isValidCustomer_wm = _this.GetValidCustomer(customerItems, sUrlElement, iObjType, customerNo + 1, customerType);
                            if (_this.AppCommonNullCheck())
                                AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomer::initiateValidation", Xrm.Page.data.entity.getId(), "", true);
                            return isValidCustomer_wm;
                        });
            ...
            this.AppendValidCustomerAttributes = function (customerItems, customerNo, customerType) {
                if (customerType != "lead")
                    return "&customerId=" + encodeURIComponent(customerItems[customerNo].id) + "&customerType=" + encodeURIComponent(CrmService.ClientUtil.getEntityTypeCodes(customerItems[customerNo].entityType).toString()) + "&customerName=" + encodeURIComponent(customerItems[customerNo].name);
                else
                    return "&leadId=" + encodeURIComponent(customerItems[customerNo].id) + "&leadName=" + encodeURIComponent(customerItems[customerNo].name);
            };
            this.GetValidCustomerSuccess = function (sUrlElement, iObjType, customerItems, customerType, entityId) {
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomerSuccess::convertCaseToPreload", Xrm.Page.data.entity.getId());
                var ownerId = "";
                var ownerType = "";
                var ownerCtrl = Xrm.Page.data.entity.attributes.get("ownerid");
                if (!ClientUtility.DataUtil.isNullOrUndefined(ownerCtrl)) {
                ...
                sUrlElement += "&ownerId=" + encodeURIComponent(ownerId);
                sUrlElement += "&ownerType=" + encodeURIComponent(ownerType);
                _this.commandBarActions.convertToCasePreload(sUrlElement, iObjType, entityId);
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "GetValidCustomerSuccess::convertCaseToPreload", Xrm.Page.data.entity.getId());
            };
            this.convertToCaseSuccess = function (response, saveActivity, openNewRecord, iObjType) {
                var caseId;
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "ServiceActivity", "convertToCaseSuccess::initiateConversion", Xrm.Page.data.entity.getId());
                ...
                if (caseId != "" && !ClientUtility.DataUtil.isNullOrUndefined(_regardingobjectid)) {
                    var itemsArray = [], reg;
                    reg.id = caseId;
                    reg.entityType = CrmService.EntityNames.Incident;
                ...
                if (CrmService.ServiceCommandBarActions.isMobileCompanionApp()) {
                    if (saveActivity == false)
                        caseId != "" && !ClientUtility.DataUtil.isNullOrUndefined(_regardingobjectid) &&
                            Xrm.Page.data.save().then(function () {
                                if (_this.AppCommonNullCheck())
                                    AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "convertToCaseSuccess::initiateConversion", Xrm.Page.data.entity.getId(), "", true);
                                if (_this.AppCommonNullCheck())
                                    AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "ServiceActivity", "convertToCaseSuccess::OpenEntityForm", Xrm.Page.data.entity.getId());
                                openNewRecord &&
                                    Xrm.Utility.openEntityForm(CrmService.EntityNames.Incident, caseId);
                                if (_this.AppCommonNullCheck())
                                    AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "convertToCaseSuccess::OpenEntityForm", Xrm.Page.data.entity.getId());
                            }, ClientUtility.DialogUtil.actionFailedCallbackForMoca);
                    else {
                        //Xrm.Page.data.setFormDirty(false);
                        if (openNewRecord && caseId != "") {
                            _this.commandBarActions.markActivityComplete(Xrm.Page.data.entity.getId(), Xrm.Page.data.entity.getEntityName(), false, CrmService.EntityNames.Incident, caseId);
                        }
                        else
                            _this.commandBarActions.markActivityComplete(Xrm.Page.data.entity.getId(), Xrm.Page.data.entity.getEntityName(), true, CrmService.EntityNames.Incident, null);
                    }
                }
                else
                    _this.convertToEntityLegacy(caseId, saveActivity, openNewRecord, iObjType, CrmService.EntityNames.Incident);
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "convertToCaseSuccess::initiateConversion", Xrm.Page.data.entity.getId());
            };
            ...
            this.convertToEntityLegacy = function (entityId, saveActivity, openNewRecord, iObjType, targetEntityName) {
                var crmFormCtrl = $find("crmForm");
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "ServiceActivity", "convertToEntityLegacy::initiateConversion", Xrm.Page.data.entity.getId());
                if (saveActivity == false) {
                    crmFormCtrl.SubmitCrmForm(1, true, true, false);
                }
                else
                    crmFormCtrl.SubmitCrmForm(58, true, true, false);
                openNewRecord && !ClientUtility.DataUtil.isNullOrUndefined(entityId) &&
                    Xrm.Utility.openEntityForm(targetEntityName, entityId);
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "ServiceActivity", "convertToEntityLegacy::initiateConversion", Xrm.Page.data.entity.getId());
            };
            ...
            this.commandBarActions = new CrmService.ServiceCommandBarActions();
        }
        return ServiceActivity;
    }());
    CrmService.ServiceActivity = ServiceActivity;
})(CrmService || (CrmService = {}));
/**
* @license Copyright (c) Microsoft Corporation. All rights reserved.
*/
/// <reference path="../../../../TypeDefinitions/CRM/ClientUtility.d.ts" />
///// <reference path="../ServiceClientCommon/Service_ClientCommon.ts" />
/// <reference path="UCI/ServiceActivity.ts" />
var CrmService;
(function (CrmService) {
    'use strict';
    ...
    Activity.Instance = new CrmService.ServiceActivity();
    Activity.ctor = (function () {
    })();
    CrmService.Activity = Activity;
})(CrmService || (CrmService = {}));
