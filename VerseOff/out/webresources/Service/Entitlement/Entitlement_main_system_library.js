...
(function (ODataContract) {
    ...
    var RenewEntitlementRequest = (function () {
        function RenewEntitlementRequest(entity /*Microsoft.Dynamics.CRM.entitlement*/, status) {
            this.entity = entity;
            this.Status = status;
        }
        RenewEntitlementRequest.prototype.getMetadata = function () {
            var metadata = {
                boundParameter: "entity",
                parameterTypes: {
                    "entity": {
                        "typeName": "Microsoft.Dynamics.CRM.entitlement",
                ...
                operationName: "RenewEntitlement",
                operationType: 0,
            };
            return metadata;
        };
    ...
    ODataContract.RenewEntitlementRequest = RenewEntitlementRequest;
})(ODataContract || (ODataContract = {}));
/**
* @license Copyright (c) Microsoft Corporation. All rights reserved.
*/
...
/// <reference path="../../ServiceClientCommon/MetadataDrivenDialogConstants.ts" />
/// <reference path="../../ServiceClientCommon/EntitlementState.ts" />
/// <reference path="../../ServiceClientCommon/DataContracts/Action/ODataUpdateRequest.ts" />
/// <reference path="../../ServiceClientCommon/DataContracts/Action/RenewEntitlementRequest.ts" />
var CrmService;
(function (CrmService) {
    'use strict';
    var ODataUpdateRequest = ODataContract.ODataUpdateRequest;
    var EntitlementGridCommandActions = (function () {
        function EntitlementGridCommandActions() {
            var _this = this;
            this._status = -1;
            this.openEntitlementTemplate = function () {
                var callBackAfterSelectEntitlementTemplate = _this.openEntitlementCreatePageAfterSelectEntitlementTemplate;
                var options = { width: 650, height: 300, position: 1 /* center */ };
                var dialogParams = {};
                Xrm.Navigation.openDialog(CrmService.DialogName.SelectEntitlementTemplate, options, dialogParams).then(callBackAfterSelectEntitlementTemplate);
            };
            //callback function of 'select entitlement template' MDD dialog
            this.openEntitlementCreatePageAfterSelectEntitlementTemplate = function (result) {
                var lastButtonClicked = result.parameters[CrmService.MetadataDrivenDialogConstants.ParamLastButtonClicked];
                var entitlementTemplateId = result.parameters[CrmService.MetadataDrivenDialogConstants.ParamEntityId];
                var entitlementTemplateEntityType = result.parameters[CrmService.MetadataDrivenDialogConstants.ParamEntityType];
                if (ClientUtility.DataUtil.isNullOrUndefined(lastButtonClicked)
                    || lastButtonClicked.toString() !== CrmService.MetadataDrivenDialogConstants.DialogOkId
                    || ClientUtility.DataUtil.isNullOrUndefined(entitlementTemplateId)
                    || ClientUtility.DataUtil.isNullOrUndefined(entitlementTemplateEntityType)) {
                    return;
                }
                var entityOption = {
                    entityName: CrmService.EntityNames.Entitlement,
                    useQuickCreateForm: false,
                    createFromEntity: {
                        id: entitlementTemplateId,
                        entityType: CrmService.EntityNames.EntitlementTemplate
                    }
                };
                var parameters = {};
                Xrm.Navigation.openForm(entityOption, parameters);
            };
            //executes when the 'select' button on 'select entitlement template' MDD dialog gets clicked
            this.selectEntitlementTemplateApplyChangesClick = function () {
                var entitlementTemplateIdValue = "";
                var entitlementTemplateId = Xrm.Page.data.attributes.get(CrmService.MetadataDrivenDialogConstants.EntitlementTemplateLookup);
                if (!ClientUtility.DataUtil.isNullOrUndefined(entitlementTemplateId) && !ClientUtility.DataUtil.isNullOrEmptyString(entitlementTemplateId.getValue())) {
                    entitlementTemplateIdValue = (entitlementTemplateId.getValue())[0].id;
                    ClientUtility.DialogUtil.setAttributeValue(CrmService.MetadataDrivenDialogConstants.ParamEntityType, CrmService.EntityTypeCodes.EntitlementTemplate);
                    ClientUtility.DialogUtil.setAttributeValue(CrmService.MetadataDrivenDialogConstants.ParamEntityId, entitlementTemplateIdValue);
                    ClientUtility.DialogUtil.setLastButtonClicked(CrmService.MetadataDrivenDialogConstants.DialogOkId);
                    Xrm.Page.ui.close();
                }
            };
            // Opens the cancel entitlement MDD dialog from the grid page
            this.Cancel = function (gridControl, selectedRecords) {
                if (!selectedRecords) {
                    return;
                }
                var maxLimit = Number(CrmService.ResourceStringProvider.getResourceString("LOCID_ENTITLEMENT_CANCEL_TH_LT"));
                if (selectedRecords.length > maxLimit) {
                    Xrm.Navigation.openAlertDialog({ text: ClientUtility.StringUtil.format(CrmService.ResourceStringProvider.getResourceString("LOCID_ENTITLEMENT_CANCEL_TH_MSG"), maxLimit.toString()) }, null);
                    return;
                }
                ...
                var options = { width: 520, height: 230, position: 1 /* center */ };
                var dialogParams = {};
                dialogParams[CrmService.MetadataDrivenDialogConstants.ParamEntityRecords] = ids;
                var callbackParams = {};
                callbackParams["gridControl"] = gridControl;
                Xrm.Navigation.openDialog(CrmService.DialogName.CancelEntitlement, options, dialogParams).then(function (dialogResult) {
                    _this.performActionAfterCancelEntitlement(dialogResult, callbackParams);
                });
            };
            // Callback that is executed when the cancel entitlement dialog is closed 
            this.performActionAfterCancelEntitlement = function (returnValue, params) {
                if (!ClientUtility.DataUtil.isNullOrUndefined(returnValue)) {
                    var entityIds = returnValue.parameters[CrmService.MetadataDrivenDialogConstants.ParamEntityRecords];
                    var lastButtonClicked = returnValue.parameters[ClientUtility.MetadataDrivenDialogConstants.paramLastButtonClicked];
                    if (lastButtonClicked === ClientUtility.MetadataDrivenDialogConstants.DialogOkId && entityIds.length > 0) {
                        var errorCount = 0;
                        var returnId = void 0;
                        var updateEntity = {};
                        var requestsToExecute = entityIds.map(function (entityId) {
                            updateEntity["statecode"] = CrmService.EntitlementState.cancelled;
                            updateEntity["statuscode"] = CrmService.EntitlementState.cancelled;
                            var currentrequest = new ODataUpdateRequest(CrmService.EntityNames.Entitlement, entityId.replace(/[{}]/g, ''), updateEntity);
                            return currentrequest;
                        });
                        Xrm.WebApi.online.executeMultiple(requestsToExecute).then(function (success) {
                            function refreshPage(refreshGrid, refreshForm) {
                                if (refreshGrid) {
                                    params.gridControl.refresh();
                                }
                                if (refreshForm) {
                                    // This code path for cancelling from the entity form
                                    Xrm.Page.ui.close();
                                    Xrm.Page.data.refresh(true);
                                }
                            }
                            var refreshGrid = !ClientUtility.DataUtil.isNullOrUndefined(params) && !ClientUtility.DataUtil.isNullOrUndefined(params.gridControl);
                            var refreshForm = !ClientUtility.DataUtil.isNullOrUndefined(Xrm.Page.data);
                            var errorCode = "";
                            var errorCount = 0;
                            for (var index = 0; index < success.length; index++) {
                                if (success[index].ok === true) {
                                    continue;
                                }
                                else {
                                    errorCount++;
                                    errorCode = success[index].json()._settledValue.error.code;
                                }
                            }
                            if (errorCount == 0) {
                                refreshPage(refreshGrid, refreshForm);
                            }
                            else if (errorCount == 1) {
                                var errorCodeValue = parseInt(errorCode);
                                if (errorCodeValue == 0x80060607 || errorCodeValue == 0x80044208) {
                                    Xrm.Navigation.openErrorDialog({ errorCode: errorCodeValue });
                                }
                                else {
                                    Xrm.Navigation.openAlertDialog({ text: CrmService.ResourceStringProvider.getResourceString("Error_Message_Action_MultipleErrorsFound") });
                                    refreshPage(refreshGrid, false);
                                }
                            }
                            else {
                                Xrm.Navigation.openAlertDialog({ text: CrmService.ResourceStringProvider.getResourceString("Error_Message_Action_MultipleErrorsFound") });
            ...
            this.renewEntitlement = function (entitlementId) {
                if (ClientUtility.DataUtil.isNullOrUndefined(entitlementId) || !ClientUtility.DataUtil.isString(entitlementId)) {
                    return;
                }
                var entitlementRecordId = entitlementId.replace(/[{}]/g, '');
                var entitlement = {
                    id: entitlementRecordId,
                    entityType: CrmService.EntityNames.Entitlement
                };
                var request = new ODataContract.RenewEntitlementRequest(entitlement, _this._status);
                Xrm.WebApi.online.execute(request).then(function (renewEntitlementResponse) {
                    if (renewEntitlementResponse.ok === true) {
                        renewEntitlementResponse.json().then(function (jsonResponse) {
                            Xrm.Utility.openEntityForm(CrmService.EntityNames.Entitlement, jsonResponse.entitlementid);
                        });
                    }
                    else {
                        var errorCodeValue = renewEntitlementResponse.json()._settledValue.error.code;
                        Xrm.Navigation.openErrorDialog({ errorCode: errorCodeValue });
                    }
                }, ClientUtility.ActionFailedHandler.actionFailedCallback);
            };
        }
        return EntitlementGridCommandActions;
    }());
    CrmService.EntitlementGridCommandActions = EntitlementGridCommandActions;
})(CrmService || (CrmService = {}));
/**
* @license Copyright (c) Microsoft Corporation. All rights reserved.
*/
...
(function (CrmService) {
    'use strict';
    var ODataUpdateRequest = ODataContract.ODataUpdateRequest;
    var EntitlementCommandActions = (function () {
        function EntitlementCommandActions(entitlementGridCommandActions) {
            var _this = this;
            this.EntitlementGridCommandActions = null;
            this.odataCustomerIdAttributeName = "_customerid_value";
            this.emptyGuid = "{00000000-0000-0000-0000-000000000000}";
            // Opens the Renew Entitlement MDD Dialog
            this.RenewEntitlement = function () {
                var options = { width: 450, height: 200, position: 1 /* center */ };
                var dialogParams = {};
                Xrm.Navigation.openDialog(CrmService.DialogName.RenewEntitlement, options, dialogParams).then(_this.performActionAfterRenewEntitlement);
            };
            // Sets localized labels when the Renew Entitlement MDD dialog loads
            this.onLoadRenewEntitlement = function () {
                var titleControl = Xrm.Page.getControl("lbl_renewdialog");
                var descriptionControl = Xrm.Page.getControl("lbl_renewdialogdescription");
                var okControl = Xrm.Page.getControl(CrmService.MetadataDrivenDialogConstants.DialogOkId);
                var cancelControl = Xrm.Page.getControl(CrmService.MetadataDrivenDialogConstants.DialogCancelId);
                if (!ClientUtility.DataUtil.isNullOrUndefined(titleControl)) {
                    titleControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Dialog_RenewEntitlement_Title"));
                }
                if (!ClientUtility.DataUtil.isNullOrUndefined(descriptionControl)) {
                    descriptionControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Dialog_RenewEntitlement_Description"));
                }
                if (!ClientUtility.DataUtil.isNullOrUndefined(okControl)) {
                    okControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Button_Label_Renew"));
                }
                if (!ClientUtility.DataUtil.isNullOrUndefined(cancelControl)) {
                    cancelControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Button_Label_Cancel"));
                }
            };
            // Executes when the "confirm" button on the Renew Entitlement MDD Dialog is clicked
            this.renewEntitlementDialogConfirmClick = function () {
                ClientUtility.DialogUtil.setLastButtonClicked(CrmService.MetadataDrivenDialogConstants.DialogOkId);
                Xrm.Page.ui.close();
            };
            // Executes when the MDD dialog is closed, either via "confirm", or "cancel"
            this.performActionAfterRenewEntitlement = function (returnValue) {
                if (!ClientUtility.DataUtil.isNullOrUndefined(returnValue) &&
                    returnValue.parameters[ClientUtility.MetadataDrivenDialogConstants.paramLastButtonClicked] === ClientUtility.MetadataDrivenDialogConstants.DialogOkId) {
                    _this.EntitlementGridCommandActions.renewEntitlement(Xrm.Page.data.entity.getId());
                }
            };
            // Opens the Cancel Entitlement MDD Dialog
            this.cancel = function () {
                var options = { width: 410, height: 200, position: 1 /* center */ };
                var dialogParams = {};
                dialogParams[CrmService.MetadataDrivenDialogConstants.ParamEntityRecords] = [Xrm.Page.data.entity.getId()];
                Xrm.Navigation.openDialog(CrmService.DialogName.CancelEntitlement, options, dialogParams).then(function (dialogResult) {
                    _this.EntitlementGridCommandActions.performActionAfterCancelEntitlement(dialogResult, null);
                });
            };
            // Sets localized labels for Cancel Entitlement MDD. Sets description appropriately for if there are 1
            // or multiple entitlements selected to be cancelled.
            this.onLoadCancelEntitlement = function (foo, bar) {
                var titleControl = Xrm.Page.getControl(CrmService.MetadataDrivenDialogConstants.CancelEntitlementTitleId);
                var descriptionControl = Xrm.Page.getControl(CrmService.MetadataDrivenDialogConstants.CancelEntitlementDescriptionId);
                var okControl = Xrm.Page.getControl(CrmService.MetadataDrivenDialogConstants.DialogOkId);
                var cancelControl = Xrm.Page.getControl(CrmService.MetadataDrivenDialogConstants.DialogCancelId);
                if (!ClientUtility.DataUtil.isNullOrUndefined(titleControl)) {
                    titleControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Web.CS.cases.dlg_cancel.aspx_20"));
                }
                if (!ClientUtility.DataUtil.isNullOrUndefined(descriptionControl)) {
                    var entityIdsAttribute = Xrm.Page.data.attributes.get(CrmService.MetadataDrivenDialogConstants.ParamEntityRecords);
                    if (!ClientUtility.DataUtil.isNullOrUndefined(entityIdsAttribute)) {
                        var entityIds = entityIdsAttribute.getValue();
                        if (!ClientUtility.DataUtil.isNullOrUndefined(entityIds)) {
                            if (entityIds.length === 1) {
                                descriptionControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Dialog_Single_Cancel_Description_Entitlement"));
                            }
                            else {
                                var label = CrmService.ResourceStringProvider.getResourceString("Dialog_Cancel_Description_Entitlement");
                                label = ClientUtility.StringUtil.format(label, entityIds.length);
                                descriptionControl.setLabel(label);
                            }
                        }
                    }
                }
                if (!ClientUtility.DataUtil.isNullOrUndefined(okControl)) {
                    okControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Button_Label_Confirm"));
                }
                if (!ClientUtility.DataUtil.isNullOrUndefined(cancelControl)) {
                    cancelControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Button_Label_Cancel"));
                }
            };
            // Executes when the confirm button is clicked on the cancel entitlement MDD
            this.cancelEntitlementDialogConfirmClick = function () {
                ClientUtility.DialogUtil.setLastButtonClicked(CrmService.MetadataDrivenDialogConstants.DialogOkId);
                Xrm.Page.ui.close();
            };
            this.isEntitlementDefault = function (entitlementId) {
                var isDefault = false;
                var isDefaultAttribute = Xrm.Page.getAttribute("isdefault");
                if (!ClientUtility.DataUtil.isNull(isDefaultAttribute)) {
                    isDefault = isDefaultAttribute.getValue();
                    return isDefault;
                }
                var columnNames = ClientUtility.ODataUtil.getSelectOption(["isdefault"]);
                Xrm.WebApi.retrieveRecord(CrmService.EntityNames.Entitlement, entitlementId, columnNames).then(function (entitlementRecord) {
                    isDefault = entitlementRecord.isdefault;
                }, ClientUtility.ActionFailedHandler.actionFailedCallback);
                return isDefault;
            };
            this.setDefaultEntitlement = function (entityId, entityName) {
                _this.updateIfEntitlementActive(entityId, entityName, ["statecode", _this.odataCustomerIdAttributeName]);
            };
            this.updateIfEntitlementActive = function (entitlementId, entityName, columnName) {
                var columnNames = ClientUtility.ODataUtil.getSelectOption(columnName);
                Xrm.WebApi.retrieveRecord(CrmService.EntityNames.Entitlement, entitlementId, columnNames).then(function (entitlementRecord) {
                    var stateCode = entitlementRecord.statecode;
                    if (stateCode === CrmService.EntitlementState.active) {
                        _this.updateEntitlementIfNoDefaultEntitlementExists(entitlementId, entityName, entitlementRecord[_this.odataCustomerIdAttributeName]);
                    }
                    else {
                        Xrm.Navigation.openAlertDialog({ text: CrmService.ResourceStringProvider.getResourceString("LOCID_ENT_DEFAULT_INACTIVE_MSG") }, null);
                    }
                }, ClientUtility.ActionFailedHandler.actionFailedCallback);
            };
            this.updateEntitlementIfNoDefaultEntitlementExists = function (entitlementId, entityName, customerId) {
                var fetchXml = ClientUtility.StringUtil.format("?fetchXml=<fetch version='1.0' mapping='logical'><entity name='entitlement'><attribute name='entitlementid' /><filter type='and'><condition attribute='isdefault' operator='eq' value='1' /></filter><filter type='and'><condition attribute='customerid' operator='eq' value='{0}'/></filter></entity></fetch>", customerId.toString());
                var previousDefaultEntitlementId = _this.emptyGuid;
                var doesDefaultEntitlementExist = false;
                Xrm.WebApi.retrieveMultipleRecords(CrmService.EntityNames.Entitlement, fetchXml).then(function (retrieveEntitlementResponse) {
                    if (retrieveEntitlementResponse) {
                        var resolutionDescription = "";
                        if (!ClientUtility.DataUtil.isNullOrUndefined(retrieveEntitlementResponse) && !ClientUtility.DataUtil.isNullOrUndefined(retrieveEntitlementResponse.entities) && retrieveEntitlementResponse.entities.length > 0) {
                            previousDefaultEntitlementId = retrieveEntitlementResponse.entities[0].entitlementid;
                            doesDefaultEntitlementExist = true;
                        }
                        _this.updateDefaultEntitlementByForm(entitlementId, previousDefaultEntitlementId, doesDefaultEntitlementExist);
                    }
                }, ClientUtility.ActionFailedHandler.actionFailedCallback);
            };
            this.updateDefaultEntitlementByForm = function (entitlementId, previousDefaultEntitlementId, doesDefaultEntitlementExist) {
                var width = parseInt(CrmService.ResourceStringProvider.getResourceString("LOCID_ENT_DEFAULT_DIALOG_WIDTH"));
                var height = parseInt(CrmService.ResourceStringProvider.getResourceString("LOCID_ENT_DEFAULT_DIALOG_HEIGHT"));
                var options = { width: width, height: height, position: 1 /* center */ };
                var dialogParams = {};
                dialogParams[CrmService.MetadataDrivenDialogConstants.PreviousDefaultEntitlementId] = previousDefaultEntitlementId;
                dialogParams[CrmService.MetadataDrivenDialogConstants.CurrentDefaultEntitlementId] = entitlementId;
                dialogParams[CrmService.MetadataDrivenDialogConstants.DoesDefaultEntitlementExist] = doesDefaultEntitlementExist;
                Xrm.Navigation.openDialog(CrmService.DialogName.SetDefaultEntitlement, options, dialogParams).then(_this.performActionAfterSetDefaultEntitlement);
            };
            this.onLoadSetDefaultEntitlement = function () {
                var titleControl = Xrm.Page.getControl("lbl_setdefaultdialog");
                var descriptionControl = Xrm.Page.getControl("lbl_setdefaultdescription");
                var okControl = Xrm.Page.getControl(CrmService.MetadataDrivenDialogConstants.DialogOkId);
                var cancelControl = Xrm.Page.getControl(CrmService.MetadataDrivenDialogConstants.DialogCancelId);
                if (!ClientUtility.DataUtil.isNullOrUndefined(titleControl)) {
                    titleControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Entitlement_Default_Confirmation_DialogTitle"));
                }
                if (!ClientUtility.DataUtil.isNullOrUndefined(descriptionControl)) {
                    var doesDefaultEntitlementExistAttribute = Xrm.Page.data.attributes.get(CrmService.MetadataDrivenDialogConstants.DoesDefaultEntitlementExist);
                    var doesDefaultEntitlementExist = !ClientUtility.DataUtil.isNullOrUndefined(doesDefaultEntitlementExistAttribute) ? doesDefaultEntitlementExistAttribute.getValue() : false;
                    if (!ClientUtility.DataUtil.isNull(doesDefaultEntitlementExist) && doesDefaultEntitlementExist) {
                        descriptionControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Entitlement_Default_Change_Confirmation_Message"));
                    }
                    else {
                        descriptionControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Entitlement_Default_Confirmation_Message"));
                    }
                }
                if (!ClientUtility.DataUtil.isNullOrUndefined(okControl)) {
                    okControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Button_Label_Confirm"));
            ...
            this.setDefaultEntitlementDialogConfirmClick = function () {
                ClientUtility.DialogUtil.setLastButtonClicked(CrmService.MetadataDrivenDialogConstants.DialogOkId);
                Xrm.Page.ui.close();
            };
            this.performActionAfterSetDefaultEntitlement = function (returnValue) {
                if (!ClientUtility.DataUtil.isNullOrUndefined(returnValue) &&
                    returnValue.parameters[ClientUtility.MetadataDrivenDialogConstants.paramLastButtonClicked] === ClientUtility.MetadataDrivenDialogConstants.DialogOkId) {
                    var previousDefaultEntitlementId = returnValue.parameters[CrmService.MetadataDrivenDialogConstants.PreviousDefaultEntitlementId];
                    var currentDefaultEntitlementId = returnValue.parameters[CrmService.MetadataDrivenDialogConstants.CurrentDefaultEntitlementId];
                    var requestsToExecute = [];
                    if (!ClientUtility.DataUtil.isNullOrUndefined(previousDefaultEntitlementId) && previousDefaultEntitlementId !== _this.emptyGuid) {
                        var updateEntitlement = {};
                        updateEntitlement["isdefault"] = false;
                        var currentrequest = new ODataUpdateRequest(CrmService.EntityNames.Entitlement, previousDefaultEntitlementId.replace(/[{}]/g, ''), updateEntitlement);
                        requestsToExecute.push(currentrequest);
                    }
                    if (!ClientUtility.DataUtil.isNullOrUndefined(currentDefaultEntitlementId)) {
                        var updateEntitlement = {};
                        updateEntitlement["isdefault"] = true;
                        var currentrequest = new ODataUpdateRequest(CrmService.EntityNames.Entitlement, currentDefaultEntitlementId.replace(/[{}]/g, ''), updateEntitlement);
                        requestsToExecute.push(currentrequest);
                    }
                    Xrm.WebApi.online.executeMultiple(requestsToExecute).then(function (response) {
            ...
            this.deactivatePrimaryRecord = function (entityId, entityName) {
                XrmCore.Commands.Deactivate.deactivatePrimaryRecord(entityId, entityName);
                //open the entity form again to reload with fresh status
            ...
            this.EntitlementGridCommandActions = entitlementGridCommandActions;
        }
        ;
        return EntitlementCommandActions;
    }());
    CrmService.EntitlementCommandActions = EntitlementCommandActions;
})(CrmService || (CrmService = {}));
/**
* @license Copyright (c) Microsoft Corporation. All rights reserved.
*/
/// <reference path="../../../../../TypeDefinitions/CRM/ClientUtility.d.ts" />
/// <reference path="../../ServiceClientCommon/Service_ClientCommon.ts" />
/// <reference path="./EntitlementGridCommandActions.ts" />
/// <reference path="./EntitlementCommandActions.ts" />
/// <reference path="../../../../../TypeDefinitions/AppCommon/Telemetry/TelemetryLibrary.d.ts" />
var CrmService;
(function (CrmService) {
    'use strict';
    var EntitlementLibrary = (function () {
        function EntitlementLibrary() {
            var entitlementMainSystemLibraryWebResource = new EntitlementMainSystemLibraryWebResource();
            var global = window;
            var mscrm = global.Mscrm;
            mscrm.EntitlementMainSystemLibraryWebResource = entitlementMainSystemLibraryWebResource;
            mscrm.Form_onload = entitlementMainSystemLibraryWebResource.form_onload;
            mscrm.allocationType_onchange = entitlementMainSystemLibraryWebResource.allocationType_onchange;
            mscrm.customerid_onchange = entitlementMainSystemLibraryWebResource.customerid_onchange;
            mscrm.entityType_onchange = entitlementMainSystemLibraryWebResource.entityType_onchange;
        }
        ;
        return EntitlementLibrary;
    }());
    CrmService.EntitlementLibrary = EntitlementLibrary;
    var EntitlementMainSystemLibraryWebResource = (function () {
        function EntitlementMainSystemLibraryWebResource() {
            var _this = this;
            this.mapping = [];
            this.manageEntitlementStateWorkFlowId = "5DD3DC0D-0BA9-49C6-950B-54F1EDC4A709";
            this.manageEntitlementStateWorkFlowName = "ManageEntitlementState";
            this.form_onload = function () {
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "EntitlementLibrary", "form_onload::initiateFormLoad", Xrm.Page.data.entity.getId());
                EntitlementMainSystemLibraryWebResource.setEntitlementTermsGridVisibility();
                var formType = Xrm.Page.ui.getFormType();
                if (formType !== 4 /* Disabled */) {
                    _this.allocationType_onchange();
                }
                if (!ClientUtility.ClientUtil.isMobile()) {
                    var attribute = Xrm.Page.getAttribute("remainingterms");
                    !ClientUtility.DataUtil.isNullOrUndefined(attribute.controls) && attribute.controls.forEach(function (item, index) {
                        var dataControl = item;
                        dataControl.setDisabled(true);
                    });
                }
                ;
                // Fire async call and filtering if case is not the only entityType option
                var attrAllocationTypeCode = Xrm.Page.getAttribute("allocationtypecode");
                var attrEntityType = Xrm.Page.getAttribute("entitytype");
                var ctrlEntityType = Xrm.Page.getControl("entitytype");
                if (!ClientUtility.DataUtil.isNullOrUndefined(attrAllocationTypeCode) && !ClientUtility.DataUtil.isNullOrUndefined(attrEntityType)) {
                    var options = attrEntityType.getOptions();
                    if (options.length > 1) {
                        var entityType = attrEntityType.getValue();
                        var selectedAllocationTypeCode = attrAllocationTypeCode.getValue();
                        if (_this.mapping.length == 0) {
                            // fires only on first onload
                            _this.loadMapping(function () {
                                ctrlEntityType.setVisible(true);
                                attrEntityType.addOnChange(_this.entityType_onchange);
                                _this.filterAllocationType(entityType, selectedAllocationTypeCode);
                            });
                        }
                        else {
                            _this.filterAllocationType(entityType, selectedAllocationTypeCode);
                        }
                        // Entity type cannot be changed after record creation
                        if (Xrm.Page.ui.getFormType() != 1 /* Create */) {
                            ctrlEntityType.setDisabled(true);
                        }
                    }
                }
                // Set allocationtypecode to Disable when form opened for update
                if (Xrm.Page.ui.getFormType() != 1 /* Create */) {
                    var ctrlAllocationType = Xrm.Page.getControl("allocationtypecode");
                    if (!ClientUtility.DataUtil.isNullOrUndefined(ctrlAllocationType)) {
                        ctrlAllocationType.setDisabled(true);
                    }
                }
                // Show form notification if ManageEntitlementState workflow deactivated
                _this.showNotificationIfEntitlementWorkflowDeactivated();
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "EntitlementLibrary", "form_onload::initiateFormLoad", Xrm.Page.data.entity.getId());
            };
            this.entityType_onchange = function () {
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "EntitlementLibrary", "entityType_onchange::initiateEntityChange", Xrm.Page.data.entity.getId());
                var entityType = Xrm.Page.getAttribute("entitytype").getValue();
                _this.filterAllocationType(entityType);
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "EntitlementLibrary", "entityType_onchange::initiateEntityChange", Xrm.Page.data.entity.getId());
            };
            /* Checking ManageEntitlementState workflow state and showing notification if deactivated */
            this.showNotificationIfEntitlementWorkflowDeactivated = function () {
                var self = _this;
                var selectQuery = "?$select=statecode,name&$filter=statecode eq 1 and workflowid eq " + _this.manageEntitlementStateWorkFlowId;
                Xrm.WebApi.retrieveMultipleRecords("workflow", selectQuery).then(function (result) {
                    if (result.entities.length == 0) {
                        var notificationMessage = CrmService.ResourceStringProvider.getResourceString("EntitlementWorkflowNotInActiveState");
                        notificationMessage = ClientUtility.StringUtil.format(notificationMessage, self.manageEntitlementStateWorkFlowName);
                        Xrm.Page.ui.setFormNotification(notificationMessage, "WARNING", "EntitlementWorkflowNotInActiveState");
                    }
                });
            };
            /* Load the table mapping allocation type to entity type
              * @param postLoad callback to fire after mapping has been populated
              */
            this.loadMapping = function (postLoad) {
                var self = _this;
                Xrm.WebApi.retrieveMultipleRecords("entitlemententityallocationtypemapping", "?$select=allocationtype,entitytype,statecode").then(function (result) {
                    if (!ClientUtility.DataUtil.isNullOrUndefined(result) && !ClientUtility.DataUtil.isNullOrUndefined(result.entities)) {
                        if (result.entities.length == 0) {
                            var notificationMessage = CrmService.ResourceStringProvider.getResourceString("Entitlement_AllocationTypeNotification");
                            notificationMessage = ClientUtility.StringUtil.format(notificationMessage, "https://go.microsoft.com/fwlink/?linkid=2220703");
                            Xrm.Page.ui.setFormNotification(notificationMessage, "ERROR", "Entitlement_AllocationTypeNotification");
                        }
                        for (var i = 0; i < result.entities.length; i++) {
                            var pair = result.entities[i];
                            if (pair.statecode == 0) {
                                self.mapping.push({
                                    allocationType: pair.allocationtype,
                                    entityType: pair.entitytype
                                });
                            }
                        }
                        if (!ClientUtility.DataUtil.isNullOrUndefined(postLoad)) {
                            postLoad();
                        }
                    }
                });
            };
            /* Filter allocationtype options on entilement entity
             * requires mapping to be loaded before function call
             * @param entity enumeration value for the entity type selected
             * @param selected allocation type that has been selected if it should be preseverved
             */
            this.filterAllocationType = function (entityType, selectedAllocationTypeCode) {
                var attrAllocationTypeCode = Xrm.Page.getAttribute("allocationtypecode");
                if (!ClientUtility.DataUtil.isNullOrUndefined(attrAllocationTypeCode)) {
                    Xrm.Page.getControl("allocationtypecode").clearOptions();
                    var firstEligible = true;
                    for (var i = 0; i < _this.mapping.length; i++) {
                        if (_this.mapping[i].entityType == entityType) {
                            var option = attrAllocationTypeCode.getOption(_this.mapping[i].allocationType);
                            if (!ClientUtility.DataUtil.isNullOrUndefined(option)) {
                                Xrm.Page.getControl("allocationtypecode").addOption(option);
                                // make the value set match the value displayed
                                if (firstEligible) {
                                    attrAllocationTypeCode.setValue(option.value);
                                    firstEligible = false;
                                }
                            }
                        }
                    }
                    if (!ClientUtility.DataUtil.isNullOrUndefined(selectedAllocationTypeCode)) {
                        attrAllocationTypeCode.setValue(selectedAllocationTypeCode);
                    }
                }
            };
            this.allocationType_onchange = function () {
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "EntitlementLibrary", "allocationType_onchange::initiateAllocationChange", Xrm.Page.data.entity.getId());
                var allocationtypecode = Xrm.Page.data.entity.attributes.get("allocationtypecode");
                var decreaseRemainingOn = Xrm.Page.data.entity.attributes.get("decreaseremainingon");
                var decreaseControl = Xrm.Page.ui.controls.get("decreaseremainingon");
                if (!(ClientUtility.DataUtil.isNullOrUndefined(allocationtypecode) || ClientUtility.DataUtil.isNullOrUndefined(decreaseRemainingOn) || ClientUtility.DataUtil.isNullOrUndefined(decreaseControl))) {
                    var options = decreaseRemainingOn.getOptions();
                    if (!ClientUtility.DataUtil.isNullOrUndefined(allocationtypecode.getValue())) {
                        if (1 === allocationtypecode.getValue()) {
                            decreaseRemainingOn.setValue(options[0].value);
                            decreaseControl.setDisabled(true);
                            decreaseRemainingOn.setSubmitMode("always");
                        }
                        else {
                            // In UCI, after activating the entitlement, decreaseRemainingOn control is not getting disabled. In the form_onload itself decreaseControl is getting enabled.
                            // checking control state, if it is disabled enabling the control
                            if (decreaseControl.getDisabled()) {
                                decreaseControl.setDisabled(false);
                            }
                            decreaseRemainingOn.setSubmitMode("dirty");
                        }
                    }
                }
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "EntitlementLibrary", "allocationType_onchange::initiateAllocationChange", Xrm.Page.data.entity.getId());
            };
            this.customerid_onchange = function () {
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostStartMarker("ServiceWebResources", "EntitlementLibrary", "customerid_onchange::initiateCustomerIdChange", Xrm.Page.data.entity.getId());
                ...
                if (!ClientUtility.DataUtil.isNullOrUndefined(oRelatedLookup) && !ClientUtility.DataUtil.isNullOrUndefined(oRelatedLookup.getValue())) {
                    var lookup = Xrm.Page.ui.controls.get("lookup_grid_EntitlementContacts");
                    if (!ClientUtility.DataUtil.isNullOrUndefined(lookup)) {
                        var customerlookup = oRelatedLookup.getValue(), fetchXml = '<filter type="and"><condition attribute="parentcustomerid" operator="like" value="' + CrmEncodeDecode.CrmXmlAttributeEncode(customerlookup[0].id.toString()) + '"/></filter>';
                        lookup.addCustomFilter(fetchXml);
                        lookup.allowUserToDisableRelationshipFilter(false);
                    }
                }
                if (_this.AppCommonNullCheck())
                    AppCommon.TelemetryReporter.Instance().PostEndMarker("ServiceWebResources", "EntitlementLibrary", "customerid_onchange::initiateCustomerIdChange", Xrm.Page.data.entity.getId());
            };
        ...
        return EntitlementMainSystemLibraryWebResource;
    }());
    //Change the visibility of the Entitlement terms grid based on whether the client is web or UCI
    EntitlementMainSystemLibraryWebResource.setEntitlementTermsGridVisibility = function () {
        var generalTab = Xrm.Page.ui.tabs.get("general");
        var gridControlToBeHidden = "grid_EntitlementChannel";
        if (!ClientUtility.ClientUtil.isUCI()) {
            gridControlToBeHidden = "editableEntitlementChannelGridControl";
        }
        if (!ClientUtility.DataUtil.isNullOrUndefined(generalTab)) {
            for (var i = 0; i < generalTab.sections.getLength(); i++) {
                var section = generalTab.sections.get(i);
                if (section.controls.getLength() > 0) {
                    var gridControl = section.controls.get(gridControlToBeHidden);
    ...
    CrmService.EntitlementMainSystemLibraryWebResource = EntitlementMainSystemLibraryWebResource;
})(CrmService || (CrmService = {}));
/**
* @license Copyright (c) Microsoft Corporation. All rights reserved.
*/
...
(function (CrmService) {
    'use strict';
    var ServiceCommandBarActionsLegacyUtil = (function () {
        function ServiceCommandBarActionsLegacyUtil() {
        }
        return ServiceCommandBarActionsLegacyUtil;
    }());
    ServiceCommandBarActionsLegacyUtil.setStateRecord = function (newState, confirmDialogStrings, options) {
        var callbackFunction = Mscrm.InternalUtilities.GridUtilities.createCallbackFunctionFactory(ServiceCommandBarActionsLegacyUtil._setStateRecordCallback, [newState]);
        Xrm.Dialog.openConfirmDialog(confirmDialogStrings, options, callbackFunction, null);
    };
    ServiceCommandBarActionsLegacyUtil._setStateRecordCallback = function (result, newState) {
        ServiceCommandBarActionsLegacyUtil._setEntityState(Xrm.Page.data.entity.getEntityName(), Xrm.Page.data.entity.getId(), newState, -1).then(function (updateResponse) {
            Xrm.Page.data.refresh(true);
        }, ServiceCommandBarActionsLegacyUtil.displayErrorDialogAfterFailedAction);
    };
    ServiceCommandBarActionsLegacyUtil._setEntityState = function (entityType, id, state, status) {
        ...
        var entityReference = new Xrm.Objects.EntityReference(entityType, id);
        var record = new Microsoft.Crm.Client.Core.Storage.Common.ObjectModel.EntityRecord(entityReference, attributeValues, attributeTypes, {}, {}, new Microsoft.Crm.Client.Core.Storage.Common.ObjectModel.RelatedEntityCollection(new Array(0)));
        record.get_changedFieldNames().addRange(['statecode', 'statuscode']);
        return Xrm.Internal.messages.update(record);
    };
    ServiceCommandBarActionsLegacyUtil.setStateRecords = function (gridControl, records, state, confirmDialogStrings, options) {
        var callbackFunction = Mscrm.InternalUtilities.GridUtilities.createCallbackFunctionFactory(ServiceCommandBarActionsLegacyUtil._setStateRecordsCallback, [gridControl, records, state]);
        Xrm.Dialog.openConfirmDialog(confirmDialogStrings, options, callbackFunction, null);
    };
    ServiceCommandBarActionsLegacyUtil._setStateRecordsCallback = function (result, gridControl, records, state) {
        Mscrm.InternalUtilities.DialogUtility.showProgressMessage();
        if (records.length === 1) {
            var entityName = records[0].TypeName;
            var entityId = records[0].Id;
            ServiceCommandBarActionsLegacyUtil._setEntityState(entityName, entityId, state, -1)
                ...
                Mscrm.InternalUtilities.DialogUtility.hideProgressMessage();
                ServiceCommandBarActionsLegacyUtil.displayErrorDialogAfterFailedAction(errorResponse);
            });
        }
        else {
            ServiceCommandBarActionsLegacyUtil._performChainSetState(records, state, -1, gridControl, 0, true);
        }
    };
    ServiceCommandBarActionsLegacyUtil.displayErrorDialogAfterFailedAction = function (errorResponse) {
        if (!Mscrm.InternalUtilities.JSTypes.isNull(errorResponse)) {
            var errorOptions = {};
            var organizationServiceFault = errorResponse.get_organizationServiceFault();
            if (!Mscrm.InternalUtilities.JSTypes.isNull(organizationServiceFault)) {
                errorOptions.message = organizationServiceFault.get_message();
                errorOptions.errorCode = organizationServiceFault.get_errorCode();
            }
            Xrm.Navigation.openErrorDialog(errorOptions);
        }
    };
    ServiceCommandBarActionsLegacyUtil._performChainSetState = function (records, state, status, gridControl, currentIndex, setStateSuccess) {
        if (currentIndex >= records.length) {
            Mscrm.InternalUtilities.DialogUtility.hideProgressMessage();
            !setStateSuccess &&
                Mscrm.GridCommandActions.openAlertDialogForSetStateMultipleError(gridControl);
            gridControl.refresh();
        }
        else
            ServiceCommandBarActionsLegacyUtil._setEntityState(records[currentIndex].TypeName, records[currentIndex].Id, state, status)
                .then(function (successParameter) {
                ServiceCommandBarActionsLegacyUtil._performChainSetState(records, state, status, gridControl, currentIndex + 1, setStateSuccess);
            }, function (errorParameter) {
                ServiceCommandBarActionsLegacyUtil._performChainSetState(records, state, status, gridControl, currentIndex + 1, false);
            });
    };
    ServiceCommandBarActionsLegacyUtil.isRecordsLengthNullOrZero = function (records, displayErrorIfNullOrZero) {
        if (displayErrorIfNullOrZero === void 0) { displayErrorIfNullOrZero = true; }
        if (!Mscrm.InternalUtilities.JSTypes.isNull(records) && records.length) {
            return false;
        }
        if (displayErrorIfNullOrZero) {
    ...
    ServiceCommandBarActionsLegacyUtil.cloneDate = function (date) {
        if (!Mscrm.InternalUtilities.JSTypes.isNull(date)) {
            var cloneDate = new Date();
            cloneDate.setMinutes(cloneDate.getMinutes() + cloneDate.getTimezoneOffset() + Xrm.Page.context.getTimeZoneOffsetMinutes());
    ...
    ServiceCommandBarActionsLegacyUtil.isStartDateLessThanEndDate = function (oBeginDate, oEndDate, oSourceCtrl, oBeginDateValue, oEndDateValue) {
        if (!Mscrm.InternalUtilities.JSTypes.isNull(oBeginDate.getValue()) && !Mscrm.InternalUtilities.JSTypes.isNull(oEndDate.getValue()) && oBeginDate.getValue() > oEndDate.getValue()) {
            var EntityName = Xrm.Page.data.entity.getEntityName();
            switch (EntityName) {
                case CrmService.EntityNames.Contract:
                case CrmService.EntityNames.ContractDetail:
                    Xrm.Utility.alertDialog(CrmService.ResourceStringProvider.getResourceString("Web.cs.contracts.end_date_earlier_than_start_date"), null);
                    break;
                default:
                    Xrm.Utility.alertDialog(CrmService.ResourceStringProvider.getResourceString("LOCID_ENDDATE_LT_STARTDATE"), null);
                    break;
            }
            switch (oSourceCtrl) {
                case CrmService.DateType.ActiveOn:
                case CrmService.DateType.ActualStart:
                case CrmService.DateType.BeginDate:
                case CrmService.DateType.BillingStartOn:
                case CrmService.DateType.ProposedStart:
                case CrmService.DateType.ScheduledStart:
                case CrmService.DateType.ValidFromDate:
    ...
    CrmService.ServiceCommandBarActionsLegacyUtil = ServiceCommandBarActionsLegacyUtil;
})(CrmService || (CrmService = {}));
/**
* @license Copyright (c) Microsoft Corporation. All rights reserved.
*/
...
*/
/// <reference path="../../../../../TypeDefinitions/CRM/ClientUtility.d.ts" />
/// <reference path="../../CommandBarActions/Legacy/ServiceCommandBarActionsLegacyUtil.ts" />
/// <reference path="../../ServiceClientCommon/EntitlementState.ts" />
/// <reference path="../../ServiceClientCommon/Service_ClientCommon.ts" />
...
(function (CrmService) {
    'use strict';
    var EntitlementLegacyGridCommandActions = (function () {
        function EntitlementLegacyGridCommandActions() {
            var _this = this;
            this._status = -1;
            this.openEntitlementTemplate = function () {
                ...
                options.width = 650;
                options.height = 300;
                var callBackAfterSelectEntitlementTemplate = _this._openContractCreatePageAfterSelectContractTemplate$p;
                Xrm.Dialog.openDialog(CrmService.DialogName.SelectEntitlementTemplate, options, null, callBackAfterSelectEntitlementTemplate, null);
            };
            //callback function of 'select entitlement template' MDD dialog
            this._openContractCreatePageAfterSelectContractTemplate$p = function (result) {
                var lastButtonClicked = result[CrmService.MetadataDrivenDialogConstants.LastButtonClicked];
                var entitlementTemplateId = result[CrmService.MetadataDrivenDialogConstants.EntitlementTemplateId];
                var entitlementTemplateEntityType = result[CrmService.MetadataDrivenDialogConstants.EntitlementTemplateEntityType];
                if (Mscrm.InternalUtilities._Script.isNullOrUndefined(lastButtonClicked)
                    || lastButtonClicked.toString() !== CrmService.MetadataDrivenDialogConstants.DialogOkId
                    || Mscrm.InternalUtilities._Script.isNullOrUndefined(entitlementTemplateId)
                    || Mscrm.InternalUtilities._Script.isNullOrUndefined(entitlementTemplateEntityType)) {
                    return;
                }
                var parameters = {};
                parameters["_CreateFromId"] = entitlementTemplateId;
                parameters["_CreateFromType"] = entitlementTemplateEntityType;
                Xrm.Utility.openEntityForm(CrmService.EntityNames.Entitlement, null, parameters);
            };
            //executes when the 'select' button on 'select entitlement template' MDD dialog gets clicked
            this.selectEntitlementTemplateApplyChangesClick = function () {
                var entitlementTemplateId = CrmService.LegacyUtils.getIdFromLookupAttribute(CrmService.MetadataDrivenDialogConstants.EntitlementTemplateLookup);
                Mscrm.InternalUtilities.DialogUtility.setAttributeValue(CrmService.MetadataDrivenDialogConstants.EntitlementTemplateEntityType, CrmService.EntityTypeCodes.EntitlementTemplate);
                Mscrm.InternalUtilities.DialogUtility.setAttributeValue(CrmService.MetadataDrivenDialogConstants.EntitlementTemplateId, entitlementTemplateId);
                Mscrm.InternalUtilities.DialogUtility.setLastButtonClicked(CrmService.MetadataDrivenDialogConstants.DialogOkId);
                Xrm.Page.ui.close();
            };
            // Opens the cancel entitlement MDD dialog from the grid page
            this.Cancel = function (gridControl, selectedRecords) {
                ...
                var maxLimit = Number(CrmService.ResourceStringProvider.getResourceString("LOCID_ENTITLEMENT_CANCEL_TH_LT"));
                if (selectedRecords.length > maxLimit) {
                    Xrm.Utility.alertDialog(ClientUtility.StringUtil.format(CrmService.ResourceStringProvider.getResourceString("LOCID_ENTITLEMENT_CANCEL_TH_MSG"), maxLimit.toString()), null);
                    return;
                }
                var ids = new Array(selectedRecords.length);
                for (var i = 0; i < selectedRecords.length; i++) {
                ...
                options.height = 230;
                var dialogParams = {};
                dialogParams["entityIds"] = ids;
                var callbackParams = {};
                callbackParams["gridControl"] = gridControl;
                Xrm.Dialog.openDialog(CrmService.DialogName.CancelEntitlement, options, dialogParams, _this.performActionAfterCancelEntitlement, callbackParams);
            };
            // Callback that is executed when the cancel entitlement dialog is closed 
            this.performActionAfterCancelEntitlement = function (returnValue, params) {
                if (!ClientUtility.DataUtil.isNullOrUndefined(returnValue) &&
                    returnValue.lastButtonClicked === CrmService.MetadataDrivenDialogConstants.DialogOkId &&
                    returnValue.entityIds.length > 0) {
                    var entitlementAttributeValues = {
                        statecode: CrmService.EntitlementState.cancelled,
                        statuscode: CrmService.EntitlementState.cancelled
                    };
                    var entitlementAttributeTypes = {
                        statecode: Xrm.Objects.AttributeType.state,
                        statuscode: Xrm.Objects.AttributeType.status
                    };
                    var entitlementRecords = [];
                    for (var i = 0; i < returnValue.entityIds.length; i++) {
                        var entitlementEntityReference = new Xrm.Objects.EntityReference(CrmService.EntityNames.Entitlement, returnValue.entityIds[i]);
                        var entitlementRecord = new Microsoft.Crm.Client.Core.Storage.Common.ObjectModel.EntityRecord(entitlementEntityReference, entitlementAttributeValues, entitlementAttributeTypes, {}, {}, new Microsoft.Crm.Client.Core.Storage.Common.ObjectModel.RelatedEntityCollection(new Array(0)));
                        entitlementRecord.get_changedFieldNames().addRange(['statecode', 'statuscode']);
                        entitlementRecords.push(entitlementRecord);
                    }
                    Xrm.Internal.messages.updateMultiple(entitlementRecords).then(function (response) {
                        if (response.isFaulted) {
                            if (returnValue.entityIds.length === 1) {
                                // If only one entitlement is being cancelled and it fails to be
                                // cancelled, give a detailed error message.
                                Xrm.Internal.messages.retrieve("entitlement", returnValue.entityIds[0], ["statecode"]).then(function (response) {
                                    if (!ClientUtility.DataUtil.isNullOrUndefined(response) && !ClientUtility.DataUtil.isNullOrUndefined(response.entity)) {
                                        var entitlementFields = response.entity.getFields();
                                        if (!ClientUtility.DataUtil.isNullOrUndefined(entitlementFields) && !ClientUtility.DataUtil.isNullOrUndefined(entitlementFields.statecode)) {
                                            var state = entitlementFields.statecode.value;
                                            var errorOptions = {};
                                            if (state === CrmService.EntitlementState.draft ||
                                                state === CrmService.EntitlementState.expired) {
                                                errorOptions.message = CrmService.ResourceStringProvider.getResourceString("Error_Message_0x80060607");
                                                errorOptions.errorCode = 0x80060607;
                                                Xrm.Navigation.openErrorDialog(errorOptions);
                                                return;
                                            }
                                            else if (state === CrmService.EntitlementState.cancelled) {
                                                errorOptions.message = CrmService.ResourceStringProvider.getResourceString("Error_Message_0x80044208");
                                                errorOptions.errorCode = 0x80044208;
                                                Xrm.Navigation.openErrorDialog(errorOptions);
                                                return;
                                            }
                                        }
                                ...
                                // If multiple entitlements are being cancelled and one or more fails,
                                // give a general error message.
                                Xrm.Utility.alertDialog(CrmService.ResourceStringProvider.getResourceString("Error_Message_Action_MultipleErrorsFound"));
                            }
                        }
                        if (!ClientUtility.DataUtil.isNullOrUndefined(params) &&
                            !ClientUtility.DataUtil.isNullOrUndefined(params.gridControl)) {
                            // This code path for cancelling from the grid control
                            params.gridControl.refresh();
                        }
                        else if (!ClientUtility.DataUtil.isNullOrUndefined(Xrm.Page.data)) {
                            // This code path for cancelling from the entity form
                            Xrm.Page.ui.close();
                            Xrm.Page.data.refresh(true);
                        }
            ...
            this.renewEntitlement = function (entitlementId) {
                Xrm.Internal.messages.renewEntitlement(Microsoft.Crm.Client.Core.Framework.Guid.tryCreate(entitlementId), _this._status)
                    .then(function (response) {
                    var childEntity = response.entity, renewedEntitlementid = childEntity.get_identifier().Id.toString();
                    !Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString(renewedEntitlementid) && Xrm.Utility.openEntityForm(CrmService.EntityNames.Entitlement, renewedEntitlementid, null);
                }, function (resp) {
                    Xrm.Internal.openErrorDialog(resp.get_organizationServiceFault().get_errorCode(), resp.get_message());
                });
            };
            this.generateActivateDialogStrings = function (recordCount) {
                var entityDisplayName = Xrm.Internal.getEntityDisplayName(CrmService.EntityNames.Entitlement);
                var entitySetName = Xrm.Internal.getEntitySetName(CrmService.EntityNames.Entitlement);
                var entityName = (recordCount === 1) ? entityDisplayName : entitySetName;
                var confirmDialogStrings = new Xrm.ConfirmDialogStrings;
                confirmDialogStrings.title = ClientUtility.StringUtil.format(CrmService.ResourceStringProvider.getResourceString("Dialog_Activate_Title"), entityName);
                confirmDialogStrings.subtitle = ClientUtility.StringUtil.format(CrmService.ResourceStringProvider.getResourceString("Dialog_Activate_Description_Activate"), recordCount, entityName);
                confirmDialogStrings.text = ClientUtility.StringUtil.format(CrmService.ResourceStringProvider.getResourceString("Web._grid.cmds.dlg_activate.aspx_55"), entityName, CrmService.ResourceStringProvider.getResourceString("Dialog_Label_Active"));
                confirmDialogStrings.confirmButtonLabel = CrmService.ResourceStringProvider.getResourceString("Button_Label_Activate");
                confirmDialogStrings.cancelButtonLabel = CrmService.ResourceStringProvider.getResourceString("Button_Label_Cancel");
                return confirmDialogStrings;
            };
            this.generateDeactivateDialogStrings = function (recordCount) {
                var entityDisplayName = Xrm.Internal.getEntityDisplayName(CrmService.EntityNames.Entitlement);
                var entitySetName = Xrm.Internal.getEntitySetName(CrmService.EntityNames.Entitlement);
                var entityName = (recordCount === 1) ? entityDisplayName : entitySetName;
                var confirmDialogStrings = new Xrm.ConfirmDialogStrings;
                confirmDialogStrings.title = ClientUtility.StringUtil.format(CrmService.ResourceStringProvider.getResourceString("Dialog_Deactivate_Title"), entityName);
                confirmDialogStrings.subtitle = ClientUtility.StringUtil.format(CrmService.ResourceStringProvider.getResourceString("Dialog_Deactivate_Description"), recordCount, entityName);
                confirmDialogStrings.text = ClientUtility.StringUtil.format(CrmService.ResourceStringProvider.getResourceString("Web._grid.cmds.dlg_deactivate.aspx_55"), entityName);
                confirmDialogStrings.confirmButtonLabel = CrmService.ResourceStringProvider.getResourceString("Button_Label_Deactivate");
                confirmDialogStrings.cancelButtonLabel = CrmService.ResourceStringProvider.getResourceString("Button_Label_Cancel");
                return confirmDialogStrings;
            };
            // Called when "activate" is clicked on entitlement grid page
            this.activateRecords = function (gridControl, records, entityTypeCode) {
                if (CrmService.ServiceCommandBarActionsLegacyUtil.isRecordsLengthNullOrZero(records, true)) {
                    return;
                }
                var options = new Xrm.DialogOptions;
                options.width = 610;
                options.height = 200;
                var confirmDialogStrings = _this.generateActivateDialogStrings(records.length);
                CrmService.ServiceCommandBarActionsLegacyUtil.setStateRecords(gridControl, records, CrmService.MetadataDrivenDialogConstants.EntitlementState.active, confirmDialogStrings, options);
            };
            // Called when "deactivate" is clicked on entitlement grid page
            this.deactivateRecords = function (gridControl, records, entityTypeCode) {
                if (CrmService.ServiceCommandBarActionsLegacyUtil.isRecordsLengthNullOrZero(records, true)) {
                    return;
                }
                var options = new Xrm.DialogOptions;
                options.width = 610;
                options.height = 200;
                var confirmDialogStrings = _this.generateDeactivateDialogStrings(records.length);
                CrmService.ServiceCommandBarActionsLegacyUtil.setStateRecords(gridControl, records, CrmService.MetadataDrivenDialogConstants.EntitlementState.draft, confirmDialogStrings, options);
            };
        }
        return EntitlementLegacyGridCommandActions;
    }());
    CrmService.EntitlementLegacyGridCommandActions = EntitlementLegacyGridCommandActions;
})(CrmService || (CrmService = {}));
/**
* @license Copyright (c) Microsoft Corporation. All rights reserved.
*/
...
(function (CrmService) {
    'use strict';
    var EntitlementLegacyCommandActions = (function () {
        function EntitlementLegacyCommandActions(entitlementGridCommandActions) {
            var _this = this;
            this.EntitlementGridCommandActions = null;
            // Opens the Renew Entitlement MDD Dialog
            this.RenewEntitlement = function () {
                ...
                options.width = 450;
                var dialogParams = {};
                Xrm.Dialog.openDialog(CrmService.DialogName.RenewEntitlement, options, dialogParams, _this.performActionAfterRenewEntitlement, null);
            };
            // Sets localized labels when the Renew Entitlement MDD dialog loads
            this.onLoadRenewEntitlement = function () {
                var titleControl = Xrm.Page.getControl("lbl_renewdialog");
                var descriptionControl = Xrm.Page.getControl("lbl_renewdialogdescription");
                var okControl = Xrm.Page.getControl(CrmService.MetadataDrivenDialogConstants.DialogOkId);
                var cancelControl = Xrm.Page.getControl("cancel_id");
                if (!ClientUtility.DataUtil.isNullOrUndefined(titleControl)) {
                    titleControl.setLabel(CrmService.ResourceStringProvider.getResourceString("Dialog_RenewEntitlement_Title"));
                }
                if (!ClientUtility.DataUtil.isNullOrUndefined(descriptionControl)) {
            ...
            // Executes when the MDD dialog is closed, either via "confirm", or "cancel"
            this.performActionAfterRenewEntitlement = function (returnValue) {
                if (!ClientUtility.DataUtil.isNullOrUndefined(returnValue) && returnValue.lastButtonClicked === "ok_id") {
                    _this.EntitlementGridCommandActions.renewEntitlement(Xrm.Page.data.entity.getId());
                }
            };
            // Opens the Cancel Entitlement MDD Dialog
            this.cancel = function () {
                ...
                options.height = 200;
                var dialogParams = {};
                dialogParams["entityIds"] = [Xrm.Page.data.entity.getId()];
                Xrm.Dialog.openDialog(CrmService.DialogName.CancelEntitlement, options, dialogParams, _this.EntitlementGridCommandActions.performActionAfterCancelEntitlement, null);
            };
            // Sets localized labels for Cancel Entitlement MDD. Sets description appropriately for if there are 1
            // or multiple entitlements selected to be cancelled.
            this.onLoadCancelEntitlement = function (foo, bar) {
                ...
                if (!ClientUtility.DataUtil.isNullOrUndefined(descriptionControl)) {
                    var entityIdsAttribute = Xrm.Page.getAttribute("entityIds");
                    if (!ClientUtility.DataUtil.isNullOrUndefined(entityIdsAttribute)) {
                        var entityIds = entityIdsAttribute.getValue();
                        if (!ClientUtility.DataUtil.isNullOrUndefined(entityIds)) {
                            if (entityIds.length === 1) {
            ...
            this.isEntitlementDefault = function (entitlementId) {
                var isDefault = false, isDefaultAttribute = Xrm.Page.getAttribute("isdefault");
                if (!Mscrm.InternalUtilities.JSTypes.isNull(isDefaultAttribute)) {
                    isDefault = isDefaultAttribute.getValue();
                    return isDefault;
                }
                Xrm.Internal.messages.retrieve(CrmService.EntityNames.Entitlement, entitlementId, ["isdefault"])
                    .then(function (retrieveSLAResponse) {
                    var entitlementRow = retrieveSLAResponse.entity;
                    isDefault = entitlementRow.getValue("isdefault");
                }, Mscrm.InternalUtilities.ClientApiUtility.actionFailedCallback);
                return isDefault;
            };
            this.setDefaultEntitlement = function (entityId, entityName) {
                _this.updateIfEntitlementActive(entityId, entityName, ["statecode", "customerid"], null, null);
            };
            this.updateIfEntitlementActive = function (entitlementId, entityName, columnName, records, entityTypeCode) {
                var self = _this;
                Xrm.Internal.messages.retrieve(CrmService.EntityNames.Entitlement, entitlementId, columnName)
                    .then(function (retrieveEntitlementResponse) {
                    var entitlementRow = retrieveEntitlementResponse.entity;
                    if (entitlementRow.hasValue("statecode")) {
                        var columnValue = entitlementRow.getValue("statecode");
                        if (columnValue.get_value() === CrmService.EntitlementState.active) {
                            self.updateEntitlementIfNoDefaultEntitlementExists(records, entityTypeCode, entitlementId, entityName, entitlementRow.getValue("customerid"));
                        }
                        else {
                            Xrm.Utility.alertDialog(CrmService.ResourceStringProvider.getResourceString("LOCID_ENT_DEFAULT_INACTIVE_MSG"), null);
                        }
                    }
                }, Mscrm.InternalUtilities.ClientApiUtility.actionFailedCallback);
            };
            this.updateEntitlementIfNoDefaultEntitlementExists = function (records, entityTypeCode, entitlementId, entityName, customer) {
                var self = _this;
                var fetchXml = "<fetch version='1.0' mapping='logical'><entity name='entitlement'><attribute name='entitlementid' /><filter type='and'><condition attribute='isdefault' operator='eq' value='1' /></filter><filter type='and'><condition attribute='customerid' operator='eq' value='" + CrmEncodeDecode.CrmXmlAttributeEncode(customer.Id.toString()) + "' /></filter></entity></fetch>", entitlementEntities = null;
                Xrm.Internal.messages.retrieveMultiple(fetchXml)
                    .then(function (retrieveEntitlementResponse) {
                    entitlementEntities = retrieveEntitlementResponse.entityCollection;
                    var defaultEntitlementId = null;
                    if (entitlementEntities.get_count() > 0) {
                        defaultEntitlementId = entitlementEntities.get_entities()[0].getValue("entitlementid").toString();
                    }
                    self._updateDefaultEntitlementByForm$p(entitlementId, entityName, defaultEntitlementId, customer.Id.toString());
                }, Mscrm.InternalUtilities.ClientApiUtility.actionFailedCallback);
            };
            this._updateDefaultEntitlementByForm$p = function (entitlementId, entityName, defaultEntitlementId, customerId) {
                var defaultExists = "false", actionUri = Mscrm.InternalUtilities.GridUtilities.generateStandardActionUri("defaultentitlement_confirm", CrmService.EntityTypeCodes.Entitlement, 1);
                actionUri.get_query()["customerId"] = customerId;
                if (!Mscrm.InternalUtilities.JSTypes.isNullOrEmptyString(defaultEntitlementId) && entityName.toString() === "9700") {
                    defaultExists = "true";
                }
                actionUri.get_query()["defaultExists"] = defaultExists;
                var selectedRecords = [entitlementId], dialogOptions = new Xrm.DialogOptions;
                dialogOptions.width = parseInt(CrmService.ResourceStringProvider.getResourceString("LOCID_ENT_DEFAULT_DIALOG_WIDTH").toString());
                dialogOptions.height = parseInt(CrmService.ResourceStringProvider.getResourceString("LOCID_ENT_DEFAULT_DIALOG_HEIGHT").toString());
                Xrm.Internal.openDialog(actionUri.toString(), dialogOptions, selectedRecords, null, _this._performRefreshAfterUpdate$p);
            };
            this._performRefreshAfterUpdate$p = function () {
                Xrm.Page.data.refresh(true);
            };
            // Called when "activate" is clicked on entitlement main form
            this.activateRecord = function () {
                var options = new Xrm.DialogOptions;
                options.width = 610;
                options.height = 200;
                var numberOfEntities = 1;
                var confirmDialogStrings = _this.EntitlementGridCommandActions.generateActivateDialogStrings(numberOfEntities);
                CrmService.ServiceCommandBarActionsLegacyUtil.setStateRecord(CrmService.MetadataDrivenDialogConstants.EntitlementState.active, confirmDialogStrings, options);
            };
            // Called when "deactivate" is clicked on entitlement main form
            this.deactivateRecord = function () {
                var options = new Xrm.DialogOptions;
                options.width = 610;
                options.height = 200;
                var numberOfEntities = 1;
                var confirmDialogStrings = _this.EntitlementGridCommandActions.generateDeactivateDialogStrings(numberOfEntities);
                CrmService.ServiceCommandBarActionsLegacyUtil.setStateRecord(CrmService.MetadataDrivenDialogConstants.EntitlementState.draft, confirmDialogStrings, options);
            };
            this.deactivatePrimaryRecord = function (entityId, entityName) {
                _this.deactivateRecord();
            };
            this.EntitlementGridCommandActions = entitlementGridCommandActions;
        }
        ;
        return EntitlementLegacyCommandActions;
    }());
    CrmService.EntitlementLegacyCommandActions = EntitlementLegacyCommandActions;
})(CrmService || (CrmService = {}));
/**
* @license Copyright (c) Microsoft Corporation. All rights reserved.
*/
/// <reference path="../../ServiceClientCommon/Service_ClientCommon.ts" />
/// <reference path="./EntitlementLegacyGridCommandActions.ts" />
/// <reference path="./EntitlementLegacyCommandActions.ts" />
var CrmService;
(function (CrmService) {
    'use strict';
    var EntitlementLegacyLibrary = (function () {
        function EntitlementLegacyLibrary() {
            var global = window;
            var mscrm = global.Mscrm;
            mscrm.EntitlementMainSystemLibraryWebResource = new EntitlementLegacyMainSystemLibraryWebResource();
            ...
            mscrm.entityType_onchange = mscrm.EntitlementMainSystemLibraryWebResource.entityType_onchange;
        }
        ;
        return EntitlementLegacyLibrary;
    }());
    CrmService.EntitlementLegacyLibrary = EntitlementLegacyLibrary;
    var EntitlementLegacyMainSystemLibraryWebResource = (function () {
        function EntitlementLegacyMainSystemLibraryWebResource() {
            var _this = this;
            this.mapping = [];
            this.form_onload = function () {
                EntitlementLegacyMainSystemLibraryWebResource.setEntitlementTermsGridVisibility();
                Xrm.Page.ui.getFormType() !== Xrm.FormType.disabled && _this.allocationType_onchange();
                if (Xrm.Page.context.client.getClient() !== Xrm.ClientName.mobile) {
                    var attribute = Xrm.Page.getAttribute("remainingterms");
                    !Mscrm.InternalUtilities.JSTypes.isNull(attribute.controls) && attribute.controls.forEach(function (item, index) {
                        var dataControl = item;
                        dataControl.setDisabled(true);
                    });
                }
                ...
                var attrEntityType = Xrm.Page.getAttribute("entitytype");
                var ctrlEntityType = Xrm.Page.getControl("entitytype");
                if (!Mscrm.InternalUtilities.JSTypes.isNull(attrAllocationTypeCode) && !Mscrm.InternalUtilities.JSTypes.isNull(attrEntityType)) {
                    var options = attrEntityType.getOptions();
                    if (options.length > 1) {
                        var entityType = attrEntityType.getValue();
                        var selectedAllocationTypeCode = attrAllocationTypeCode.getValue();
            ...
            this.loadMapping = function (postLoad) {
                var self = _this;
                Xrm.WebApi.retrieveMultipleRecords("entitlemententityallocationtypemapping", "?$select=allocationtype,entitytype,statecode").then(function (result) {
                    if (!Mscrm.InternalUtilities.JSTypes.isNull(result) && !Mscrm.InternalUtilities.JSTypes.isNull(result.entities)) {
                        for (var i = 0; i < result.entities.length; i++) {
                            var pair = result.entities[i];
                            if (pair.statecode == 0) {
                                self.mapping.push({
                        ...
                        if (!Mscrm.InternalUtilities.JSTypes.isNull(postLoad)) {
                            postLoad();
                        }
                    }
                });
            ...
            this.filterAllocationType = function (entityType, selectedAllocationTypeCode) {
                var attrAllocationTypeCode = Xrm.Page.getAttribute("allocationtypecode");
                if (!Mscrm.InternalUtilities.JSTypes.isNull(attrAllocationTypeCode)) {
                    Xrm.Page.getControl("allocationtypecode").clearOptions();
                    var firstEligible = true;
                    for (var i = 0; i < _this.mapping.length; i++) {
                        if (_this.mapping[i].entityType == entityType) {
                            var option = attrAllocationTypeCode.getOption(_this.mapping[i].allocationType);
                            if (!Mscrm.InternalUtilities.JSTypes.isNull(option)) {
                                Xrm.Page.getControl("allocationtypecode").addOption(option);
                                // make the value set match the value displayed
                                if (firstEligible) {
                                    attrAllocationTypeCode.setValue(option.value);
                    ...
                    if (!Mscrm.InternalUtilities.JSTypes.isNull(selectedAllocationTypeCode)) {
                        attrAllocationTypeCode.setValue(selectedAllocationTypeCode);
                    }
                }
            };
            this.allocationType_onchange = function () {
                ...
                if (!(ClientUtility.DataUtil.isNullOrUndefined(allocationtypecode) || ClientUtility.DataUtil.isNullOrUndefined(decreaseRemainingOn) || ClientUtility.DataUtil.isNullOrUndefined(decreaseControl))) {
                    var options = decreaseRemainingOn.getOptions();
                    if (!Mscrm.InternalUtilities.JSTypes.isNull(allocationtypecode.getValue())) {
                        if (1 === allocationtypecode.getValue()) {
                            decreaseRemainingOn.setValue(options[0].value);
                            decreaseControl.setDisabled(true);
                            decreaseRemainingOn.setSubmitMode("always");
                            ...
                            // In web client, after deactivating the entitlement decreaseRemainingOn control is not getting enabled.
                            // checking control state, if it is disabled enabling the control
                            if (decreaseControl.getDisabled()) {
                                decreaseControl.setDisabled(false);
                            }
            ...
            this.customerid_onchange = function () {
                var oRelatedLookup = Xrm.Page.data.entity.attributes.get("customerid");
                if (!Mscrm.InternalUtilities.JSTypes.isNull(oRelatedLookup) && !Mscrm.InternalUtilities.JSTypes.isNull(oRelatedLookup.getValue())) {
                    var lookup = Xrm.Page.ui.controls.get("lookup_grid_EntitlementContacts");
                    if (!Mscrm.InternalUtilities.JSTypes.isNull(lookup)) {
                        var customerlookup = oRelatedLookup.getValue(), fetchXml = '<filter type="and"><condition attribute="parentcustomerid" operator="like" value="' + CrmEncodeDecode.CrmXmlAttributeEncode(customerlookup[0].id.toString()) + '"/></filter>';
                        lookup.addCustomFilter(fetchXml);
                        lookup.allowUserToDisableRelationshipFilter(false);
                    }
        ...
        return EntitlementLegacyMainSystemLibraryWebResource;
    }());
    //Change the visibility of the Entitlement terms grid based on whether the client is web or UCI
    EntitlementLegacyMainSystemLibraryWebResource.setEntitlementTermsGridVisibility = function () {
        var generalTab = Xrm.Page.ui.tabs.get("general");
        var gridControlToBeHidden = "grid_EntitlementChannel";
        if (!ClientUtility.ClientUtil.isUCI()) {
            gridControlToBeHidden = "editableEntitlementChannelGridControl";
    ...
    CrmService.EntitlementLegacyMainSystemLibraryWebResource = EntitlementLegacyMainSystemLibraryWebResource;
})(CrmService || (CrmService = {}));
/**
* @license Copyright (c) Microsoft Corporation. All rights reserved.
*/
/// <reference path="UCI/EntitlementLibrary.ts" />
/// <reference path="Legacy/EntitlementLegacyLibrary.ts"/>
var CrmService;
(function (CrmService) {
    'use strict';
    ...
    Entitlement.GridCommandActions = CrmService.ClientUtil.isUCI() ? new CrmService.EntitlementGridCommandActions() : new CrmService.EntitlementLegacyGridCommandActions();
    Entitlement.CommandBarActions = CrmService.ClientUtil.isUCI() ? new CrmService.EntitlementCommandActions(new CrmService.EntitlementGridCommandActions()) : new CrmService.EntitlementLegacyCommandActions(Entitlement.GridCommandActions);
    Entitlement.Instance = CrmService.ClientUtil.isUCI() ? new CrmService.EntitlementLibrary() : new CrmService.EntitlementLegacyLibrary();
    Entitlement.ctor = (function () {
        var global = window;
        var mscrm = global.Mscrm;
        mscrm.EntitlementGridCommandActions = Entitlement.GridCommandActions;
        mscrm.EntitlementCommandActions = Entitlement.CommandBarActions;
    })();
    CrmService.Entitlement = Entitlement;
})(CrmService || (CrmService = {}));
//# sourceMappingURL=Entitlement_main_system_library.js.map