...
(function (CrmService) {
    'use strict';
    var SiteLegacyLibrary = (function () {
        function SiteLegacyLibrary() {
            var _this = this;
            this.form_onload = function () {
                if (Xrm.Page.ui.getFormType() == 1 || Xrm.Page.ui.getFormType() == 5) {
                    _this.setDefaultTimeZone();
                }
            };
            this.setDefaultTimeZone = function () {
                var userid = Xrm.Page.context.getUserId();
                var columnName = "timezonecode";
                var options = "?$select=" + columnName;
                Xrm.WebApi.retrieveRecord(CrmService.EntityNames.UserSettings, userid, options).then(function (response) {
                    if (response != null) {
                        var timezoneAttribute = Xrm.Page.data.entity.attributes.get(columnName);
                        timezoneAttribute.setValue(response.timezonecode);
                    }
                }, ClientUtility.ActionFailedHandler.actionFailedCallback);
            };
        }
        ;
        return SiteLegacyLibrary;
    }());
    CrmService.SiteLegacyLibrary = SiteLegacyLibrary;
})(CrmService || (CrmService = {}));
/**
* @license Copyright (c) Microsoft Corporation. All rights reserved.
*/
/// <reference path="./Legacy/SiteLegacyLibrary.ts" />
var CrmService;
(function (CrmService) {
    'use strict';
    var Site = (function () {
    ...
    Site.Instance = new CrmService.SiteLegacyLibrary();
    Site.ctor = (function () {
        // This needed on the window because of "general" command bar actions calling hard-coded methods with some conditions
    })();
    CrmService.Site = Site;
})(CrmService || (CrmService = {}));
//# sourceMappingURL=Site_main_system_library.js.map