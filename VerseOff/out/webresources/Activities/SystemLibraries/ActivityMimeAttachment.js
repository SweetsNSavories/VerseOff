    ...
    var AddAttachment = (function () {
        ...
        AddAttachment.addEmailFollowStatusToDialogParams = function (entityName, subGridControl, dialogParams) {
            if (entityName === Activities.Common.Util.AttachmentDialogConstants.AttachmentParentEntityEmailLogicalName) {
                var emailFollowStatus = subGridControl.formContext.data.entity.attributes.get(Activities.Common.Util.AttachmentDialogConstants.IsEmailFollowed)
                    ? subGridControl.formContext.data.entity.attributes
                        .get(Activities.Common.Util.AttachmentDialogConstants.IsEmailFollowed)
                        .getValue()
                    : false;
        ...
        AddAttachment.openEditAttachmentDialog = function (primaryRecordId, gridControl, records) {
            if (Activities.Common.Util.isFCBEnabled(Activities.Constants.FCBConstant.AttachmentPreviewFCB, Activities.Constants.FCBConstant.October2020UpdateFCB) &&
                records.length === 1) {
        ...
        AddAttachment.openEditAttachmentDialogInternal = function (primaryRecordId, gridControl, records) {
            ...
            if (records != undefined && records.length == 1) {
                ...
                var filter = "?$select=" +
                    Activities.Constants.AttachmentConstants.MimeType +
                    "," +
                    Activities.Constants.AttachmentConstants.FileName +
                    "," +
                    Activities.Constants.AttachmentConstants.FileSize +
                    "," +
                    Activities.Constants.AttachmentConstants.Body;
                Xrm.WebApi.retrieveRecord(Activities.Constants.EntityNames.ActivityMimeAttachment, attachmentId, filter).then(function (entity) {
                    dialogParams[Activities.Common.Util.AttachmentDialogConstants.ParameterFileType] =
                        entity[Activities.Constants.AttachmentConstants.MimeType];
                    dialogParams[Activities.Common.Util.AttachmentDialogConstants.ParameterFileName] =
                        entity[Activities.Constants.AttachmentConstants.FileName];
                    dialogParams[Activities.Common.Util.AttachmentDialogConstants.ParameterFileSize] =
                        entity[Activities.Constants.AttachmentConstants.FileSize].toString();
                    dialogParams[Activities.Common.Util.AttachmentDialogConstants.ParameterFileContent] =
                        entity[Activities.Constants.AttachmentConstants.Body];
                    dialogParams[Activities.Common.Util.AttachmentDialogConstants.ParameterEntityName] = entityName;
                    if (Activities.Common.Util.isEmailEngagementFCBEnabled(isUci) && Activities.Common.Util.isEmailEngagementActionsFCBEnabled(isUci))
        ...
        AddAttachment.addEmailEngagementParams = function (entityName, entity, attachmentGridControl, dialogParams) {
            if (entityName === Activities.Common.Util.AttachmentDialogConstants.AttachmentParentEntityEmailLogicalName) {
                var emailFollowStatus = attachmentGridControl.formContext.data.entity.attributes.get(Activities.Common.Util.AttachmentDialogConstants.IsEmailFollowed)
                    ? attachmentGridControl.formContext.data.entity.attributes
                        .get(Activities.Common.Util.AttachmentDialogConstants.IsEmailFollowed)
                        .getValue()
                    : false;
                dialogParams[Activities.Common.Util.AttachmentDialogConstants.ParameterIsFollowed] =
                    entity[Activities.Constants.AttachmentConstants.IsFollowed];
                dialogParams[Activities.Common.Util.AttachmentDialogConstants.ParameterIsEmailFollowed] = emailFollowStatus;
            }
        };
        AddAttachment.attachmentUploadHandler = function (eventContext, telemetryItem) {
            ...
            AddAttachment.validateUploadFiles(entityId, entityName, file, telemetryItem, eventContext).catch(function () { });
        };
        AddAttachment.openDialog = function (dialogParams, entityId, attachmentId, attachmentReadMode, gridControl, telemetryItem) {
            ...
            dialogParams[Activities.Common.Util.AttachmentDialogConstants.ParameterAttachmentId] = attachmentId;
            dialogParams[Activities.Common.Util.AttachmentDialogConstants.ParameterParentEntityLogicalName] =
            ...
            Xrm.Navigation.openDialog(Activities.Common.Util.AttachmentDialogConstants.AddAttachmentToActivityDialogName, dialogOptions, dialogParams).then(function (data) {
                if (gridControl &&
                    data &&
                    dialogParams &&
                    data.parameters[Activities.Common.Util.AttachmentDialogConstants.ParameterAttachmentId] !=
                        dialogParams[Activities.Common.Util.AttachmentDialogConstants.ParameterAttachmentId]) {
                    gridControl.refresh();
                }
            }, function (errorResponse) {
                Activities.ClientApi.dialogActionFailedCallback(errorResponse, telemetryItem);
        ...
        AddAttachment.isFollowFeatureEnabled = function (isUci, documentEnableResponse) {
            return (Activities.Common.Util.isEmailEngagementFCBEnabled(isUci) &&
                Activities.Common.Util.isEmailEngagementActionsFCBEnabled(isUci) &&
                Activities.Common.Util.isOneDriveFCBEnabled(isUci) &&
                Activities.Common.Util.isSharePointFCBEnabled(isUci) &&
                Activities.Common.Util.isEmailEngagementAndOneDriveEnabledAtOrgLevel() &&
        ...
        AddAttachment.onLoad = function (eventContext) {
            ...
            telemetryItem.traceEventInformation(Activities.Constants.TelemetryConstant.EventSourceEntityType + ": " + eventContext
                .getFormContext()
                .data.attributes.get("param_entityName")
                .getValue());
            var entityName = eventContext
                .getFormContext()
                .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterEntityName)
                ? eventContext
                    .getFormContext()
                    .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterEntityName)
                    .getValue()
        ...
        AddAttachment.handleOnLoadForEmail = function (eventContext) {
            var isUci = Xrm.Internal.isUci();
            AddAttachment.isAttachmentFollowed = eventContext
                .getFormContext()
                .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterIsFollowed)
                .getValue();
            Activities.Common.Util.isDocumentManagementEnabled().then(function (response) {
                ...
                var emailFollowedStatus = eventContext
                    .getFormContext()
                    .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterIsEmailFollowed)
                    .getValue();
        ...
        AddAttachment.attachmentButtonHandler = function (eventContext) {
            var attachmentId = eventContext
                .getFormContext()
                .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterAttachmentId)
                .getValue();
            var lblSubHeading = eventContext
                .getFormContext()
                .ui.controls.get("lbl_descriptionattachment_id");
            var lblAttachMessage = eventContext
                .getFormContext()
                .ui.controls.get("attachmentControl");
        ...
        AddAttachment.toggleFollowButton = function (visible, eventContext, isFollowed) {
            if (isFollowed !== null) {
                if (eventContext.getFormContext().ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonUnFollowId) != null) {
                    eventContext
                        .getFormContext()
                        .ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonUnFollowId)
                        .setVisible(!visible && !isFollowed);
                }
                if (eventContext.getFormContext().ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonFollowId) != null) {
                    eventContext
                        .getFormContext()
                        .ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonFollowId)
                        .setVisible(!visible && isFollowed);
                }
            }
            else {
        ...
        AddAttachment.toggleButton = function (visible, eventContext, labelSubHeading, labelAttachMessage) {
            if (eventContext.getFormContext().ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonAttachId) != null) {
                eventContext
                    .getFormContext()
                    .ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonAttachId)
                    .setVisible(visible);
            }
            if (eventContext.getFormContext().ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonRemoveId) != null) {
                eventContext
                    .getFormContext()
                    .ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonRemoveId)
                    .setVisible(Activities.ClientApi.IsOffline() ? false : !visible);
            }
            if (labelSubHeading != null) {
                labelSubHeading.setLabel(visible
                    ? Activities.ClientApi.getResourceString(Activities.Common.Util.AttachmentDialogConstants.NewAttachmentFormDescription)
                    : Activities.ClientApi.getResourceString(Activities.Common.Util.AttachmentDialogConstants.ExistingAttachmentFormDescription));
            }
            if (labelAttachMessage != null) {
                labelAttachMessage.setLabel(visible
                    ? Activities.ClientApi.getResourceString(Activities.Common.Util.AttachmentDialogConstants.NewAttachmentLabelMessage) + ":"
                    : Activities.ClientApi.getResourceString(Activities.Common.Util.AttachmentDialogConstants.NewAttachmentLabelMessage));
            }
        };
        ...
        AddAttachment.remove = function (attachmentId, eventContext, telemetryItem) {
            ...
            Xrm.Navigation.openConfirmDialog(confirmDialogStrings, AddAttachment.defaultDialogOptions()).then(function (confirmResponse) {
                if (confirmResponse.confirmed) {
                    var filter = "?$select=" + Activities.Constants.AttachmentConstants.MimeType;
                    Xrm.WebApi.retrieveRecord(Activities.Constants.EntityNames.ActivityMimeAttachment, attachmentId, filter).then(function (retrieveResponse) {
                        Xrm.WebApi.deleteRecord(Activities.Constants.EntityNames.ActivityMimeAttachment, attachmentId).then(function (deleteResponse) {
                            ...
                            Xrm.Navigation.openAlertDialog(fileRemovalMessage).then(function (response) {
                                ...
                                eventContext
                                    .getFormContext()
                                    .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterFileType)
                                    .setValue(null);
                                eventContext
                                    .getFormContext()
                                    .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterFileMode)
                                    .setValue("1");
                                eventContext
                                    .getFormContext()
                                    .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterFileName)
                                    .setValue(null);
                                eventContext
                                    .getFormContext()
                                    .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterFileSize)
                                    .setValue(null);
                                var lblSubHeading = eventContext
                                    .getFormContext()
                                    .ui.controls.get("lbl_descriptionattachment_id");
                                ...
                                eventContext
                                    .getFormContext()
                                    .ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonCloseId)
                                    .setFocus();
                                eventContext
                                    .getFormContext()
                                    .ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonAttachId)
        ...
        AddAttachment.isBlockedAttachment = function (entityId, entityName, file, telemetryItem, eventContext) {
            ...
            var blockedAttachments = Xrm.Utility.getGlobalContext().organizationSettings.attributes[Activities.Common.Util.AttachmentDialogConstants.OrganizationEntityColumnBlockedAttachments];
            var maxUploadFileSize = Xrm.Utility.getGlobalContext().organizationSettings.attributes[Activities.Common.Util.AttachmentDialogConstants.OrganizationEntityColumnMaxUploadFileSize];
            ...
            var blockedAttachmentsList = blockedAttachments
                ? blockedAttachments.toLowerCase().split(";")
                : [];
            ...
            return Xrm.WebApi.createRecord(Activities.Constants.EntityNames.ActivityMimeAttachment, attachmentEntity).then(function (createdAttachmentEntityRef) {
                if (eventContext) {
                    eventContext
                        .getFormContext()
                        .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterAttachmentId)
                        .setValue(createdAttachmentEntityRef.id);
                    eventContext
                        .getFormContext()
                        .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterFileMode)
                        .setValue("0");
        ...
        AddAttachment.followAttachmentClick = function (eventContext) {
            ...
            temeletryItem.traceEventInformation(Activities.Constants.TelemetryConstant.EventSourceEntityType + ": " + eventContext
                .getFormContext()
                .data.attributes.get("param_entityName")
                .getValue());
            eventContext
                ...
                .ui.controls.get(Activities.Common.Util.AttachmentDialogConstants.ButtonFollowId)
                .setDisabled(true);
            var activityMimeAttachmentId = eventContext
                .getFormContext()
                .data.attributes.get(Activities.Common.Util.AttachmentDialogConstants.ParameterAttachmentId)
                .getValue();
            if (activityMimeAttachmentId != null) {
                var FollowEmailAttachmentRequest_1 = new Activities.FollowEmailAttachmentRequest(activityMimeAttachmentId);
                AddAttachment.handleAttachmentFollow(eventContext, temeletryItem, FollowEmailAttachmentRequest_1, Activities.Constants.TelemetryConstant.EventFollowAttachment, Activities.Common.Util.AttachmentDialogConstants.ButtonFollowId, false);
            }
        };
        AddAttachment.unFollowAttachmentClick = function (eventContext) {
            ...
            eventContext
                ...
                .setDisabled(true);
            if (activityMimeAttachmentId != null) {
                var UnfollowEmailAttachmentRequest_1 = new Activities.UnfollowEmailAttachmentRequest(activityMimeAttachmentId);
                AddAttachment.handleAttachmentFollow(eventContext, telemetryItem, UnfollowEmailAttachmentRequest_1, Activities.Constants.TelemetryConstant.EventUnFollowAttachment, Activities.Common.Util.AttachmentDialogConstants.ButtonUnFollowId, true);
            }
        };
        ...
        AddAttachment.downloadAttachmentDialog = function (eventContext) {
            var attachmentId = eventContext
                ...
                .getValue();
            if (attachmentId) {
                var telemetryItem_1 = new TelemetryLogger.TelemetryItem(Activities.Constants.EntityNames.ActivityMimeAttachment, Activities.Constants.TelemetryConstant.EventDownloadAttachmentOctober2020);
                telemetryItem_1.traceEventInformation("Downloading attachment");
                telemetryItem_1.report();
                Activities.Common.AttachmentDownloadUtility.downloadAttachment(attachmentId).catch(function (errorResponse) {
                    telemetryItem_1.traceEventError("Failed to download attachment", errorResponse);
                    telemetryItem_1.report();
        ...
        AddAttachment.downloadAttachments = function (formContext, records) {
            if (records && records.length > 0) {
                ...
                var telemetryItem_2 = new TelemetryLogger.TelemetryItem(Activities.Constants.EntityNames.ActivityMimeAttachment, Activities.Constants.TelemetryConstant.EventDownloadAttachmentOctober2020);
                Promise.all(promises_1)
                    ...
                    telemetryItem_2.traceEventInformation("Downloaded records");
                    telemetryItem_2.report();
                })
                    .catch(function (errorResponse) {
                    telemetryItem_2.traceEventError("Failed to download attachment", errorResponse);
                    telemetryItem_2.report();
                    formContext.ui.clearFormNotification(Activities.Constants.AttachmentConstants.AttachmentFormNotificationId);
                    Xrm.Navigation.openAlertDialog({
                        text: Activities.ClientApi.getResourceString(Activities.Constants.AttachmentConstants.AttachmentDownloadGenericError),
        ...
        AddAttachment.uploadAttachments = function (gridControl) {
            ...
            Xrm.Device.pickFile(options).then(function (files) {
                ...
                if (errorResponse && errorResponse.errorCode === Activities.Constants.AttachmentConstants.FilePickerCancelledCode) {
                    return;
                }
                Xrm.Navigation.openAlertDialog({
        ...
        AddAttachment.setThumbnailImage = function (dataSetRecord, userLCID) {
            ...
            if (dataSetRecord && thumbnailFCB) {
                var telemetryItem_3 = new TelemetryLogger.TelemetryItem(Activities.Constants.EntityNames.ActivityMimeAttachment, Activities.Constants.TelemetryConstant.EventLoadAttachmentThumbnail);
                var attachmentRecord_1 = null;
                try {
                    attachmentRecord_1 = JSON.parse(dataSetRecord);
                }
                catch (err) {
                    telemetryItem_3.traceEventError("Failed to parse grid dataset record json", err);
                    telemetryItem_3.report();
                    return null;
                }
                ...
                if (mimeType_1 && mimeType_1.indexOf("image") !== -1) {
                    ...
                    return Xrm.WebApi.retrieveRecord(Activities.Constants.EntityNames.ActivityMimeAttachment, attachmentId_1, filter).then(function (attachment) {
                        ...
                        return [
                            Activities.Common.AttachmentBlobUtility.createObjectUrl(attachmentId_1, body, mimeType_1),
                            attachmentRecord_1.filename,
                        ];
                    }, function (err) {
                        telemetryItem_3.traceEventError("Failed to retrieve body from attachment", err);
                        telemetryItem_3.report();
                        return null;
                    });
                }
...
(function (Activities) {
    ...
    (function (Constants) {
        ...
        (function (AttachmentConstants) {
            ...
            AttachmentConstants.OpenFileSaveMode = 2;
            ...
            AttachmentConstants.FilePickerCancelledCode = -2147094125;
