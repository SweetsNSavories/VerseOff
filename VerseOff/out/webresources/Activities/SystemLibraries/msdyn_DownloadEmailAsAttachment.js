...
var Activities;
(function (Activities) {
    var DownloadEmailAsAttachment = (function () {
        function DownloadEmailAsAttachment() {
        }
        DownloadEmailAsAttachment.OnClick = function (formContext) {
            var telemetryItem = new TelemetryLogger.TelemetryItem(Activities.Constants.TelemetryConstant.DownloadEmailAsAttachment, Activities.Constants.TelemetryConstant.DownloadEmailAsAttachment);
            telemetryItem.traceEventInformation(Activities.Constants.TelemetryConstant.DownloadEmailAsAttachment + " : Preparing request");
            var emailId = formContext.data.entity.getId();
            var globalContext = Xrm.Utility.getGlobalContext();
            var serverURL = globalContext.getClientUrl();
            var actionName = Activities.Constants.Actions.DownloadEmailAsAttachment;
            Xrm.Utility.showProgressIndicator(Activities.ClientApi.getResourceString("DownloadEmailAsAttachment_PreparingFile"));
            var data = {
                "emailIdToBeDownloaded": emailId,
            };
            ...
            req.onreadystatechange = function () {
                if (this.readyState == 3) {
                    ...
                    var downloadedFile = this.response.length * 3 / 4194304;
                    downloadedFile = downloadedFile.toFixed(2);
                    var str = Activities.ClientApi.getResourceString("DownloadEmailAsAttachment_Download") + downloadedFile.toString() + Activities.ClientApi.getResourceString("DownloadEmailAsAttachment_MB");
                    Xrm.Utility.showProgressIndicator(str.toString());
                }
                if (this.readyState == 4) {
                    req.onreadystatechange = null;
                    if (this.status == 200 || this.status == 204) {
                        var result = JSON.parse(this.response);
                        var emailRecordData = result.emailRecordData;
                        var emailRecordDataObject = atob(emailRecordData);
                        var textFile = null;
                        var data = new Blob([emailRecordDataObject], { type: 'text/plain' });
                        if (textFile !== null) {
                            window.URL.revokeObjectURL(textFile);
                        }
                        textFile = window.URL.createObjectURL(data);
                        var emailSubject = formContext.getAttribute("subject") != null ? formContext.getAttribute("subject").getValue() : null;
                        if (!(typeof emailSubject != 'undefined' && emailSubject)) {
                            emailSubject = DownloadEmailAsAttachment.getSubject(emailRecordDataObject);
                        }
                        ...
                        a.href = textFile;
                        a.id = 'fileLink';
                        a.download = (emailSubject.length > 0 ? emailSubject.trim() : "file") + ".eml";
                        a.style.visibility = "hidden";
                        a.click();
                        Xrm.Utility.closeProgressIndicator();
                        telemetryItem.traceEventInformation(Activities.Constants.TelemetryConstant.DownloadEmailAsAttachment + " : Email download completed");
                        telemetryItem.report();
                    }
                    else {
                        ...
                        telemetryItem.traceEventError(Activities.Constants.TelemetryConstant.DownloadEmailAsAttachment + " : Error while downloading email", error.message);
                        telemetryItem.report();
                    }
                }
            };
            req.send(JSON.stringify(data));
            telemetryItem.traceEventInformation(Activities.Constants.TelemetryConstant.DownloadEmailAsAttachment + " : Request sent successfully");
        };
        DownloadEmailAsAttachment.getSubject = function (email) {
            var subjectMatch = email.match(/^Subject: (.*)$/m);
            return subjectMatch ? subjectMatch[1] : "Attachment";
        };
        return DownloadEmailAsAttachment;
    }());
    Activities.DownloadEmailAsAttachment = DownloadEmailAsAttachment;
})(Activities || (Activities = {}));
//# sourceMappingURL=DownloadEmailAsAttachment.js.map