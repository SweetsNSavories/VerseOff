...
Mscrm.Form_onload = function Mscrm_Form_onload(executionContext) {
	...
	//if form type is BulkEdit we are hiding Header tab to avoid duplicate owner field in the form
	if (formContext.ui.getFormType() == 6) {
		if (formContext.ui.tabs.getByName('HEADER') != null && formContext.ui.tabs.getByName('HEADER') != undefined) {
			formContext.ui.tabs.getByName('HEADER').setVisible(false);
        }
	}
	else {
		if (_isUci) {
			...
			if (!isTemplateUCIEnabled) {
				...
				if (sections) {
					...
					var enableDynamicTextForSignature = isFCBEnabled("EnableDynamicTextForSignature", null);
					if (enableDynamicTextForSignature) {
						Activities.Template.RegisterCkeditorEvents(this, formContext);
					}
				}
			}
		} else {
...
function setCaret(text, isLeft, eventData) {
	...
	if (!IsNull(range))
		if (!IsNull(text) && text.nodeType === 3) {
			...
			if (!text.nodeValue.match(new RegExp("^[\\s" + escape(ThinSpace).replace(/%/g, "\\") + "]+"))) {
