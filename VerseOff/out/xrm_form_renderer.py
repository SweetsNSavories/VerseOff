...
class LookupWidget(QWidget):
    ...
    def open_lookup_dialog(self):
        ...
        target_entities = list(self.target_entities) if self.target_entities else []
        if not target_entities:
            fname = getattr(self, "field_name", "") or ""
            if "owner" in fname:
                target_entities = ["systemuser"]
            elif "customer" in fname:
                target_entities = ["account", "contact"]
            elif "contact" in fname:
                target_entities = ["contact"]
            elif "regarding" in fname:
                target_entities = ["account", "contact", "incident", "opportunity", "lead"]
            else:
                target_entities = ["account", "contact"]

        active_target_holder = [target_entities[0]]
        
        if len(target_entities) > 1:
            lbl = QLabel("Look for:")
            lbl.setStyleSheet("font-weight: 600; color: #323130;")
            search_layout.addWidget(lbl)
            entity_combo = QComboBox()
            entity_combo.setStyleSheet("padding: 4px 10px; border: 1px solid #d1d1d1; border-radius: 4px; font-size: 12px;")
            for te in target_entities:
                disp = te.replace("_", " ").title()
                entity_combo.addItem(disp, te)
            search_layout.addWidget(entity_combo)
        else:
            entity_combo = None

        search_bar = QLineEdit()
        search_bar.setPlaceholderText("Search records...")
        search_bar.setStyleSheet("padding: 5px 10px; border: 1px solid #d1d1d1; border-radius: 4px; font-size: 12px;")
        search_layout.addWidget(search_bar, 1)
        ...
        cancel_btn.setStyleSheet("padding: 6px 18px; border: 1px solid #d1d1d1; border-radius: 4px; background: white; font-weight: 600; font-size: 12px;")
        cancel_btn.clicked.connect(dialog.reject)
        
        select_btn = QPushButton("Select")
        select_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        select_btn.setStyleSheet("padding: 6px 22px; border: none; border-radius: 4px; background: #0f6cbd; color: white; font-weight: 600; font-size: 12px;")
        btn_layout.addWidget(cancel_btn)
        btn_layout.addWidget(select_btn)
        layout.addLayout(btn_layout)

        def load_data(search_string=None):
            active_target = active_target_holder[0]
            target_metadata = next(
                (
                    entity
                    for entity in self.renderer.manifest.get("entities", [])
                    if entity.get("LogicalName") == active_target
                ),
                {},
            )
            primary_id_col = (
                target_metadata.get("PrimaryIdAttribute")
                or f"{active_target}id"
            )
            primary_name_col = (
                target_metadata.get("PrimaryNameAttribute")
                or ("fullname" if active_target in ["contact", "systemuser", "lead"] else "name")
            )

            try:
                from view_parser import ViewParser
                database = LocalDatabase()
                database._validate_entity_name(active_target)
                with database.get_connection() as conn:
                    database._ensure_table(conn, active_target)
                    if active_target == "systemuser":
                        cursor = conn.cursor()
                        cursor.execute("SELECT COUNT(*) FROM systemuser")
                        if cursor.fetchone()[0] == 0:
                            sample_users = [
                                {"systemuserid": "449f240d-544b-f111-bec6-6045bdad5157", "fullname": "admin thonda", "internalemailaddress": "admin@praveenthondahotmail.onmicrosoft.com"},
                                {"systemuserid": "00000000-0000-0000-0000-000000000001", "fullname": "SYSTEM", "internalemailaddress": "system@crm.dynamics.com"},
                                {"systemuserid": "00000000-0000-0000-0000-000000000002", "fullname": "Sales Manager", "internalemailaddress": "salesmanager@contoso.com"}
                            ]
                            for u in sample_users:
                                conn.execute("INSERT OR REPLACE INTO systemuser (id, data_json, sync_status) VALUES (?, ?, 'synced')",
                                             (u["systemuserid"], json.dumps(u)))
                            conn.commit()

                    cursor = conn.cursor()
                    cursor.execute("SELECT fetchxml, layoutxml FROM saved_queries WHERE returnedtypecode = ? AND querytype = 64", (active_target,))
                    view_row = cursor.fetchone()
                    if not view_row and search_string:
                        cursor.execute("SELECT fetchxml, layoutxml FROM saved_queries WHERE returnedtypecode = ? AND querytype = 4", (active_target,))
                        view_row = cursor.fetchone()
                    ...
                    if not columns:
                        if active_target == "contact":
                            columns = [
                                {"name": "fullname", "label": "Full Name"},
                                {"name": "emailaddress1", "label": "Email"},
                                {"name": "telephone1", "label": "Business Phone"},
                            ]
                        elif active_target == "account":
                            columns = [
                                {"name": "name", "label": "Account Name"},
                                {"name": "telephone1", "label": "Main Phone"},
                                {"name": "address1_city", "label": "City"},
                            ]
                        elif active_target == "systemuser":
                            columns = [
                                {"name": "fullname", "label": "User Name"},
                                {"name": "internalemailaddress", "label": "Email"},
                            ]
                        elif active_target == "incident":
                            columns = [
                                {"name": "title", "label": "Case Title"},
                                {"name": "ticketnumber", "label": "Case Number"},
                            ]
                        elif active_target == "opportunity":
                            columns = [
                                {"name": "name", "label": "Topic"},
                                {"name": "estimatedvalue", "label": "Est. Revenue"},
                            ]
                        elif active_target == "lead":
                            columns = [
                                {"name": "fullname", "label": "Full Name"},
                    ...
                    columns = [c for c in columns if not c.get("ishidden")]
                    table.setColumnCount(len(columns))
                    table.setHorizontalHeaderLabels([c.get('label', c['name']).replace("_", " ").title() for c in columns])

                    cursor.execute(f"SELECT id, data_json FROM {active_target}")
                    rows = []
                    search_val = str(search_string or "").casefold()
                    for stored_row in cursor.fetchall():
                        try:
                            record = database._deserialize_data(stored_row["data_json"])
                        except Exception:
                            try:
                        ...
                        if search_val:
                            match = any(
                                search_val in str(val or "").casefold()
                    ...
                    for i, row in enumerate(rows):
                        rec_id = str(row.get(primary_id_col, ""))
                        display_name = str(row.get(primary_name_col) or rec_id)
                ...
                table.setRowCount(0)
                logger.warning("Lookup data load issue: %s", error)

        def do_select():
            selected = table.selectedItems()
            if selected:
                row_idx = selected[0].row()
                first_cell = table.item(row_idx, 0)
                if first_cell:
                    self.current_id = first_cell.data(Qt.ItemDataRole.UserRole)
                    self.current_logical_name = active_target_holder[0]
        ...
        search_bar.textChanged.connect(lambda t: load_data(t.strip()))
        
        if entity_combo:
            def on_entity_changed(idx):
                active_target_holder[0] = entity_combo.itemData(idx)
                load_data(search_bar.text().strip())
            entity_combo.currentIndexChanged.connect(on_entity_changed)
...
class XrmFormRenderer(QWidget):
    ...
    def _get_options_for_field(self, field_name: str) -> list:
        opts = (self.entity_def.get("option_sets") or {}).get(field_name)
        if opts:
            return opts
        for attr in self.entity_def.get("attributes", []):
            if attr.get("LogicalName") == field_name:
                raw_opts = (attr.get("OptionSet") or {}).get("Options", [])
                if raw_opts:
                    result = []
                    for o in raw_opts:
                        val = o.get("Value")
                        lbl = o.get("Label", {}).get("UserLocalizedLabel", {}).get("Label") or str(val)
    ...
    def _create_widget_for_field(
        ...
        # Check for Lookup / Customer / Owner targets
        lookup_targets = self.entity_def.get("lookup_targets", {}).get(field_name)
        if not lookup_targets and (
            attr_type in ["Lookup", "Customer", "Owner"]
            or field_name.endswith("id")
            or field_name.startswith("_")
            or "owner" in field_name
            or "customer" in field_name
            or "regarding" in field_name
        ):
            if field_name in ("ownerid", "_ownerid_value") or "owner" in field_name:
                lookup_targets = ["systemuser"]
            elif field_name in ("customerid", "parentcustomerid") or "customer" in field_name:
                lookup_targets = ["account", "contact"]
            elif field_name in ("primarycontactid", "parentcontactid"):
                lookup_targets = ["contact"]
            elif field_name in ("parentaccountid",):
                lookup_targets = ["account"]
            elif field_name in ("regardingobjectid", "_regardingobjectid_value") or "regarding" in field_name:
                lookup_targets = ["account", "contact", "incident", "opportunity", "lead"]

        if attr_type in ["Lookup", "Customer", "Owner"] or lookup_targets:
            targets = lookup_targets or (target_entity and [target_entity]) or ["account", "contact"]
            widget = LookupWidget(self, targets)
    ...
    def _set_widget_value(self, field, widget, row):
        ...
        if isinstance(widget, QLineEdit):
            ...
            widget.current_id = str(lookup_id) if lookup_id is not None else None
            ...
            # If FormattedValue was missing, or equal to the raw GUID, resolve from SQLite!
            if lookup_id and (not display_value or str(display_value) == str(lookup_id)):
                try:
                    db = LocalDatabase()
                    candidates = []
                    if widget.current_logical_name:
                        candidates.append(widget.current_logical_name)
                    if hasattr(widget, "target_entities") and widget.target_entities:
                        for t in widget.target_entities:
                            if t not in candidates:
                                candidates.append(t)
                    if "owner" in field and "systemuser" not in candidates:
                        candidates.append("systemuser")
                    
                    for target_ent in candidates:
                        rec = db.get_record(target_ent, str(lookup_id))
                        if rec:
                            widget.current_logical_name = target_ent
                            pname = rec.get("fullname") or rec.get("name") or rec.get("title") or rec.get("subject")
                            if pname:
                                display_value = pname
                                break
                except Exception:
                    pass
            widget.setText(str(display_value) if display_value is not None else (str(lookup_id) if lookup_id else ""))
        elif isinstance(widget, PcfControlWidget):
            widget.setValue(value)
        elif hasattr(widget, "setValue") and callable(widget.setValue):
            widget.setValue(value)
